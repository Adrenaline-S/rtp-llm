...
def _dsv4_kv_cache_desc_for_tag(
    ...
    if tag in ("csa_kv", "hca_kv", "indexer_kv"):
        ...
        # FULL descriptors must carry the attention-kernel page size; the
        # config creator validates it against attn_config.kernel_tokens_per_block.
        kernel_tokens = int(config.attn_config.kernel_tokens_per_block)
        if kernel_tokens <= 0:
            kernel_tokens = int(config.attn_config.tokens_per_block)
        desc.kernel_seq_size_per_block = kernel_tokens
...
def _refresh_dsv4_kv_cache_spec_descs(config: ModelConfig) -> None:
    ...
    # The hybrid-pool config path requires an explicit per-layer attention-type
    # row; DSV4 routes purely by cache-spec tags, so every layer is NONE.
