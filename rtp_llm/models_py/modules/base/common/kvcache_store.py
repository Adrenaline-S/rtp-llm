...
class WriteCacheStoreOp(nn.Module):
    ...
    def _block_ids_for_layer_cache(
        ...
        cache_tag = getattr(kv_cache, "cache_tag", None)
        if gid < 0 or gid >= len(self._block_ids_by_group):
            raise RuntimeError(
                "missing cache-store block table for owned KV cache tag: "
                f"layer_id={layer_id}, cache_tag={cache_tag}, group_id={gid}, "
        ...
        if block_ids is None or block_ids.numel() == 0:
            raise RuntimeError(
                "empty cache-store block table for owned KV cache tag: "
                f"layer_id={layer_id}, cache_tag={cache_tag}, group_id={gid}"
            )
        return block_ids
...
def create_write_cache_store_impl(
    ...
    has_multi_tag_group = (
        ...
        and bool(getattr(kv_cache, "layer_tag_to_group_id", None))
        and bool(getattr(kv_cache, "group_tags", None))
        and bool(getattr(attn_inputs, "kv_cache_kernel_block_id_host_by_group", None))
    )
    if has_multi_tag_group:
