...
Generic ``(layer, cache_tag) -> block_table`` helpers shared between
prefill and decode. Kept separate from DSV4-local attn type constants
(pure int constants, no torch) and from path-specific forward helpers in
...
from rtp_llm.models_py.modules.dsv4.attn_type import TAG_TO_ATTN_TYPE
...
def build_block_tables(
    ...
    ``kv_cache.group_tags`` to produce a dict keyed by DSV4-local attn_type
    integer value instead of group id.
    ...
    for group_id, tag in enumerate(group_tags):
        ...
        if group_block_table is None or group_block_table.numel() == 0:
            continue
        block_tables_by_attn_type[attn_type] = group_block_table[batch_offset : batch_offset + 1]
