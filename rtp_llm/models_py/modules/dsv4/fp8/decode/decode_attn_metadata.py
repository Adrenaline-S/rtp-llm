...
class DSv4DecodeAttnMetadataFP8:
    ...
    paged_pool_tokens_per_block: Dict[str, int] = field(default_factory=dict)

    # Per-tag new-token write slot mapping: [max_T_total] int64.
    ...
    pool_write_slot_mappings: Dict[str, torch.Tensor] = field(default_factory=dict)

    # Per-tag compressor state-pool slot mapping: [max_T_total] int64.
    ...
    compressor_state_slot_mappings: Dict[str, torch.Tensor] = field(
    ...
    # views via ``self._kv_cache.get_layer_cache(layer_id, tag)``
...
def _pool_compress_ratio(tag: str) -> int:
    from rtp_llm.models_py.modules.dsv4.kv_cache_utils import CSA_KV, HCA_KV, INDEXER_KV

    if tag in (CSA_KV, INDEXER_KV):
        return 4
    if tag == HCA_KV:
        return 128
    return 1
...
def _parse_paged_pool_specs(
    paged_pool_specs: Optional[Dict[str, Tuple[int, int, int]]],
) -> Tuple[Dict[str, int], Dict[str, int], Dict[str, int]]:
    """Parse ``cache tag -> (entries, raw_tokens_per_block, max_blocks)``."""
    entries_by_pool: Dict[str, int] = {}
    tokens_by_pool: Dict[str, int] = {}
    max_blocks_by_pool: Dict[str, int] = {}
    ...
    for tag, spec in paged_pool_specs.items():
        ...
        entries_by_pool[str(tag)] = entries_per_block
        tokens_by_pool[str(tag)] = tokens_per_block
        max_blocks_by_pool[str(tag)] = max_blocks
    return entries_by_pool, tokens_by_pool, max_blocks_by_pool


def _resolve_paged_pool_tokens_per_block(
    entries_by_pool: Dict[str, int],
    tokens_by_pool: Optional[Dict[str, int]],
) -> Dict[str, int]:
    ...
    for tag in entries_by_pool:
        ...
        if tokens_per_block <= 0:
            raise ValueError(
                ...
                f"got tag={tag}, tokens_per_block={tokens_per_block}"
            )
        resolved[str(tag)] = tokens_per_block
    return resolved
...
def _compressed_domain_tokens_per_block(
    ...
    ratio = _pool_compress_ratio(tag)
    if ratio <= 1:
...
def _update_compressor_state_slot_mappings(
    ...
    bs: int,
    paged_pool_entries_per_block: Dict[str, int],
) -> None:
...
def allocate_decode_metadata_fp8(
    ...
    index_topk: int,
    device: torch.device,
    paged_pool_specs: Optional[Dict[str, Tuple[int, int, int]]] = None,
    ...
    #   cache tag → (entries_per_block, tokens_per_block, max_blocks_per_req)
    ...
    compressor_state_slot_mappings: Dict[str, torch.Tensor] = {}
...
def update_decode_metadata_in_place_fp8(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    # Phase 2: paged write slot mappings (per cache tag).
...
def build_decode_metadata_fp8(
    ...
    *,
    ...
    pool_write_slot_mappings: Dict[str, torch.Tensor] = {}
    compressor_state_slot_mappings: Dict[str, torch.Tensor] = {}
    paged_pool_tokens_per_block_resolved: Dict[str, int] = {}
    req_id_per_token: Optional[torch.Tensor] = None
    req_id_per_token_long: Optional[torch.Tensor] = None
