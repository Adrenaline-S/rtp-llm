...
class GenericMoeModel(GptModelBase):
    ...
    def prepare_fmha_impl(
        ...
        tags = ("default", "indexer_kv")
        group_inputs = {
            tag: select_attention_inputs_for_tag(attention_inputs, tag) for tag in tags
        }
        ...
        use_fast_path = resolve_mla_use_fast_path(
            attn_configs, group_inputs["default"], self.parallelism_config
        )
        return {
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers[: self.layer_num]):
            if self.config.attn_config.is_sparse:
                layer_fmha_impl = {
