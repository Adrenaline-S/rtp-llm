...
void runtimeWriteCacheStore(const CacheStoreInputs&     cache_store_inputs,
    ...
    for (size_t batch_id = 0; batch_id < param.context_batch_size; batch_id++) {
        ...
        const bool uses_cp_canonical_keys = param.cp_size > 1
                                            && group_policy.cp_mapping != CpBlockMappingMode::NONE
        ...
        const int total_blocks = uses_cp_canonical_keys ?
                                     (canonical_total_blocks + param.cp_size - 1) / param.cp_size :
        ...
        auto addBlock = [&](int key_index, int offset_index) {
            ...
            const bool       is_swa_cp_slice        = !has_policy_cp_slice && param.tag == "swa_kv" && param.cp_size > 1
                                         && kv_block_stride_bytes % kDsv4SwaFp8EntryBytes == 0;
            ...
            if (kv_scale_data) {
                ...
                if (use_opaque_key_prefix) {
                    ...
                    RTP_LLM_CHECK_WITH_INFO(kv_scale_stride_bytes % 2 == 0,
                                            "scale stride must split evenly into K/V");
        ...
        // cache_keys stays in the full logical namespace. CPSlotMapper owns
        // the key/offset projection for both legacy and sharded cases.
        ...
        CPSlotMapper cp_mapper(param.cp_rank, param.cp_size, static_cast<int>(seq_size_per_block));
        const auto   block_plan = cp_mapper.buildStorePlan(
            group_policy,
