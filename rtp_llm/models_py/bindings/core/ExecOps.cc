...
void runtimeWriteCacheStore(const CacheStoreInputs&     cache_store_inputs,
    ...
    const auto kv_block_transfer_bytes = kv_transfer_it != param.kv_block_transfer_bytes_by_tag.end() ?
                                             kv_transfer_it->second :
    ...
    RTP_LLM_CHECK_WITH_INFO(kv_block_transfer_bytes > 0,
                            "cache-store tag=%s has zero kv transfer bytes",
                            param.tag.c_str());
    ...
    for (size_t batch_id = 0; batch_id < param.context_batch_size; batch_id++) {
        ...
        auto addBlock = [&](int key_index, int offset_index) {
            ...
            const bool       is_swa_cp_slice        = !has_policy_cp_slice && param.tag == "swa_kv" && param.cp_size > 1
                                         && kv_block_transfer_bytes % kDsv4SwaFp8EntryBytes == 0;
            ...
            if (is_swa_cp_slice) {
                ...
                const size_t          local_entries       = kv_block_transfer_bytes / kDsv4SwaFp8EntryBytes;
