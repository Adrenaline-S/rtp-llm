...
static std::shared_ptr<KVCacheSpec> makeTestSpec(const std::string& tag, size_t tokens_per_block, bool mla_cache) {
    ...
    spec->kernel_seq_size_per_block   = static_cast<uint32_t>(tokens_per_block);
    return spec;
}
...
static CacheConfig makeCacheConfig(size_t             tokens_per_block,
    ...
    config                    = CacheConfig(std::move(groups), std::move(layers), static_cast<uint32_t>(layer_id + 1));
...
TEST_F(ExecOpsTest, testWriteCacheStoreCallbackFailureReachesPublicationWait) {
    ...
    EXPECT_NO_THROW(runtimeWriteCacheStore(inputs,
                                          ...
                                          [&writer](const std::vector<int64_t>&,
                                                    ...
                                                    const std::string&) { return writer.registerStoreCompletion(); }));
...
TEST_F(ExecOpsTest, testWriteCacheStoreSharedPoolUsesPhysicalBlockStrideInsteadOfLayerViewStride) {
    ...
    EXPECT_EQ(it->second.len, pool_block_stride);
...
TEST_F(ExecOpsTest, testWriteCacheStoreSameLayerRoutesByTag) {
    ...
    CacheConfig config({make_group("linear", CacheGroupType::LINEAR), make_group("full", CacheGroupType::FULL)},
                       {{"linear", "full"}},
                       /*main_layer_num=*/1);
