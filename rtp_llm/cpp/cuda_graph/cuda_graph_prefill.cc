...
void CudaGraphRunner::capturePrefill() {
    ...
    for (int i = capture_range_size - 1; i >= 0; i--) {
        ...
        // MTP draft prefill executes a fixed-capacity decode-shaped forward,
        // so its output remains max_bs_ * num_tokens_per_bs_ for every key.
        // Embedding prefill produces the real flattened seq_len instead.
        if (isEmbeddingStylePrefillCudaGraph()) {
