...
bool validateGroupedBlockTable(const torch::Tensor& source,
...
void CudaGraphRunner::prepareInputs(const PyModelInputs& inputs, CudaGraphState& state) {
    ...
    // Grouped inputs own their block tables. The common table may alias one capture group, so writing
...
bool CudaGraphRunner::canReplaySelectedGraph(const PyModelInputs& inputs, const CudaGraphState& state) const {
    ...
    if (!inputs.group_attention_inputs.empty()) {
        const auto& captured_groups = graph_it->second.mem_hold_.py_model_inputs_.group_attention_inputs;
