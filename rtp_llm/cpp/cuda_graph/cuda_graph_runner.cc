...
void CudaGraphRunner::initCaptureAttentionInputs(PyModelInputs& inputs, int max_bs, int num_tokens_per_bs) {
    ...
    if (kv_cache_group_tags_.size() > 1) {
        for (size_t slot = 0; slot < kv_cache_group_tags_.size(); ++slot) {
            ...
            const auto [it, inserted] =
                inputs.attention_inputs_by_tag.emplace(kv_cache_group_tags_[slot], std::move(tagged_inputs));
            (void)it;
            RTP_LLM_CHECK_WITH_INFO(
                inserted, "duplicate CUDA graph KV cache tag=%s", kv_cache_group_tags_[slot].c_str());
        }
    }
}
...
void CudaGraphRunner::prepareCaptureInputs(PyModelInputs& inputs, int batch_size, int seq_len_or_tokens) {
    ...
    inputs.attention_inputs.is_prefill       = is_prefill_cuda_graph_mode_ || num_tokens_per_bs_ > 1;
    inputs.attention_inputs.is_target_verify = is_target_verify_;
    // MTP draft prefill graphs execute the decode-shaped Python path, where
    // T is derived from B * q_len_per_req. Keep token/hidden buffers at the
    // fixed graph capacity even when the current capture key is smaller;
    // embedding-style prefill still consumes the real flattened token slice.
