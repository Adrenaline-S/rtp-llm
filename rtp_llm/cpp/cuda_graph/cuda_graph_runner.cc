...
bool CudaGraphRunner::canRun(const PyModelInputs& inputs, CudaGraphState& state) {
    ...
    if (kv_cache_groups_.size() > 1) {
        ...
        for (const auto& [tag, type] : kv_cache_groups_) {
            (void)type;
            const auto input_it = inputs.attention_inputs_by_tag.find(tag);
            if (input_it == inputs.attention_inputs_by_tag.end()) {
                RTP_LLM_LOG_WARNING("Tagged kv cache is missing tag=%s, fallback to normal run.", tag.c_str());
                return false;
            }
            const auto captured_it = capture_mem_hold_.py_model_inputs_.attention_inputs_by_tag.find(tag);
            if (captured_it == capture_mem_hold_.py_model_inputs_.attention_inputs_by_tag.end()) {
...
void CudaGraphRunner::initCaptureAttentionInputs(PyModelInputs& inputs, int max_bs, int num_tokens_per_bs) {
    ...
    if (kv_cache_groups_.size() > 1) {
        for (const auto& [tag, type] : kv_cache_groups_) {
            (void)type;
            const auto    physical_it   = physical_tokens_per_block_by_tag_.find(tag);
            const auto    kernel_it     = kernel_tokens_per_block_by_tag_.find(tag);
            const int64_t physical_span = physical_it == physical_tokens_per_block_by_tag_.end() ?
                                              static_cast<int64_t>(seq_size_per_block_) :
                                              static_cast<int64_t>(physical_it->second);
            const int64_t kernel_span   = kernel_it == kernel_tokens_per_block_by_tag_.end() ?
