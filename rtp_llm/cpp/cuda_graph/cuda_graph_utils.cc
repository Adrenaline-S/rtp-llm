...
void refreshGroupedAttentionInputs(torch_ext::PyModelInputs& inputs) {
    for (auto& [tag, grouped_inputs] : inputs.group_attention_inputs) {
        (void)tag;
        auto kernel_block_id                          = grouped_inputs.kv_cache_kernel_block_id;
        auto kernel_block_id_device                   = grouped_inputs.kv_cache_kernel_block_id_device;
        grouped_inputs                                 = inputs.attention_inputs;
        grouped_inputs.kv_cache_kernel_block_id        = std::move(kernel_block_id);
        grouped_inputs.kv_cache_kernel_block_id_device = std::move(kernel_block_id_device);
