...
bool KVCacheResource::maintainsTailBlocksOnly(std::string_view tag) const {
    if (topology_ == nullptr || !topology_->containsTag(tag)) {
        return false;
    }
    // LINEAR keeps 1 tail block and SWA keeps 2, while FULL leaves this at 0. The same predicate
    // already decides tail-block transfer in buildCacheStorePlan.
    return topology_->group(tag).policy.active_tail_blocks > 0;
}
