...
std::vector<size_t> cacheTransferPositions(
    size_t block_num, size_t reuse_block_size, bool use_hybrid, bool transfer_tail_blocks, size_t tail_block_count) {
    ...
    const size_t start = use_hybrid ? 0 : reuse_block_size;
    for (size_t block_pos = start; block_pos < block_num; ++block_pos) {
        block_pos_list.push_back(block_pos);
    }
    return block_pos_list;
...
std::vector<CacheStoreBlockPair> buildStorePlanFromLayout(size_t total_logical_blocks,
                                                          size_t reuse_block_size,
                                                          bool   use_hybrid,
                                                          bool   block_round_robin,
                                                          bool   compact_last_rank_blocks,
    ...
    const bool compact_swa_by_cp = (cp_size > 1) && compact_last_rank_blocks;
    ...
    auto positions = cacheTransferPositions(total_logical_blocks,
                                            reuse_block_size,
                                            use_hybrid,
...
std::vector<CacheStoreBlockPair> CPSlotMapper::buildStorePlan(const CacheConfig& config,
    ...
    return buildStorePlanFromLayout(total_logical_blocks,
                                    reuse_block_size,
                                    use_hybrid,
                                    layout.mapping == CpBlockMappingMode::BLOCK_ROUND_ROBIN,
                                    layout.mapping == CpBlockMappingMode::COMPACT_LAST_RANK,
                                    layout.active_tail_blocks,
                                    cp_rank_,
                                    cp_size_);
}
