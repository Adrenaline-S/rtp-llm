...
public:
    ...
    // Single source of truth for KV cache key naming on both the prefill write path and the decode
    // read path. Must stay purely config-derived: the write side cannot observe per-request routing,
    // so anything request-scoped (page-level routing) belongs in the caller, not here.
    bool              requiresKVKeyPrefix() const;
