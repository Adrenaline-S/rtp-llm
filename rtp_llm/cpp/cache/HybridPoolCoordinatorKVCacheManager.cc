...
MallocResult HybridPoolCoordinatorKVCacheManager::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    for (const auto& [tag, unused_block_ids] : kv_resource->groupBlocks()) {
        ...
        if (!singleTypeManagerForTag(tag, "initMalloc")
                 ->malloc(block_ids_0, group_seq_len, malloc_info.reuse_cache, 0)) {
...
void HybridPoolCoordinatorKVCacheManager::rollbackBlockIdsToSize(std::string_view tag,
                                                                 BlockIds&        block_ids,
...
void HybridPoolCoordinatorKVCacheManager::rollbackIncrMalloc(
    BatchKVCacheResource&                                       kv_resource,
    ...
    for (int b = 0; b <= last_touched_batch; ++b) {
        for (const auto& [tag, unused_block_ids] : kv_resource.groupBlocks(b)) {
            auto&        block_ids    = kv_resource.mutableBlockIds(b, tag);
            const size_t original_num = original_sizes[static_cast<size_t>(b)].at(tag);
            rollbackBlockIdsToSize(tag, block_ids, original_num);
        }
    }
}
...
MemoryType HybridPoolCoordinatorKVCacheManager::memoryTypeForGroup(std::string_view tag) const {
    (void)tag;
    return allocation_type_ == AllocationType::DEVICE ? MemoryType::MEMORY_GPU : MemoryType::MEMORY_CPU;
}

void HybridPoolCoordinatorKVCacheManager::copyBlockMappingForGroup(
    std::string_view tag, const std::vector<BlockIdPair>& block_update_mapping) const {
    ...
    const auto   copy_type           = BatchCopyParams::get_copy_type(memory_type, memory_type);
    const size_t kv_block_size_bytes = config_.kvBlockStrideBytesForGroup(tag);
    const size_t scale_block_bytes   = config_.kvScaleStrideBytesForGroup(tag);
    ...
    for (const auto& [src_block_index, dest_block_index] : block_update_mapping) {
        for (int layer_id : config_.layerIdsForGroup(tag)) {
            auto src_addr_info = singleTypeManagerForTag(tag, "copyBlockMappingForGroup source")
                                     ->convertIndexToAddr(layer_id, src_block_index);
            auto dst_addr_info = singleTypeManagerForTag(tag, "copyBlockMappingForGroup destination")
                                     ->convertIndexToAddr(layer_id, dest_block_index);
...
size_t HybridPoolCoordinatorKVCacheManager::freeBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
size_t HybridPoolCoordinatorKVCacheManager::availableBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
size_t HybridPoolCoordinatorKVCacheManager::requestRefBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
size_t HybridPoolCoordinatorKVCacheManager::connectorRefBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
size_t HybridPoolCoordinatorKVCacheManager::blockCacheRefBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
size_t HybridPoolCoordinatorKVCacheManager::notInUseBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
size_t HybridPoolCoordinatorKVCacheManager::totalBlocksNum() const {
    size_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
void HybridPoolCoordinatorKVCacheManager::regUserMr(size_t model_id, std::shared_ptr<CacheStore> cache_store) {
    for (auto& [tag, pool] : group_block_pools_) {
        pool->regUserMr(model_id, cache_store);
    }
}

int64_t HybridPoolCoordinatorKVCacheManager::getMrCostTimeMs() const {
    int64_t total = 0;
    for (const auto& [tag, pool] : group_block_pools_) {
...
bool HybridPoolCoordinatorKVCacheManager::hasAvailableBlocksForReserve(const MallocInfo& malloc_info,
    ...
    for (const auto& [tag, group] : single_type_managers_) {
        ...
        const auto&  pool             = blockPoolForTag(tag, "reserveBlocksForPool");
