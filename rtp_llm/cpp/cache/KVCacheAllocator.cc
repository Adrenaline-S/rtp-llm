...
int KVCacheAllocator::reuseBlocks(BatchKVCacheResource& kv_resource, size_t max_reuse_blocks) {
    size_t                                            common_blocks = std::numeric_limits<size_t>::max();
    ...
    for (const auto& group : config_.topology().groups()) {
        ...
        if (group.policy.group_type == CacheGroupType::FULL) {
            ...
            matched_blocks = result.block_indices.size();
        ...
        common_blocks = std::min(common_blocks, matched_blocks);
    }
    if (!saw_reusable || common_blocks == 0) {
        return 0;
    }
    common_blocks = std::min(common_blocks, max_reuse_blocks);
    if (common_blocks == 0) {
        return 0;
    }
    ...
    for (const auto& group : config_.topology().groups()) {
        ...
        if (group.policy.group_type == CacheGroupType::FULL) {
            ...
            blocks.resize(std::min(blocks.size(), common_blocks));
        ...
        CacheKeysType match_keys    = cpBlockRoundRobinGroup(cp_mapper, config_, tag) ?
                                          cpCanonicalCacheKeys(cp_mapper, physical_keys) :
                                          physical_keys;
        RTP_LLM_CHECK(common_blocks <= match_keys.size());
        auto boundary = kv_cache_groups_.at(tag)->matchSingleKey(match_keys[common_blocks - 1]);
        RTP_LLM_CHECK_WITH_INFO(!boundary.block_indices.empty(),
                                "missing common-prefix boundary for group %s key %lu",
                                tag.c_str(),
                                match_keys[common_blocks - 1]);
        ids.assign(BlockIndicesType(common_blocks, NULL_BLOCK_IDX));
        ids.setAt(common_blocks - 1, boundary.block_indices.front());
    }
    return static_cast<int>(common_blocks);
}
...
MallocResult KVCacheAllocator::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    if (malloc_info.enable_device_cache) {
        ...
        const size_t max_reuse_blocks = seq_len > 0 ? static_cast<size_t>((seq_len - 1) / seqSizePerBlock()) : 0;
        reuse_blocks                  = reuseBlocks(*kv_resource, max_reuse_blocks);
...
size_t KVCacheAllocator::minTokenCapacity(bool use_available_blocks) const {
    ...
    for (const auto& [tag, group] : kv_cache_groups_) {
        if (config_.group(tag).policy.explicit_block_num > 0) {
            continue;
        }
        const auto& pool = group->blockPool();
        if (!pool) {
...
size_t KVCacheAllocator::reservableLogicalUnits() const {
    ...
    for (const auto& [tag, group] : kv_cache_groups_) {
        if (!group || !group->blockPool() || !group->isReservable() || config_.usesExplicitIndependentBlocks(tag)) {
            continue;
        }
        saw_reservable    = true;
...
size_t KVCacheAllocator::logicalCoverageUnitTokens() const {
    size_t unit = 1;
    for (const auto& [tag, group] : kv_cache_groups_) {
        if (!group || !group->isReservable() || config_.usesExplicitIndependentBlocks(tag)) {
            continue;
        }
...
size_t KVCacheAllocator::reserveBlocksForPool(std::string_view tag, size_t reserve_units) const {
    ...
    if (group == kv_cache_groups_.end() || !group->second || !group->second->blockPool()
        || !group->second->isReservable() || config_.usesExplicitIndependentBlocks(tag) || reserve_units == 0) {
        return 0;
    }
...
bool KVCacheAllocator::hasAvailableBlocksForReserve(const MallocInfo& malloc_info, size_t reserve_blocks) const {
    ...
    for (const auto& [tag, group] : kv_cache_groups_) {
        if (!group->isReservable() || config_.usesExplicitIndependentBlocks(tag)) {
            continue;
        }
        const auto cp_mapper              = cpSlotMapper(tag);
