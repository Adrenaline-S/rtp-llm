...
size_t KVCacheAllocator::freeBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->freeBlocksNum();
    }
    return blocks;
}
...
int64_t KVCacheAllocator::getMrCostTimeMs() const {
    int64_t cost_ms = 0;
    for (const auto& group : config_.topology().groups()) {
        cost_ms += blockPool(group.tag)->getMrCostTimeMs();
    }
    return cost_ms;
}
...
size_t KVCacheAllocator::availableBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->availableBlocksNum();
    }
    return blocks;
}
...
void KVCacheAllocator::blockCacheFree(const BatchKVCacheResourcePtr& batch_kv_cache_resource) {
    ...
    for (const auto& config_group : config_.topology().groups()) {
        BlockIndicesType                 blocks_to_free;
        std::unordered_set<BlockIdxType> seen_blocks;
        for (int batch_id = 0; batch_id < batch_kv_cache_resource->batchSize(); ++batch_id) {
            const auto& blocks = batch_kv_cache_resource->blocks(batch_id, config_group.tag);
        ...
        if (!blocks_to_free.empty()) {
            blockPool(config_group.tag)->blockCacheFree(blocks_to_free);
        }
    }
}
...
size_t KVCacheAllocator::requestRefBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->requestRefBlocksNum();
    }
    return blocks;
}
...
size_t KVCacheAllocator::connectorRefBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->connectorRefBlocksNum();
    }
    return blocks;
}
...
size_t KVCacheAllocator::blockCacheRefBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->blockCacheRefBlocksNum();
    }
    return blocks;
}
...
size_t KVCacheAllocator::notInUseBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->notInUseBlocksNum();
    }
    return blocks;
}
...
size_t KVCacheAllocator::totalBlocksNum() const {
    size_t blocks = 0;
    for (const auto& group : config_.topology().groups()) {
        blocks += blockPool(group.tag)->totalBlocksNum();
    }
    return blocks;
}
...
void KVCacheAllocator::regUserMr(size_t model_id, std::shared_ptr<CacheStore> cache_store) {
    for (const auto& group : config_.topology().groups()) {
        blockPool(group.tag)->regUserMr(model_id, cache_store);
    }
}
