...
// An explicitly-sized pool holds a fixed block count provisioned for its own group instead of
// sharing the dynamic block budget, so it never defines the shared bottleneck.
inline bool countsTowardDynamicBottleneck(const CacheConfig& config, std::string_view tag) {
    return !config.usesExplicitIndependentBlocks(tag);
}

}  // namespace

// KVCacheAllocator common implementation.
...
void KVCacheAllocator::blockBatchCopy(const std::vector<TaggedBlockIdPair>& copy_mapping) {
    ...
    // Group pools can live in different memory spaces, so the copy type is resolved per group.
    const auto fallback_type = allocation_type_ == AllocationType::DEVICE ? rtp_llm::MEMORY_GPU : rtp_llm::MEMORY_CPU;
    const auto copy_type_for = [this, fallback_type](const std::string& tag) {
        const auto pool        = getBlockPool(tag);
        const auto memory_type = pool ? pool->where() : fallback_type;
        return BatchCopyParams::get_copy_type(memory_type, memory_type);
    };

    size_t copy_counts[BatchCopyParams::TYPE_SIZE] = {};
    for (const auto& mapping : copy_mapping) {
        const auto& group = config_.topology().group(mapping.tag);
        copy_counts[copy_type_for(mapping.tag)] += group.layer_ids.size() * (group.kv_scale_stride_bytes > 0 ? 2 : 1);
    }

    BatchCopyParams copy_params;
    for (size_t copy_type = 0; copy_type < BatchCopyParams::TYPE_SIZE; ++copy_type) {
        copy_params.reserve(static_cast<BatchCopyParams::CopyType>(copy_type), copy_counts[copy_type]);
    }
    for (const auto& mapping : copy_mapping) {
        const auto& group     = config_.topology().group(mapping.tag);
        const auto  copy_type = copy_type_for(mapping.tag);
...
BatchKVCacheResourcePtr KVCacheAllocator::popBlocksFromCache(size_t min_blocks_to_free) {
    ...
    // Evicted keys already come from the GPU cache's actual key namespace. Under CP that batch can
    // mix canonical paged keys with logical state/SWA keys, so the coordinator must not remap it.
...
size_t KVCacheAllocator::minGroupTokens(CapacityScope scope, bool use_available_blocks) const {
    const auto in_scope = [this, scope](const GroupBase& group) {
        switch (scope) {
            case CapacityScope::FULL_GROUPS:
                return group.policy.group_type == CacheGroupType::FULL;
            case CapacityScope::DYNAMIC_POOLS:
                return countsTowardDynamicBottleneck(config_, group.tag);
            case CapacityScope::ALL_POOLS:
                break;
        }
        return true;
    };
    ...
    for (const auto& group : config_.topology().groups()) {
        ...
        const auto pool = getBlockPool(group.tag);
        if (!pool) {
            continue;
        }
        const size_t seq_size = logicalSeqSizePerBlockForCapacity(group.tag);
        RTP_LLM_CHECK_WITH_INFO(seq_size > 0 && config_.seq_size_per_block > 0
                                    && seq_size % config_.seq_size_per_block == 0,
                                "cache group tag=%s logical seq_size_per_block=%zu is not a whole multiple of the "
                                "global block size=%zu, token capacity would truncate",
                                group.tag.c_str(),
                                seq_size,
    ...
    switch (scope) {
        case CapacityScope::FULL_GROUPS:
            return minGroupTokens(CapacityScope::DYNAMIC_POOLS, use_available_blocks);
        case CapacityScope::DYNAMIC_POOLS:
            return minGroupTokens(CapacityScope::ALL_POOLS, use_available_blocks);
        case CapacityScope::ALL_POOLS:
            break;
    }
    return 0;
...
size_t KVCacheAllocator::maxAvailableTokensNum() const {
    return minGroupTokens(CapacityScope::FULL_GROUPS, /*use_available_blocks=*/false);
}

size_t KVCacheAllocator::availableTokensNum() const {
    return minGroupTokens(CapacityScope::DYNAMIC_POOLS, /*use_available_blocks=*/true);
}
...
size_t KVCacheAllocator::totalTokensNum() const {
    return minGroupTokens(CapacityScope::DYNAMIC_POOLS, /*use_available_blocks=*/false);
}
...
const KVCacheGroupPtr& HybridPoolKVCacheAllocator::cacheGroupForTag(std::string_view tag, const char* context) const {
    const auto value = std::string(tag);
    const auto it    = kv_cache_groups_.find(value);
    RTP_LLM_CHECK_WITH_INFO(
        it != kv_cache_groups_.end(), "%s: missing single-type KV cache manager for tag=%s", context, value.c_str());
    RTP_LLM_CHECK_WITH_INFO(it->second != nullptr, "%s: null KV cache group for tag=%s", context, value.c_str());
    return it->second;
}

const BlockPoolPtr& HybridPoolKVCacheAllocator::blockPoolForTag(std::string_view tag, const char* context) const {
    const auto value = std::string(tag);
...
int HybridPoolKVCacheAllocator::reuseCache(const CacheKeysType&                 cache_keys,
    ...
    for (; pos >= 0 && has_tail_groups; --pos) {
        ...
        for (const auto& tag : linear_group_tags_) {
            auto result =
                cacheGroupForTag(tag, "reuseCache linear match")->matchSingleKey(cache_keys[static_cast<size_t>(pos)]);
        ...
        for (const auto& tag : swa_group_tags_) {
            ...
            auto result =
                cacheGroupForTag(tag, "reuseCache SWA match")->matchSingleKey(cache_keys[static_cast<size_t>(pos)]);
            if (result.block_indices.empty()) {
                all_tail_groups_matched = false;
                break;
            }
...
int HybridPoolKVCacheAllocator::singleBatchNeedBlocks(const BatchKVCacheResourcePtr& batch_kv_cache_resource,
    ...
    for (const auto& [tag, unused_block_ids] : batch_kv_cache_resource->groupBlocks()) {
        ...
        need_blocks +=
            cacheGroupForTag(tag, "singleBatchNeedBlocks")->needBlocksNum(effective_seq_len, cur_blocks, reserve_step);
    }
    return need_blocks;
}
