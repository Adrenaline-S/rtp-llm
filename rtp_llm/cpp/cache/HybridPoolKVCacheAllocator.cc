...
size_t KVCacheAllocator::maxSequenceLength() const {
...
const KVCacheGroupPtr& HybridPoolKVCacheAllocator::cacheGroupForTag(std::string_view tag, const char* context) const {
    ...
    RTP_LLM_CHECK_WITH_INFO(
        it != kv_cache_groups_.end(), "%s: missing KV cache group for tag=%s", context, value.c_str());
...
MallocResult HybridPoolKVCacheAllocator::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    for (const auto& [tag, unused_block_ids] : kv_resource->groupBlocks()) {
        ...
        if (!cacheGroupForTag(tag, "initMalloc")->malloc(block_ids_0, group_seq_len, malloc_info.reuse_cache, 0)) {
...
void HybridPoolKVCacheAllocator::copyBlockMappingForGroup(std::string_view                tag,
    ...
    for (const auto& [src_block_index, dest_block_index] : block_update_mapping) {
        for (int layer_id : config_.layerIdsForGroup(tag)) {
            auto src_addr_info =
                cacheGroupForTag(tag, "copyBlockMappingForGroup source")->convertIndexToAddr(layer_id, src_block_index);
            auto dst_addr_info = cacheGroupForTag(tag, "copyBlockMappingForGroup destination")
...
void HybridPoolKVCacheAllocator::blockBatchCopy(const std::vector<GroupBlockIdPair>& copy_mapping) {
    ...
    for (const auto& mapping : copy_mapping) {
        ...
        for (int layer_id : config_.layerIdsForGroup(mapping.tag)) {
            auto src_addr_info =
                cacheGroupForTag(mapping.tag, "blockBatchCopy source")->convertIndexToAddr(layer_id, mapping.src);
            auto dst_addr_info =
                cacheGroupForTag(mapping.tag, "blockBatchCopy destination")->convertIndexToAddr(layer_id, mapping.dst);
