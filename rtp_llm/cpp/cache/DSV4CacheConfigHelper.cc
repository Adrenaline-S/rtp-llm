...
std::pair<DSV4LayerSets, DSV4SpecMap> parseDSV4Specs(const ModelConfig& model_config) {
    ...
    for (const auto& spec : model_config.kv_cache_specs) {
        ...
        RTP_LLM_CHECK_WITH_INFO(specs.emplace(decl.tag, spec).second,
                                "duplicate DSV4 kv_cache spec tag=%s",
                                decl.tag.c_str());
        ...
        std::set<int> unique_layers;
        for (int layer_id : decl.layers) {
            RTP_LLM_CHECK_WITH_INFO(layer_id >= 0 && layer_id < model_config.num_layers,
                                    "DSV4 kv_cache spec tag=%s has invalid layer id %d for num_layers=%ld",
                                    decl.tag.c_str(),
                                    layer_id,
                                    model_config.num_layers);
            RTP_LLM_CHECK_WITH_INFO(unique_layers.insert(layer_id).second,
                                    "DSV4 kv_cache spec tag=%s has duplicate layer id %d",
                                    decl.tag.c_str(),
                                    layer_id);
        }
    }
    for (const auto& expected : kExpectedDsv4Specs) {
        RTP_LLM_CHECK_WITH_INFO(specs.count(expected.tag) == 1, "missing DSV4 kv_cache spec tag=%s", expected.tag);
    }

    DSV4LayerSets sets;
    sets.csa_layers      = specForTag(specs, "csa_kv").layers;
    sets.hca_layers      = specForTag(specs, "hca_kv").layers;
    sets.all_layers      = specForTag(specs, "swa_kv").layers;
    const auto& csa      = sets.csa_layers;
    const auto& hca      = sets.hca_layers;
    const auto& all      = sets.all_layers;

    RTP_LLM_CHECK_WITH_INFO(sameLayers(specForTag(specs, "indexer_kv").layers, csa),
                            "DSV4 indexer_kv layers must match csa_kv layers");
    RTP_LLM_CHECK_WITH_INFO(sameLayers(specForTag(specs, "indexer_state").layers, csa),
                            "DSV4 indexer_state layers must match csa_kv layers");
    RTP_LLM_CHECK_WITH_INFO(sameLayers(specForTag(specs, "csa_state").layers, csa),
                            "DSV4 csa_state layers must match csa_kv layers");
    RTP_LLM_CHECK_WITH_INFO(sameLayers(specForTag(specs, "hca_state").layers, hca),
