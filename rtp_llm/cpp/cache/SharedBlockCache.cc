...
bool SharedBlockCache::isLogicallyCompleteLocked(const UnifiedCacheItem& item) const {
    ...
    for (const int group_id : required_group_ids_) {
        if (group_id < 0 || static_cast<size_t>(group_id) >= group_tags_in_order_.size()) {
            return false;
        }
        const auto& tag      = group_tags_in_order_[static_cast<size_t>(group_id)];
        const auto  block_it = item.group_block_ids.find(tag);
        if (block_it == item.group_block_ids.end() || isNullBlockIdx(block_it->second) || !groupMatchable(item, tag)) {
            return false;
        }
    }
    return true;
