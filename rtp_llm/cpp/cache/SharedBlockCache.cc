...
std::vector<SharedBlockCache::UnifiedCacheItem::TaggedBlock>
copyTaggedBlocks(const SharedBlockCache::UnifiedCacheItem& item) {
    std::vector<SharedBlockCache::UnifiedCacheItem::TaggedBlock> blocks;
    blocks.reserve(item.blocks_by_tag.size());
    for (const auto& [tag, block] : item.blocks_by_tag) {
        (void)tag;
...
void SharedBlockCache::init(const std::vector<TaggedPool>& pools) {
    std::lock_guard<std::mutex> lock(mu_);
    pools_by_tag_.clear();
    pools_by_tag_.reserve(pools.size());
    for (const auto& entry : pools) {
        RTP_LLM_CHECK_WITH_INFO(!entry.tag.empty(), "SharedBlockCache pool has empty tag");
        RTP_LLM_CHECK_WITH_INFO(entry.pool != nullptr, "SharedBlockCache tag=%s has null pool", entry.tag.c_str());
        RTP_LLM_CHECK_WITH_INFO(pools_by_tag_.emplace(entry.tag, entry.pool).second,
...
void SharedBlockCache::put(CacheKeyType                                      cache_key,
                           const std::vector<UnifiedCacheItem::TaggedBlock>& blocks,
                           bool                                              is_resident) {
    BlockDependency dependency;
    put(cache_key, blocks, is_resident, kDefaultNamespace, dependency);
}
...
void SharedBlockCache::put(CacheKeyType                                      cache_key,
        ...
        if (success) {
            ...
            for (const auto& block : blocks) {
                ...
                RTP_LLM_CHECK_WITH_INFO(
                    pools_by_tag_.count(block.tag) != 0, "SharedBlockCache missing pool tag=%s", block.tag.c_str());
                auto [it, inserted] = existing_item.blocks_by_tag.emplace(block.tag, block);
                if (inserted || isNullBlockIdx(it->second.block_id)) {
                    it->second                 = block;
                    it->second.created_time_us = now_us;
                    updated                    = true;
                    pools_by_tag_.at(block.tag)->blockCacheReference(block.block_id);
                } else if (block.matchable && !it->second.matchable) {
                    it->second.matchable = true;
                    updated              = true;
                }
            }
    ...
    for (auto block : blocks) {
        RTP_LLM_CHECK_WITH_INFO(!block.tag.empty(), "SharedBlockCache block has empty tag");
        RTP_LLM_CHECK_WITH_INFO(
            pools_by_tag_.count(block.tag) != 0, "SharedBlockCache missing pool tag=%s", block.tag.c_str());
        block.created_time_us = isNullBlockIdx(block.block_id) ? 0 : now_us;
        RTP_LLM_CHECK_WITH_INFO(item.blocks_by_tag.emplace(block.tag, std::move(block)).second,
                                "SharedBlockCache duplicate block tag");
    }
    ...
    for (const auto& [tag, block] : item.blocks_by_tag) {
        if (!isNullBlockIdx(block.block_id)) {
            pools_by_tag_.at(tag)->blockCacheReference(block.block_id);
        }
    }
}
...
SharedBlockCache::MatchResult SharedBlockCache::match(CacheKeyType cache_key) {
    ...
    return {true, copyTaggedBlocks(item)};
}
...
BlockIdxType SharedBlockCache::matchGroup(CacheKeyType cache_key, std::string_view tag) {
    ...
    touchTreeAliasesLocked(cache_key);
    const auto it = item.blocks_by_tag.find(std::string(tag));
    if (it == item.blocks_by_tag.end() || !it->second.matchable) {
        return NULL_BLOCK_IDX;
    }
...
SharedBlockCache::EvictResult SharedBlockCache::selectAndEvict(size_t min_blocks) {
    ...
    if (prefix_tree_enabled_ && !leaf_lru_.empty()) {
        ...
        while (selected_blocks < min_blocks && !leaf_lru_.empty()) {
            ...
            for (const auto& tree_key : ordered_chain) {
                ...
                if (result.evicted_blocks_by_tag.find(tree_key.cache_key) == result.evicted_blocks_by_tag.end()) {
                    result.evicted_keys.push_back(tree_key.cache_key);
                    result.evicted_blocks_by_tag[tree_key.cache_key] = copyTaggedBlocks(removed_item);
                    ...
                    for (const auto& [tag, block] : removed_item.blocks_by_tag) {
                        (void)tag;
                        if (!isNullBlockIdx(block.block_id)) {
                            selected_blocks++;
                        }
    ...
    for (const auto cache_key : lru_keys) {
        ...
        result.evicted_blocks_by_tag[cache_key] = copyTaggedBlocks(removed_item);
...
size_t SharedBlockCache::evictAndFree(size_t min_blocks) {
    ...
    for (size_t i = 0; i < evict_result.evicted_keys.size(); ++i) {
        ...
        const auto& blocks    = evict_result.evicted_blocks_by_tag.at(cache_key);

        for (const auto& block : blocks) {
            if (!isNullBlockIdx(block.block_id)) {
                pools_by_tag_.at(block.tag)->blockCacheFree(block.block_id);
                freed++;
            }
        }
    }
...
void SharedBlockCache::setIndependentGroupEviction(bool enabled, const std::vector<std::string>& tags) {
    ...
    independent_eviction_tags_.clear();
    for (const auto& tag : tags) {
        RTP_LLM_CHECK_WITH_INFO(
            pools_by_tag_.count(tag) != 0, "SharedBlockCache unknown independent tag=%s", tag.c_str());
        independent_eviction_tags_.insert(tag);
    }
}
...
bool SharedBlockCache::hasUsableGroup(const UnifiedCacheItem& item, std::string_view tag) {
    const auto it = item.blocks_by_tag.find(std::string(tag));
    return it != item.blocks_by_tag.end() && !isNullBlockIdx(it->second.block_id);
}
...
bool SharedBlockCache::selectIndependentGroupEvictionsLocked(std::string_view tag,
                                                             size_t           min_blocks,
                                                             EvictResult&     result) {
    if (pools_by_tag_.count(std::string(tag)) == 0 || min_blocks == 0) {
        return false;
    }
    size_t               selected_blocks = 0;
    std::vector<LeafKey> leaves(leaf_lru_.begin(), leaf_lru_.end());
...
void SharedBlockCache::removeGroupFromItemLocked(CacheKeyType cache_key, std::string_view tag, EvictResult& result) {
    ...
    auto block_it = item.blocks_by_tag.find(std::string(tag));
    if (block_it == item.blocks_by_tag.end() || isNullBlockIdx(block_it->second.block_id)) {
        lru_cache_.put(cache_key, item);
        return;
    }

    result.evicted_keys.push_back(cache_key);
    result.evicted_blocks_by_tag[cache_key] = {block_it->second};
    result.evicted_namespaces[cache_key] =
    ...
    const int64_t created_time_us =
        block_it->second.created_time_us > 0 ? block_it->second.created_time_us : item.created_time_us;
    ...
    item.blocks_by_tag.erase(block_it);

    const bool has_any_group = std::any_of(item.blocks_by_tag.begin(), item.blocks_by_tag.end(), [](const auto& entry) {
        return !isNullBlockIdx(entry.second.block_id);
    });
    if (has_any_group) {
        lru_cache_.put(cache_key, item);
        refreshAllTreeAliasesLocked(cache_key);
