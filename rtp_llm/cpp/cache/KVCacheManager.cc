...
GroupedCacheLayerLayout projectLayout(const GroupedCacheLayerLayout& source,
                                      const CacheConfig&             target_topology,
    ...
    for (const auto& target_group : target_topology.groups()) {
        ...
        for (size_t local = 0; local < global_layer_ids.size(); ++local) {
            const auto& local_tags = target_topology.groupsForLayer(static_cast<int>(local));
            if (std::find(local_tags.begin(), local_tags.end(), target_group.tag) == local_tags.end()) {
                continue;
            }
...
BlockAddrInfo KVCacheManager::convertIndexToAddr(int block_index, int layer_id, const std::string& tag) const {
    return coordinator_cache_manager_->convertIndexToAddr(layer_id, tag, block_index);
}

std::vector<BlockInfo>
KVCacheManager::convertIndexToBuffer(int block_index, int layer_id, const std::string& tag) const {
    return coordinator_cache_manager_->convertIndexToBuffer(layer_id, tag, block_index);
}
...
std::vector<BlockInfo> KVCacheManager::convertIndexToBuffer(
    ...
    return coordinator_cache_manager_->convertIndexToBuffer(layer_id, tag, block_index, partition_count, partition_id);
}
...
GroupedCacheLayerLayout KVCacheManager::getMainModelGroupedCacheLayerLayout() const {
    ...
    return projectLayout(all_layout, config_, global_layer_ids);
}
...
GroupedCacheLayerLayout KVCacheManager::getMTPModuleGroupedCacheLayerLayout(int mtp_module_id) const {
    ...
    return projectLayout(coordinator_cache_manager_->allLayerCacheBase(), *mtp_sub_config, global_layer_ids);
}
...
void KVCacheManager::initCacheEventPublisher() {
    try {
        ...
        std::vector<int> reuse_group_ids;
        for (size_t group_id = 0; group_id < config_.groups().size(); ++group_id) {
            if (cacheGroupPublishesPrefixChain(config_.groups()[group_id].policy)) {
                reuse_group_ids.push_back(static_cast<int>(group_id));
            }
        }
        ...
        for (const auto group_id : reuse_group_ids) {
            if (config_.groups().at(static_cast<size_t>(group_id)).policy.memory_placement
        ...
        publisher_shared_cache_ = coordinator_cache_manager_->sharedBlockCache();
        ...
        for (const auto group_id : reuse_group_ids) {
            ...
            group_block_size_bytes.push_back(static_cast<int64_t>(config_.blockSizeBytes(group.tag)));
        }
