...
public:
    ...
    virtual BlockAddrInfo          convertIndexToAddr(int layer_id, const std::string& tag, int block_id) const   = 0;
    ...
    // Aggregate capacity in logical (cp-aware) tokens: the minimum over the pools that share the
    // dynamic block budget. Explicitly-sized pools are reported per pool instead.
...
protected:
    ...
    // Which pools an aggregate capacity figure is measured over. Each scope falls back to the next
    // one when the topology selects no pool.
    enum class CapacityScope : uint8_t {
        FULL_GROUPS,    // request-length limit: FULL attention groups
        DYNAMIC_POOLS,  // live headroom: pools that share the dynamic block budget
        ALL_POOLS,
    };
    size_t minGroupTokens(CapacityScope scope, bool use_available_blocks) const;
