...
public:
    ...
    incrKVCacheRef(const KVCacheResource& resource, const CacheKeysType& cache_keys, bool is_connector = false) {
    ...
    virtual BlockPoolPtr blockPool(std::string_view tag) const {
...
protected:
    ...
    virtual int  estimateInitialBatchPeakNeedBlocks(int  seq_len,
                                                    ...
                                                    bool enable_reuse_cache,
                                                    int  target_batch_size) const;
    virtual void checkCPShardedMallocResult(const MallocInfo& malloc_info) const;
    ...
    int  reuseBlocks(BatchKVCacheResource& kv_resource, size_t max_reuse_blocks);
    ...
    void         initializeReuseSpan();
    std::pair<size_t, size_t> layerCopyStrides(std::string_view tag, int layer_id) const;
    ...
    size_t reservableLogicalUnits() const;
    size_t reserveBlocksForPool(std::string_view tag, size_t reserve_units) const;
