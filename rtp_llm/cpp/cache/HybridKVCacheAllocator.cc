    ...
    for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
        ...
        if (!cp_active) {
            ...
            for (size_t pos = max_keys; pos > 0; --pos) {
                ...
                for (const auto& entry : kv_cache_resource->groupBlocks(batch_id)) {
                    ...
                    if (!isNullBlockIdx(blocks[i])) {
                        tagged_blocks.push_back({entry.tag, blocks[i], true, 0});
                    }
                }
        ...
        for (const auto& entry : kv_cache_resource->groupBlocks(batch_id)) {
            ...
            for (size_t pos = loop_end; pos > 0; --pos) {
                ...
                std::vector<SharedBlockCache::UnifiedCacheItem::TaggedBlock> tagged_blocks{{tag, blocks[i], true, 0}};
                const auto                                                   dependency =
                    i < dependencies.size() ? dependencies[i] : BlockDependency{false, 0, static_cast<uint32_t>(i)};
                shared_block_cache_->put(src_keys[i], tagged_blocks, insert_info.is_resident, namespace_id, dependency);
            }
        }
    }
}
