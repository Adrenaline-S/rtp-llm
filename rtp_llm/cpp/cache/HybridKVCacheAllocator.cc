...
int HybridKVCacheAllocator::reuseCache(BatchKVCacheResource& kv_resource, size_t max_reuse_tokens) {
    ...
    for (const auto& group : config_.topology().groups()) {
        ...
        if (group.policy.group_type == CacheGroupType::FULL) {
            ...
            matched_tokens = result.reuse_blocks
...
MallocResult HybridKVCacheAllocator::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    if (malloc_info.enable_device_cache) {
        ...
        reuse_blocks                  = reuseCache(*kv_resource, max_reuse_tokens);
    ...
    return {true, reuse_blocks, match_cost_time_us};
}
    ...
    for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
        ...
        for (const auto& entry : kv_cache_resource->groupResources(batch_id)) {
            ...
            for (size_t pos = loop_end; pos > 0; --pos) {
                ...
                std::vector<SharedBlockCache::UnifiedCacheItem::TaggedBlock> tagged_blocks{
                    {tag, blocks[i], true, 0, dependencies[i].ordinal}};
                shared_block_cache_->put(
                    src_keys[i], tagged_blocks, insert_info.is_resident, namespace_id, dependencies[i]);
            }
        }
    }
}
...
std::shared_ptr<KVCacheResource> HybridKVCacheAllocator::incrKVCacheRef(const KVCacheResource& kvcache_resource,
                                                                        const CacheKeysByTag&  cache_keys_by_tag,
                                                                        bool                   is_connector) {
    if (cache_keys_by_tag.empty() || kvcache_resource.groupNums() <= 0) {
        return nullptr;
    }
    ...
    for (const auto& entry : kvcache_resource.groupResources()) {
        const auto& tag          = entry.tag;
        const auto  requested_it = cache_keys_by_tag.find(tag);
        if (requested_it == cache_keys_by_tag.end()) {
            continue;
        }
