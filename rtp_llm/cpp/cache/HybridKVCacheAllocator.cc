...
MallocResult HybridKVCacheAllocator::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    for (int b = 1; b < batch_size; ++b) {
        for (const auto& [tag, unused_block_ids] : kv_resource->groupBlocks()) {
            kv_cache_groups_.at(tag)->reference(kv_resource->mutableBlockIds(b, tag), kv_resource->blocks(0, tag));
        }
    }
...
MallocResult HybridKVCacheAllocator::incrMalloc(const MallocInfo& malloc_info) {
    ...
    if (all_success) {
        ...
        for (int b = 0; b < batch_size; ++b) {
            for (const auto& [tag, unused_block_ids] : kv_resource->groupBlocks(b)) {
                kv_cache_groups_.at(tag)->removeSkippedBlocks(
...
void HybridKVCacheAllocator::free(const FreeInfo& free_info) {
    ...
    for (int batch_id = 0; batch_id < kv_cache_resource->batchSize(); ++batch_id) {
        for (const auto& [tag, block_ids] : kv_cache_resource->groupBlocks(batch_id)) {
            kv_cache_groups_.at(tag)->free(block_ids->blocks());
        }
    }
...
bool HybridKVCacheAllocator::updateKVBlock(const BatchKVCacheResourcePtr&  batch_kv_cache_resource,
    ...
    for (int old_batch_idx = 0; old_batch_idx < old_batch_size; ++old_batch_idx) {
        ...
        for (const auto& [tag, block_ids] : batch_kv_cache_resource->groupBlocks(old_batch_idx)) {
            ...
            if (!to_free.empty()) {
                kv_cache_groups_.at(tag)->free(to_free);
            }
        }
    }
    ...
    for (int new_batch_idx = 0; new_batch_idx < new_batch_size; ++new_batch_idx) {
        ...
        if (fork_count == 1) {
            ...
            for (const auto& [tag, source_block_ids] : old_resources[old_batch_idx].groupBlocks()) {
                ...
                kv_cache_groups_.at(tag)->reference(block_ids, source_block_ids->blocks());
...
int HybridKVCacheAllocator::estimatePeakNeedBlocks(const KVCacheResource& kv_cache_resource,
    ...
    for (const auto& [tag, block_ids] : kv_cache_resource.groupBlocks()) {
        need_blocks += kv_cache_groups_.at(tag)->estimatePeakNeedBlocks(
...
void HybridKVCacheAllocator::checkCPShardedMallocResult(const MallocInfo& malloc_info) const {
    ...
    for (int batch_id = 0; batch_id < kv_resource->batchSize(); ++batch_id) {
        for (const auto& [tag, unused_block_ids] : kv_resource->groupBlocks(batch_id)) {
...
int HybridKVCacheAllocator::singleBatchNeedBlocks(const BatchKVCacheResourcePtr& batch_kv_cache_resource,
    ...
    for (const auto& [tag, unused_block_ids] : batch_kv_cache_resource->groupBlocks()) {
        ...
        need_blocks += kv_cache_groups_.at(tag)->needBlocksNum(effective_seq_len, cur_blocks, reserve_step);
    }
    return need_blocks;
}
