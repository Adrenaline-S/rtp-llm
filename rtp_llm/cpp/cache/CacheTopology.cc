...
std::shared_ptr<const CacheTopology> CacheTopology::create(std::vector<CacheGroup> groups,
                                                           std::vector<CacheLayer> layers) {
    return std::shared_ptr<const CacheTopology>(new CacheTopology(std::move(groups), std::move(layers)));
}

CacheTopology::CacheTopology(std::vector<CacheGroup> groups, std::vector<CacheLayer> layers):
...
const CacheGroup& CacheTopology::group(std::string_view tag) const {
    return groupBySlot(slotForTag(tag));
}

const CacheGroup& CacheTopology::groupBySlot(size_t slot) const {
    RTP_LLM_CHECK_WITH_INFO(
        slot < groups_.size(), "CacheTopology invalid group slot=%zu size=%zu", slot, groups_.size());
    return groups_[slot];
}

const CacheLayer& CacheTopology::layer(int layer_id) const {
    RTP_LLM_CHECK_WITH_INFO(layer_id >= 0 && static_cast<size_t>(layer_id) < layers_.size(),
...
const CacheGroup& CacheTopology::groupForLayer(int layer_id, std::string_view tag) const {
    const auto&       layer_config = layer(layer_id);
    const std::string value(tag);
...
const CacheGroup& CacheTopology::soleGroupForLayer(int layer_id) const {
    const auto& layer_config = layer(layer_id);
...
bool CacheTopology::hasOneGroupPerLayer() const {
    return std::all_of(
        layers_.begin(), layers_.end(), [](const CacheLayer& layer) { return layer.group_tags.size() == 1; });
}
