...
size_t CacheConfig::explicitlySizedPoolReserveBytes() const {
    ...
    for (const auto& group : topology().groups()) {
        if (group.policy.explicit_block_num > 0) {
            result += static_cast<size_t>(group.policy.explicit_block_num) * blockSizeBytesForGroup(group.tag);
        }
    }
    return result;
}
...
void CacheConfig::publishSentinelOnlyBlockNum() {
    RTP_LLM_CHECK_WITH_INFO(linear_step == 1, "sentinel-only cache config requires linear_step=1, got %d", linear_step);
...
void CacheConfig::finalizeBlockNums(uint32_t global_block_num, const RuntimeConfig& runtime_config) {
    ...
    (void)runtime_config;
    RTP_LLM_CHECK_WITH_INFO(
        linear_step == 1, "CacheConfig::finalizeBlockNums only supports linear_step=1, got %d", linear_step);
...
#define OUTPUT_FIELD_EXPR(name, expr) os << indent1 << name << "=" << expr << "\n"
    ...
    for (const auto& group : topology_groups) {
        ...
        first_group = false;
        os << "{" << group.tag << ":" << blockNumForGroup(group.tag) << "}";
    }
    os << "]\n";
    os << "\n";
