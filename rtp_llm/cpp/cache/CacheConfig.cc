...
std::optional<std::string> CacheConfig::kernelAddressedFullGroupTag() const {
    ...
    for (const auto& group : topology().groups()) {
        ...
        RTP_LLM_CHECK_WITH_INFO(kernelSeqSizePerBlockForGroup(group.tag) == kernelSeqSizePerBlockForGroup(*selected),
...
uint32_t CacheConfig::blockNum() const {
    ...
    uint32_t fallback = 0;
    for (const auto& group : topology().groups()) {
        fallback = std::max(fallback, group.block_num);
    }
    return fallback;
}
...
size_t CacheConfig::kvBlockStrideBytes() const {
    size_t result = 0;
    for (const auto& group : topology().groups()) {
        result = std::max(result, group.kv_block_stride_bytes);
    }
    return result;
}

size_t CacheConfig::kvScaleStrideBytes() const {
    size_t result = 0;
    for (const auto& group : topology().groups()) {
        result = std::max(result, group.kv_scale_stride_bytes);
    }
    return result;
}

size_t CacheConfig::kvBlockSizeBytes() const {
    if (!use_independent_block_pools) {
        return groupLayerNum() * kvBlockStrideBytes();
    }
    size_t result = 0;
    for (const auto& group : topology().groups()) {
        const bool paged =
            group.policy.group_type == CacheGroupType::FULL || group.policy.group_type == CacheGroupType::LINEAR;
        if (paged && group.policy.explicit_block_num == 0) {
            result += group.layer_ids.size() * group.kv_block_stride_bytes;
        }
    }
    return result;
}

size_t CacheConfig::kvScaleSizeBytes() const {
    if (!use_independent_block_pools) {
        return groupLayerNum() * kvScaleStrideBytes();
    }
    size_t result = 0;
    for (const auto& group : topology().groups()) {
        const bool paged =
            group.policy.group_type == CacheGroupType::FULL || group.policy.group_type == CacheGroupType::LINEAR;
        if (paged && group.policy.explicit_block_num == 0) {
            result += group.layer_ids.size() * group.kv_scale_stride_bytes;
        }
    }
    return result;
}

size_t CacheConfig::blockSizeBytes() const {
    const size_t result = kvBlockSizeBytes() + kvScaleSizeBytes();
    return result == 0 && use_independent_block_pools && usesTypedCacheRegions() ? 1 : result;
}
...
size_t CacheConfig::explicitlySizedPoolReserveBytes() const {
    size_t result = 0;
    for (const auto& group : topology().groups()) {
        if (group.policy.explicit_block_num > 0 && group.policy.charge_to_paged_budget) {
            result += static_cast<size_t>(group.block_num) * blockSizeBytesForGroup(group.tag);
        }
    }
    return result;
}
...
bool CacheConfig::usesMla() const {
    return std::any_of(topology().groups().begin(), topology().groups().end(), [](const GroupBase& group) {
        return group.spec->type == KVCacheSpecType::MultiHeadLatentAttention;
    });
}

bool CacheConfig::isSparse() const {
    return std::any_of(topology().groups().begin(), topology().groups().end(), [](const GroupBase& group) {
        return group.uses_sparse_indexer_scale_layout || group.spec->type == KVCacheSpecType::OpaqueKV;
    });
}
...
#define OUTPUT_FIELD_EXPR(name, expr) os << indent1 << name << "=" << expr << "\n"
    ...
    OUTPUT_FIELD_EXPR("layer_all_num", totalLayerNum());
    OUTPUT_FIELD_EXPR("use_mla", (usesMla() ? "true" : "false"));
    os << "\n";

    os << indent1 << "# Block Configuration:\n";
    OUTPUT_FIELD_EXPR("block_num", blockNum());
    OUTPUT_FIELD(seq_size_per_block);
    os << "\n";

    os << indent1 << "# Block Sizing Information:\n";
    OUTPUT_FIELD_EXPR("kv_block_size_bytes", kvBlockSizeBytes());
    OUTPUT_FIELD_EXPR("kv_scale_size_bytes", kvScaleSizeBytes());
    OUTPUT_FIELD_EXPR("block_size_bytes", blockSizeBytes());
    OUTPUT_FIELD_EXPR("kv_block_stride_bytes", kvBlockStrideBytes());
    OUTPUT_FIELD_EXPR("kv_scale_stride_bytes", kvScaleStrideBytes());
    os << "\n";
