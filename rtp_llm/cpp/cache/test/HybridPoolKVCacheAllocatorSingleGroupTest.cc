...
TEST_F(HybridPoolKVCacheAllocatorSingleGroupTest, GetNeedBlocksComputesCommonAndExtra) {
...
TEST_F(HybridPoolKVCacheAllocatorSingleGroupTest, CapacityAndNeedBlocksUseCPVirtualBlockSize) {
    auto config = createSingleTypeTestConfig(/*layer_num=*/4, /*block_num=*/10, /*seq_size_per_block=*/8);
    allocator_  = std::make_shared<HybridPoolKVCacheAllocator>(config);
    ASSERT_TRUE(allocator_->init());
