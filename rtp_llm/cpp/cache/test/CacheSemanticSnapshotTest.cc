...
CacheConfig makeKimiHybridConfig() {
    ...
    return CacheConfigCreator::createDecodeWarmupConfig(config, parallelism, KVCacheConfig{}, 0);
}
...
TEST(CacheSemanticSnapshotTest, MlaFp8PreservesPhysicalStrideAcrossKernelSubdivision) {
    auto base = TestCacheConfigBuilder::makeBase(1, 5, 8, 4, DataType::TYPE_FP8_E4M3);
    auto spec = makeResolvedMlaSpec(DataType::TYPE_FP8_E4M3, /*kv_lora_rank=*/128, /*rope_head_dim=*/64, 8, "mla");
    const auto config =
        buildTestCacheConfigFromGroupedSpecs(std::move(base), {spec}, {{0}}, {CacheGroupType::FULL}, {"mla"});
    ...
    const CacheSemanticSnapshot expected = {{"mla",
                                             ...
                                             makeExpectedSpecSemanticSnapshot(KVCacheSpecType::MultiHeadLatentAttention,
                                                                              ...
                                                                              0)}};
    EXPECT_EQ(snapshotCacheConfig(config), expected);
    EXPECT_EQ(config.group("mla").kernelBlocksPerPoolBlock(), 2u);
}
...
TEST(CacheSemanticSnapshotTest, MtpMergePreservesMainAndModuleRecords) {
    ...
    main = TestCacheConfigBuilder::rebuildForTest(std::move(main))
               .addComposedMTPModule(propose, /*module_index=*/0, /*main_layer_num=*/2)
               .build();
    const auto&                 module          = main.mtpModule(0);
    const CacheSemanticSnapshot expected_main   = {{"default",
                                                    ...
                                                    makeExpectedSpecSemanticSnapshot(KVCacheSpecType::MultiHeadAttention,
                                                                                   ...
                                                                                   0,
                                                                                   0)}};
    const CacheSemanticSnapshot expected_module = {
        {"default",
         KVCacheSpecType::MultiHeadAttention,
         CacheGroupType::FULL,
         true,
    ...
    EXPECT_EQ(main.group("default").kv_block_stride_bytes, module.group("default").kv_block_stride_bytes);
...
TEST(CacheSemanticSnapshotTest, TestConstructionProjectionDoesNotMutatePublishedMtpDescendants) {
    ...
    auto child = makeSimpleMhaCacheConfig(
        /*layer_num=*/1, /*block_num=*/5, /*tokens_per_block=*/8, DataType::TYPE_FP16, 1, 2);
    child = TestCacheConfigBuilder::rebuildForTest(std::move(child)).setMTPModules({std::move(grandchild)}).build();
    auto source = makeSimpleMhaCacheConfig(
        /*layer_num=*/1, /*block_num=*/5, /*tokens_per_block=*/8, DataType::TYPE_FP16, 1, 2);
    source = TestCacheConfigBuilder::rebuildForTest(std::move(source)).setMTPModules({std::move(child)}).build();
    ...
    const auto finalized = TestCacheConfigBuilder::rebuildForTest(source).setProjectedBlockCountBasis(19).build();

    ASSERT_EQ(source.mtpModuleCount(), 1u);
    ASSERT_EQ(source.mtpModule(0).mtpModuleCount(), 1u);
    EXPECT_EQ(source.blockCountBasis(), 5u);
    EXPECT_EQ(source.mtpModule(0).blockCountBasis(), 5u);
    EXPECT_EQ(source.mtpModule(0).mtpModule(0).blockCountBasis(), 5u);
    EXPECT_EQ(source.mtpModule(0).group("default").block_num, 5u);
    EXPECT_EQ(source.mtpModule(0).mtpModule(0).group("default").block_num, 5u);

    ASSERT_EQ(finalized.mtpModuleCount(), 1u);
    ASSERT_EQ(finalized.mtpModule(0).mtpModuleCount(), 1u);
    EXPECT_EQ(finalized.blockCountBasis(), 19u);
    EXPECT_EQ(finalized.mtpModule(0).blockCountBasis(), 19u);
    EXPECT_EQ(finalized.mtpModule(0).mtpModule(0).blockCountBasis(), 19u);
    EXPECT_EQ(finalized.mtpModule(0).group("default").block_num, 19u);
    EXPECT_EQ(finalized.mtpModule(0).mtpModule(0).group("default").block_num, 19u);
