...
inline void setDsv4KvCacheSpecs(ModelConfig& model_config) {
    ...
    const uint32_t kv_entry_elems = fp8_kv ? DSV4_FP8_KV_ENTRY_BYTES :
                                             static_cast<uint32_t>(model_config.attn_config.size_per_head) * 2;
    const uint32_t indexer_entry_elems = fp8_kv ? DSV4_FP8_INDEXER_ENTRY_BYTES :
                                                  static_cast<uint32_t>(model_config.attn_config.indexer_head_dim) * 2;
