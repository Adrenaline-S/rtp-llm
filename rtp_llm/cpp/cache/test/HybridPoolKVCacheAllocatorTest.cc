...
TEST(CacheConfigHybridFlagTest, SparseMlaGroupsDoNotImplyHybridAttention) {
    ModelConfig mc;
    mc.num_layers                          = 2;
    mc.attn_config.use_mla                 = true;
    mc.attn_config.is_sparse               = true;
    mc.attn_config.kv_lora_rank            = 16;
    mc.attn_config.rope_head_dim           = 8;
    ...
    mla_desc.kernel_seq_size_per_block = 8;
    ...
    indexer_desc.entry_elems               = 16;
    indexer_desc.explicit_entry_count      = 8;
    indexer_desc.kernel_seq_size_per_block = 8;
    mc.kv_cache_spec_descs.assign(2, {mla_desc, indexer_desc});
    ...
    for (const auto& group : config.topology().groups()) {
        ...
        EXPECT_TRUE(pool_config.memory_layouts.front().is_mla);
    }
}
...
static HybridPoolKVCacheAllocatorPtr makeAllocator(const CacheConfig& config, int64_t reserve_block_ratio = 0) {
    auto allocator =
        std::make_shared<HybridPoolKVCacheAllocator>(config, AllocationType::DEVICE, nullptr, reserve_block_ratio);
...
snapshotPoolCounters(const HybridPoolKVCacheAllocatorPtr& allocator) {
...
TEST_F(HybridPoolKVCacheAllocatorTest, ExplicitOnlyPoolsFallBackToTightestPoolCapacity) {
    auto config = makeTinyMultiPoolHybridConfig(/*linear_block_num=*/6, /*full_block_num=*/8);
    config.finalizeBlockNums(/*global_block_num=*/8, RuntimeConfig{});
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());

    // No pool shares the dynamic budget here, so the aggregate reports the tightest pool (linear).
    EXPECT_EQ(allocator->totalTokensNum(), 5u * 4u);
    EXPECT_EQ(allocator->availableTokensNum(), 5u * 4u);
    // Request length is still bounded by the FULL group only.
    EXPECT_EQ(allocator->maxAvailableTokensNum(), 7u * 4u);
}

TEST_F(HybridPoolKVCacheAllocatorTest, CapacityScalesToLogicalTokensUnderCP) {
    auto config = makeTinyMultiPoolHybridConfig(/*linear_block_num=*/6, /*full_block_num=*/8);
    config.finalizeBlockNums(/*global_block_num=*/8, RuntimeConfig{});
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());
    ...
    // The FULL group is CP round-robin, so its blocks now cover twice as many logical tokens. The
    // linear group is not sharded and keeps pinning the aggregate figures.
    EXPECT_EQ(allocator->totalTokensNum(), 5u * 4u);
    EXPECT_EQ(allocator->availableTokensNum(), 5u * 4u);
    EXPECT_GT(allocator->maxAvailableTokensNum(), 7u * 4u);
    EXPECT_EQ(allocator->maxAvailableTokensNum(), 7u * 4u * 2u);
}

TEST_F(HybridPoolKVCacheAllocatorTest, TokenCountsFallBackToGlobalSeqSize) {
    auto config               = makeTinyMultiPoolHybridConfig(/*linear_block_num=*/6, /*full_block_num=*/6);
    config.seq_size_per_block = 4;
    config.finalizeBlockNums(/*global_block_num=*/6, RuntimeConfig{});
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());
...
TEST_F(HybridPoolKVCacheAllocatorTest, DynamicFullPoolDrivesAvailableCapacity) {
    auto config = makeTinyMultiPoolHybridConfig(/*linear_block_num=*/3, /*full_block_num=*/8);
    setExplicitBlocksForGroup(config, kFullTag, /*block_num=*/0);
    config.finalizeBlockNums(/*global_block_num=*/8, RuntimeConfig{});
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());

    // The explicitly-sized linear pool is much tighter, but its fixed block count is provisioned for
    // its own group and must not drag the shared dynamic figures down.
    EXPECT_EQ(allocator->groupBlockPools().at(kLinearTag)->availableBlocksNum(), 2u);
    EXPECT_EQ(allocator->totalTokensNum(), 7u * 4u);
    EXPECT_EQ(allocator->availableTokensNum(), 7u * 4u);
    EXPECT_EQ(allocator->maxAvailableTokensNum(), 7u * 4u);
}

TEST_F(HybridPoolKVCacheAllocatorTest, DrainedDynamicLinearPoolBoundsAvailableCapacity) {
    auto config    = makeTinyDynamicMultiPoolHybridConfig(/*block_num=*/8);
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());

    const auto linear_pool = allocator->groupBlockPools().at(kLinearTag);
    ASSERT_EQ(linear_pool->availableBlocksNum(), 7u);
    ASSERT_EQ(allocator->groupBlockPools().at(kFullTag)->availableBlocksNum(), 7u);

    ASSERT_EQ(linear_pool->malloc(5).size(), 5u);
    ASSERT_EQ(linear_pool->availableBlocksNum(), 2u);

    // FULL is plentiful, but the drained linear pool is the real bottleneck.
    EXPECT_EQ(allocator->availableTokensNum(), 2u * 4u);
    EXPECT_EQ(allocator->totalTokensNum(), 7u * 4u);
    EXPECT_EQ(allocator->maxAvailableTokensNum(), 7u * 4u);
}

TEST_F(HybridPoolKVCacheAllocatorTest, DrainedDynamicSwaPoolBoundsAvailableCapacity) {
    auto config    = makeTinyDynamicMultiPoolHybridConfig(/*block_num=*/8, CacheGroupType::SWA);
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());

    const auto swa_pool = allocator->groupBlockPools().at(kSwaTag);
    ASSERT_EQ(swa_pool->availableBlocksNum(), 7u);

    ASSERT_EQ(swa_pool->malloc(6).size(), 6u);
    ASSERT_EQ(swa_pool->availableBlocksNum(), 1u);

    EXPECT_EQ(allocator->availableTokensNum(), 1u * 4u);
    EXPECT_EQ(allocator->totalTokensNum(), 7u * 4u);
}
...
TEST_F(HybridPoolKVCacheAllocatorTest, BlockCacheFreeDeduplicatesBlockSharedAcrossBatchEntries) {
    auto config    = makeTinyMultiPoolHybridConfig();
    auto allocator = makeAllocator(config);
    ASSERT_TRUE(allocator->init());

    const auto pool   = allocator->groupBlockPools().at(kFullTag);
    auto       blocks = pool->malloc(1);
    ASSERT_EQ(blocks.size(), 1u);
    const auto shared_block = blocks[0];

    // Two cache references stand for the two batch entries that share the block.
    pool->blockCacheReference(shared_block);
    pool->blockCacheReference(shared_block);
    pool->requestFree(blocks);
    ...
    batch->initGroups(config.topologyPtr());
    batch->mutableBlockIds(0, kFullTag).assign(BlockIndicesType{shared_block});
    batch->mutableBlockIds(1, kFullTag).assign(BlockIndicesType{shared_block});

    const size_t free_before = pool->freeBlocksNum();
    allocator->blockCacheFree(batch);

    // Released exactly once: the second cache reference survives and the block stays out of the
    // free set. Releasing it per batch entry would drop the block to zero refs.
...
TEST_F(HybridPoolKVCacheAllocatorTest, ExplicitPoolDoesNotDragDownAvailability) {
    ...
    const size_t dynamic_tokens = 49u * config.seq_size_per_block;
    const size_t hca_state_tokens =
        allocator->groupBlockPools().at("hca_state")->totalBlocksNum() * config.seq_size_per_block;
    ASSERT_LT(hca_state_tokens, dynamic_tokens);

    EXPECT_EQ(allocator->availableTokensNum(), dynamic_tokens);
    EXPECT_EQ(allocator->totalTokensNum(), dynamic_tokens);
    EXPECT_EQ(allocator->maxAvailableTokensNum(), dynamic_tokens);
}
...
TEST_F(HybridPoolKVCacheAllocatorTest, ReserveRatioExcludesExplicitIndependentPools) {
    ...
    auto              allocator     = makeAllocator(config, reserve_ratio);
    ASSERT_TRUE(allocator->init());
...
TEST_F(HybridPoolKVCacheAllocatorTest, DSV4ExplicitReserveBytesAreReportedPerGroup) {
    auto       config       = makeDSV4HybridPoolConfig(/*block_num=*/50);
    const auto explicit_tag = firstExplicitIndependentGroup(config);
    setExplicitBlocksForGroup(config, explicit_tag, 50);
...
TEST_F(HybridPoolKVCacheAllocatorTest, FinalizeAppliesNonUnitLinearStepToSwaPool) {
    auto config        = makeTinyDynamicMultiPoolHybridConfig(/*block_num=*/8, CacheGroupType::SWA);
    ...
    EXPECT_EQ(config.blockNumForGroup(kSwaTag), 5u);
}
