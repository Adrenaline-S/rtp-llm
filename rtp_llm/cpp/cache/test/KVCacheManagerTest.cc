...
#include "rtp_llm/cpp/cache/test/mock/MockCoordinatorKVCacheManager.h"
...
static void assertBlockBytesEq(const std::shared_ptr<rtp_llm::KVCacheManager>& cache_manager,
    ...
    auto addr_info = cache_manager->convertIndexToAddr(block_id, layer_id, "default");
...
static bool isFullGroup(const CacheConfig& config, std::string_view tag) {
    return config.typeForGroup(tag) == CacheGroupType::FULL;
}
...
static std::vector<std::string> dsv4GroupTagsByType(const CacheConfig& config, CacheGroupType type) {
    ...
    for (const auto& group : config.topology().groups()) {
        if (config.typeForGroup(group.tag) == type) {
...
static void expectDsv4SwaAllocatedBlocks(const CacheConfig&      config,
    ...
    for (int i = 0; i < static_cast<int>(blocks.size()); ++i) {
        ...
        if (should_allocate) {
            EXPECT_FALSE(isNullBlockIdx(blocks[static_cast<size_t>(i)])) << label << " tag " << tag << " pos " << i;
        } else {
            EXPECT_TRUE(isNullBlockIdx(blocks[static_cast<size_t>(i)])) << label << " tag " << tag << " pos " << i;
        }
    }
}
...
static CacheConfig makeDSV4ConfigWithConcurrencyPool(uint32_t full_block_num, uint32_t swa_batch_size) {
    ...
    for (const auto& group : config.topology().groups()) {
        block_nums.emplace(group.tag,
                           group.policy.group_type == CacheGroupType::FULL ? full_block_num : (2u * swa_batch_size));
    }
    setGroupBlockNumsForTest(config, block_nums);
    return config;
}
...
TEST_F(KVCacheManagerTest, InitAcceptsSingleLinearGroup) {
    ...
    EXPECT_NE(cache_manager->coordinator_cache_manager_->getBlockPool("linear"), nullptr);
...
TEST_F(KVCacheManagerTest, DSV4IndependentPoolsUseGpuBacking) {
    auto expect_pool_backing = [](RoleType role_type) {
        ...
        auto allocator =
            std::dynamic_pointer_cast<HybridPoolCoordinatorKVCacheManager>(cache_manager->coordinator_cache_manager_);
        ...
        for (const auto& [tag, pool] : allocator->groupBlockPools()) {
            EXPECT_EQ(pool->where(), MemoryType::MEMORY_GPU)
                << "role=" << static_cast<int>(role_type) << " tag=" << tag;
        }
    };
...
TEST_F(KVCacheManagerTest, SetKVBlockValueAndBlockCopy) {
    ...
    auto&        spec    = cache_manager->cacheConfig().specForGroup("default");
    ...
    ASSERT_TRUE(cache_manager->writeKVBlockForTest(block_src, "default", k_t, v_t));
    ...
    ASSERT_TRUE(cache_manager->writeKVBlockForTest(block_dst, 0, "default", k2_t, v2_t));
...
TEST_F(KVCacheManagerTest, BlockCopyAlsoCopiesScaleWhenQuantized) {
    ...
    for (int layer_id = 0; layer_id < 2; ++layer_id) {
        auto addr = cache_manager->convertIndexToAddr(block_src, layer_id, "default");
...
TEST_F(KVCacheManagerTest, DSV4MallocIncrFreeExposesSevenTypedRegions) {
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(resource->blocksNum(0, tag), 3) << "tag " << tag;
        const auto& blocks = resource->blocks(0, tag);
        if (isFullGroup(manager_config, tag)) {
            EXPECT_FALSE(isNullBlockIdx(blocks[0])) << "paged tag " << tag;
            EXPECT_FALSE(isNullBlockIdx(blocks[1])) << "paged tag " << tag;
            EXPECT_FALSE(isNullBlockIdx(blocks[2])) << "paged tag " << tag;
        } else {
            expectDsv4SwaAllocatedBlocks(manager_config, blocks, tag, "tail group");
        }
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        EXPECT_EQ(resource->blocksNum(0, tag), 4) << "tag " << tag;
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        EXPECT_EQ(layout.topology().group(tag).tag, tag);
    }
...
TEST_F(KVCacheManagerTest, DSV4BlockCopyPreservesTypedRegionBytes) {
    ...
    for (const auto& tag : kDsv4Tags) {
        const auto& blocks = resource->blocks(0, tag);
        EXPECT_NE(std::find(blocks.begin(), blocks.end(), src_block), blocks.end()) << "tag " << tag;
        if (!isHcaStateGroup(tag)) {
            EXPECT_NE(std::find(blocks.begin(), blocks.end(), dst_block), blocks.end()) << "tag " << tag;
        }
    }
    ...
    for (const auto& region_case : cases) {
        const size_t bytes = manager_config.specForGroup(region_case.tag)->block_size_bytes();
        ASSERT_GT(bytes, 0u);
...
TEST_F(KVCacheManagerTest, DSV4InsertIntoDeviceBlockCacheThenReuseSamePrefix) {
    ...
    std::unordered_map<std::string, BlockIndicesType> first_blocks;
    for (const auto& tag : kDsv4Tags) {
        first_blocks.emplace(tag, first_resource->blocks(0, tag));
    }
    ...
    for (const auto& tag : dsv4GroupTagsByType(manager_config, CacheGroupType::FULL)) {
        ASSERT_GE(second_resource->blocksNum(0, tag), 3) << "paged tag " << tag;
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        ASSERT_GE(second_resource->blocksNum(0, tag), 3) << "tail tag " << tag;
...
TEST_F(KVCacheManagerTest, DSV4InitReuseKeepsSWAPrefixTailBlock) {
    ...
    std::unordered_map<std::string, BlockIdxType> first_swa_tail_blocks;
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ASSERT_EQ(first_resource->blocksNum(0, tag), 4) << "first SWA tag " << tag;
        expectDsv4SwaAllocatedBlocks(manager_config, first_resource->blocks(0, tag), tag, "first SWA");
        first_swa_tail_blocks.emplace(tag, first_resource->blocks(0, tag)[3]);
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        ASSERT_EQ(blocks.size(), 24u) << "second SWA tag " << tag;
        ...
        EXPECT_FALSE(isNullBlockIdx(blocks[22])) << "second SWA tag " << tag << " fresh tail block 22";
        EXPECT_FALSE(isNullBlockIdx(blocks[23])) << "second SWA tag " << tag << " fresh tail block 23";
    }
...
TEST_F(KVCacheManagerTest, DSV4PopCachedBlocksPreservesGroupShape) {
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(evicted->blocksNum(0, tag), static_cast<int>(evicted->cacheKeys(0).size())) << "tag " << tag;
        for (auto block : evicted->blocks(0, tag)) {
...
TEST_F(KVCacheManagerTest, AsyncLoadCache_ReturnFromCoordinator_Success) {
    ...
    KVCacheConfig kv_cache_config;
    RuntimeConfig runtime_config;
    auto          allocator = std::make_shared<MockCoordinatorKVCacheManager>(cache_config);
...
TEST_F(KVCacheManagerTest, GetKVCacheInfo_MergesDeviceAndMemoryKeys_Dedup) {
    ...
    {
        shared_cache->put(10, {{"default", 1}}, false);
        shared_cache->put(11, {{"default", 2}}, false);
        shared_cache->put(12, {{"default", 3}}, false);
    }
...
TEST_F(KVCacheManagerTest, GetKVCacheInfoReportsDynamicPoolBottleneck) {
    ...
    for (const auto& [tag, pool] : pools) {
        ASSERT_NE(pool, nullptr) << "tag=" << tag;
    }

    // Mirror minGroupTokens, including its fallback: prefer the pools that share the dynamic block
    // budget, but fall back to every pool when the topology sizes them all explicitly, which is what
    // the unified config creator does.
    const auto min_over_pools = [&](bool dynamic_only) {
        size_t available = std::numeric_limits<size_t>::max();
        size_t total     = std::numeric_limits<size_t>::max();
        for (const auto& [tag, pool] : pools) {
            if (dynamic_only && cache_config.usesExplicitIndependentBlocks(tag)) {
                continue;
            }
            const size_t seq_size = cache_config.seq_size_per_block;
            available             = std::min(available, pool->availableBlocksNum() * seq_size);
            total                 = std::min(total, pool->totalBlocksNum() * seq_size);
        }
        return std::make_pair(available, total);
    };

    auto expected = min_over_pools(/*dynamic_only=*/true);
    if (expected.first == std::numeric_limits<size_t>::max()) {
        expected = min_over_pools(/*dynamic_only=*/false);
    }
    ASSERT_NE(expected.first, std::numeric_limits<size_t>::max());
    expected_available_tokens = expected.first;
    expected_total_tokens     = expected.second;
...
TEST_F(KVCacheManagerTest, GetKVCacheInfoExcludesExplicitPoolUnderCP) {
    ...
    // The explicitly-sized FULL pool is tighter, but only pools sharing the dynamic block budget
    // define the reported capacity.
    ASSERT_TRUE(cache_config.usesExplicitIndependentBlocks("full"));
    ASSERT_LT(full_available, linear_available);
    ...
    EXPECT_EQ(info.total_kv_cache, pools.at("linear")->totalBlocksNum() * cache_config.seq_size_per_block);
    EXPECT_EQ(info.available_kv_cache, linear_available * cache_config.seq_size_per_block);
}

TEST_F(KVCacheManagerTest, MaxSequenceLengthScalesToLogicalUnitsUnderCP) {
    auto cache_config = makeDSV4ConfigWithConcurrencyPool(/*full_block_num=*/16, /*swa_batch_size=*/3);
    ...
    const size_t     physical_capacity = (cache_config.blockNum() - 1) * cache_config.seq_size_per_block;
    ASSERT_EQ(hybrid_allocator->maxAvailableTokensNum(), physical_capacity);
    ...
    // FULL groups are CP round-robin: one physical block now holds cp_size logical blocks, so the
    // admissible request length grows with cp_size instead of staying at the physical figure.
    EXPECT_GT(kv_cache_manager->maxAvailableTokensNum(), physical_capacity);
    EXPECT_EQ(kv_cache_manager->maxAvailableTokensNum(), physical_capacity * kCpSize);
}
...
TEST_F(KVCacheManagerTest, DSV4EvictionTriggeredWhenPoolExhaustedByCache) {
    ...
    // a new allocation correctly triggers LRU eviction from each group's independent SharedBlockCache.
    //
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(res_d->blocksNum(0, tag), 3) << "tag " << tag;
        const auto& blocks = res_d->blocks(0, tag);
        if (isFullGroup(manager_config, tag)) {
            for (int i = 0; i < 3; ++i) {
                EXPECT_FALSE(isNullBlockIdx(blocks[i])) << "FULL tag " << tag << " pos " << i;
            }
        } else {
            expectDsv4SwaAllocatedBlocks(manager_config, blocks, tag, "fixed group", /*enable_reuse_cache=*/true);
        }
    }
...
TEST_F(KVCacheManagerTest, DSV4MaxConcurrencyOneReuseOneBlockAndAllocTwoTailBlocks) {
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        ASSERT_EQ(manager_config.blockNumForGroup(tag), expected) << "tag " << tag;
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ASSERT_EQ(seed_res->blocksNum(0, tag), 2) << "seed tag " << tag;
...
TEST_F(KVCacheManagerTest, DSV4EvictionOnSWAGroupsDuringInferenceWithDecodeContinuation) {
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(res_c->blocksNum(0, tag), 3) << "tag " << tag;
        const auto& blocks = res_c->blocks(0, tag);
        if (isFullGroup(manager_config, tag)) {
            for (int i = 0; i < 3; ++i) {
                EXPECT_FALSE(isNullBlockIdx(blocks[i])) << "FULL tag " << tag << " pos " << i;
            }
        } else {
            expectDsv4SwaAllocatedBlocks(manager_config, blocks, tag, "SWA group", /*enable_reuse_cache=*/true);
        }
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(res_c->blocksNum(0, tag), 4) << "tag " << tag << " after incr to 4*spb";
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(res_c->blocksNum(0, tag), 5) << "tag " << tag << " after incr to 5*spb";
    }
...
TEST_F(KVCacheManagerTest, DSV4InitThenIncrWithRemoveSkippedBlocksFullLifecycle) {
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(resource->blocksNum(0, tag), 4) << "tag " << tag;
        const auto& blocks = resource->blocks(0, tag);
        if (isFullGroup(manager_config, tag)) {
            for (int i = 0; i < 4; ++i) {
                EXPECT_FALSE(isNullBlockIdx(blocks[i])) << "FULL tag " << tag << " pos " << i;
            ...
            expectDsv4SwaAllocatedBlocks(manager_config, blocks, tag, "SWA group");
        }
    }

    // Record block IDs allocated after init for later validation.
    std::unordered_map<std::string, BlockIndicesType> init_blocks;
    for (const auto& tag : kDsv4Tags) {
        init_blocks.emplace(tag, resource->blocks(0, tag));
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(resource->blocksNum(0, tag), 5) << "tag " << tag << " after incr1";
    }
    // FULL groups: all 5 blocks should be real.
    for (const auto& tag : dsv4GroupTagsByType(manager_config, CacheGroupType::FULL)) {
        const auto& blocks = resource->blocks(0, tag);
        for (int i = 0; i < 5; ++i) {
            EXPECT_FALSE(isNullBlockIdx(blocks[i])) << "FULL tag " << tag << " pos " << i << " after incr1";
        }
        // Original init blocks should be preserved.
        for (int i = 0; i < 4; ++i) {
            EXPECT_EQ(blocks[i], init_blocks.at(tag)[i]) << "FULL tag " << tag << " pos " << i << " changed";
        }
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        if (!isHcaStateGroup(tag)) {
            EXPECT_EQ(blocks[3], init_blocks.at(tag)[3]) << "SWA tag " << tag << " old tail pos 3";
        }
    }
    ...
    std::unordered_map<std::string, BlockIdxType> swa_new_C;
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        swa_new_C.emplace(tag, resource->blocks(0, tag)[4]);
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(resource->blocksNum(0, tag), 6) << "tag " << tag << " after incr2";
    }
    ...
    for (const auto& tag : dsv4GroupTagsByType(manager_config, CacheGroupType::FULL)) {
        const auto& blocks = resource->blocks(0, tag);
        for (int i = 0; i < 6; ++i) {
            EXPECT_FALSE(isNullBlockIdx(blocks[i])) << "FULL tag " << tag << " pos " << i << " after incr2";
        }
        for (int i = 0; i < 4; ++i) {
            EXPECT_EQ(blocks[i], init_blocks.at(tag)[i]) << "FULL tag " << tag << " init block preserved";
        }
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        if (!isHcaStateGroup(tag)) {
            EXPECT_EQ(blocks[4], swa_new_C.at(tag)) << "SWA tag " << tag << " pos 4 = old C";
        }
    }
    ...
    for (const auto& tag : kDsv4Tags) {
        ASSERT_EQ(resource->blocksNum(0, tag), 7) << "tag " << tag << " after incr3";
    }
