...
// expectation is the semantic tag set.
...
static CacheConfig makeTwoLinearGroupManagerConfig() {
    auto linear0 = makeResolvedLinearSpec(DataType::TYPE_FP16,
                                          /*local_num_k_heads=*/1,
                                          /*local_num_v_heads=*/1,
                                          /*head_k_dim=*/2,
                                          /*head_v_dim=*/2,
                                          ...
                                          DataType::TYPE_FP16,
                                          "linear0");
    auto linear1 = makeResolvedLinearSpec(DataType::TYPE_FP16,
                                          /*local_num_k_heads=*/1,
                                          /*local_num_v_heads=*/1,
                                          /*head_k_dim=*/2,
                                          /*head_v_dim=*/2,
                                          ...
                                          DataType::TYPE_FP16,
                                          "linear1");
    return TestCacheConfigBuilder::makeBase(2, 4, 2, 2, DataType::TYPE_FP16)
        .setLinearStep(2)
        .setGroupedSpecs(
            {linear0, linear1}, {{0}, {1}}, {CacheGroupType::LINEAR, CacheGroupType::LINEAR}, {"linear0", "linear1"})
        .setGroupBlockLayout({4, 4}, {linear0->block_size_bytes(), linear1->block_size_bytes()}, {0, 0})
        .setPagedBlockBudgetBytes(linear0->block_size_bytes() + linear1->block_size_bytes())
        .build();
}
...
static CacheConfig makeCompactDSV4ManagerConfig(uint32_t block_num = 16) {
    ...
    auto config =
        TestCacheConfigBuilder::rebuildForTest(CacheConfigCreator::createDecodeWarmupConfig(mc, pc, KVCacheConfig{}, 0))
            .setProjectedBlockCountBasis(block_num)
            .build();
...
static void expectDsv4SwaAllocatedBlocks(const CacheConfig&      config,
    ...
    const int  linear_step        = std::max(1, config.linearStep());
...
static CacheConfig makeDSV4ConfigWithConcurrencyPool(uint32_t full_block_num, uint32_t swa_batch_size) {
    ...
    auto config =
        TestCacheConfigBuilder::rebuildForTest(CacheConfigCreator::createDecodeWarmupConfig(mc, pc, KVCacheConfig{}, 0))
            .setProjectedBlockCountBasis(full_block_num)
            .build();
    std::map<std::string, uint32_t> block_nums;
    for (const auto& group : config.groups()) {
...
static void expectInvalidTopologyRejectedAfterCoordinatorSetup(const CacheConfig& config,
    ...
    const auto warmup_config = CacheConfigCreator::withRankSyncBlockCount(config, 1);
    auto       manager       = std::make_shared<KVCacheManager>(warmup_config,
                                                    /*warmup=*/true,
    ...
    try {
        manager->init();
        FAIL() << "expected invalid cache topology to be rejected";
    ...
    EXPECT_EQ(allocator->totalBlocksNum(), 0u);
}
...
static void expectOrdinarySingleManagerUsesHybridPoolWithReuseAndRollback(const CacheConfig& roomy_config,
    ...
    EXPECT_EQ(allocator->sharedBlockCache(), manager->coordinator_cache_manager_->sharedBlockCache());
    ...
    const int  seq_size_per_block = static_cast<int>(roomy_config.cacheKeyBlockTokens());
    ...
    const auto cached_prefix_block = encodedPoolBlocksForTest(seed_resource->blockBinding(0, sole_tag))[0];
    ...
    ASSERT_FALSE(encodedPoolBlocksForTest(reuse_resource->blockBinding(0, sole_tag)).empty());
    EXPECT_EQ(encodedPoolBlocksForTest(reuse_resource->blockBinding(0, sole_tag))[0], cached_prefix_block);
    ...
    auto rollback_allocator = rollback_manager->coordinator_cache_manager_;
    ...
    const auto batch0_before   = encodedPoolBlocksForTest(target_resource->blockBinding(0, tight_sole_tag));
    const auto batch1_before   = encodedPoolBlocksForTest(target_resource->blockBinding(1, tight_sole_tag));
    ...
    EXPECT_EQ(encodedPoolBlocksForTest(target_resource->blockBinding(0, tight_sole_tag)), batch0_before);
    EXPECT_EQ(encodedPoolBlocksForTest(target_resource->blockBinding(1, tight_sole_tag)), batch1_before);
...
TEST_F(KVCacheManagerTest, WarmupConfigSmoke) {
    ...
    cache_config = CacheConfigCreator::withRankSyncBlockCount(cache_config, 1);

    auto cache_manager = std::make_shared<KVCacheManager>(cache_config, /*warmup=*/true);
    ASSERT_TRUE(cache_manager->init());

    EXPECT_EQ(cache_manager->cacheConfig().blockCountBasis(), 1);
...
TEST_F(KVCacheManagerTest, InitAcceptsSingleLinearGroup) {
    ...
    ASSERT_TRUE(cache_manager->init());
    auto allocator = cache_manager->coordinator_cache_manager_;
    ASSERT_NE(allocator, nullptr);
    ASSERT_NE(allocator->blockPool("linear"), nullptr);
    EXPECT_EQ(cache_config.group("linear").policy.group_type, CacheGroupType::LINEAR);
    EXPECT_EQ(cache_config.group("linear").spec->type, KVCacheSpecType::LinearAttention);
}

TEST_F(KVCacheManagerTest, InitRejectsSingleNullSpecAfterCoordinatorSetup) {
    auto cache_config = makeSimpleMhaCacheConfig(
        /*layer_num=*/2, /*block_num=*/4, /*tokens_per_block=*/2, rtp_llm::DataType::TYPE_BF16);
    expectInvalidTopologyRejectedAfterCoordinatorSetup(
        cache_config, "cache group tag=default has null spec", /*clear_sole_spec_before_init=*/true);
}

TEST_F(KVCacheManagerTest, ConfigRejectsMhaSpecWithLinearPolicy) {
    auto cache_config = makeSimpleMhaCacheConfig(
        /*layer_num=*/2, /*block_num=*/4, /*tokens_per_block=*/2, rtp_llm::DataType::TYPE_BF16);
    auto policy       = cache_config.soleGroupForLayer(0).policy;
    policy.group_type = CacheGroupType::LINEAR;
    EXPECT_THROW(
        (void)TestCacheConfigBuilder::rebuildForTest(std::move(cache_config)).setGroupPolicies({policy}).build(),
        std::runtime_error);
}

TEST_F(KVCacheManagerTest, ConfigRejectsLinearSpecWithFullPolicy) {
    auto cache_config = makeSimpleLinearCacheConfig(
        /*layer_num=*/2, /*block_num=*/4, /*tokens_per_block=*/2, rtp_llm::DataType::TYPE_BF16);
    auto policy       = cache_config.soleGroupForLayer(0).policy;
...
TEST_F(KVCacheManagerTest, ConfigAllowsDescriptorPolicyForOpaqueSpecs) {
    for (const auto spec_type : {KVCacheSpecType::OpaqueKV, KVCacheSpecType::OpaqueState}) {
        for (const auto group_type : {CacheGroupType::FULL, CacheGroupType::SWA, CacheGroupType::LINEAR}) {
            const bool state_cache = spec_type == KVCacheSpecType::OpaqueState;
            auto       spec        = makeResolvedOpaqueSpec(
                state_cache, "opaque", DataType::TYPE_FP16, /*block_bytes=*/32, /*seq_size_per_block=*/2);
            auto config =
                makeSingleGroupCacheConfig(std::move(spec), group_type, /*layer_num=*/1, /*block_num=*/4, "opaque");
            EXPECT_EQ(config.group("opaque").spec->type, spec_type);
            EXPECT_EQ(config.group("opaque").policy.group_type, group_type);
        }
    }
}

TEST_F(KVCacheManagerTest, InitAcceptsLinearOnlyMultiGroup) {
...
TEST_F(KVCacheManagerTest, LinearOnlyMultiGroupInsertFreeReusesDeviceCache) {
    auto cache_config = makeTwoLinearGroupManagerConfig();
    ...
    const int  seq_size_per_block = static_cast<int>(cache_config.cacheKeyBlockTokens());
    const int  seq_len            = 3 * seq_size_per_block + 1;
    ...
    for (const auto& group : cache_config.groups()) {
        ...
        const auto& blocks = encodedPoolBlocksForTest(seed_resource->blockBinding(0, tag));
    ...
    for (const auto& group : cache_config.groups()) {
        const auto& tag    = group.tag;
        const auto& blocks = encodedPoolBlocksForTest(reuse_resource->blockBinding(0, tag));
        ASSERT_EQ(blocks.size(), 4u) << "group " << tag;
        EXPECT_TRUE(isNullBlockIdx(blocks[0])) << "group " << tag;
    ...
    for (const auto& group : cache_config.groups()) {
        const auto& tag = group.tag;
        ASSERT_EQ(encodedPoolBlocksForTest(evicted->blockBinding(0, tag)).size(), 1u) << "group " << tag;
        EXPECT_EQ(encodedPoolBlocksForTest(evicted->blockBinding(0, tag))[0], cached_blocks.at(tag)) << "group " << tag;
    }
...
TEST_F(KVCacheManagerTest, OrdinarySinglePreservesLegacyBatchZeroForwardInsertionAndEvictionTrace) {
    ...
    resource->mutableBlockBinding(/*batch_id=*/0, sole_tag)
        .assign(poolBlockSnapshotForTest({blocks[0], blocks[1], blocks[2]}));
    resource->mutableBlockBinding(/*batch_id=*/1, sole_tag)
        .assign(poolBlockSnapshotForTest({blocks[3], blocks[4], blocks[5]}));
...
TEST_F(KVCacheManagerTest, MultiGroupRemoteFailsBeforeCoordinatorInitialization) {
    auto cache_config = makeSimpleHybridMhaCacheConfig(
...
TEST_F(KVCacheManagerTest, DSV4MallocIncrFreeExposesSevenTypedRegions) {
    ...
    const int    spb         = static_cast<int>(manager_config.cacheKeyBlockTokens());
    ...
    for (const auto& group : manager_config.groups()) {
        ...
        const auto& blocks = encodedPoolBlocksForTest(resource->blockBinding(0, group.tag));
    ...
    EXPECT_EQ(layout.layerCount(), static_cast<size_t>(manager_config.mainLayerCount()));

    const int csa_layer = manager_config.group("csa_kv").layer_ids[0];
    const int hca_layer = manager_config.group("hca_kv").layer_ids[0];
    EXPECT_NE(manager
                  ->convertIndexToAddrByTag(
                      encodedPoolBlocksForTest(resource->blockBinding(0, "csa_kv"))[0], csa_layer, "csa_kv")
                  .kv_addr,
              nullptr);
    EXPECT_NE(manager
                  ->convertIndexToAddrByTag(
                      encodedPoolBlocksForTest(resource->blockBinding(0, "indexer_kv"))[0], csa_layer, "indexer_kv")
                  .kv_addr,
              nullptr);
    EXPECT_NE(manager
                  ->convertIndexToAddrByTag(
                      encodedPoolBlocksForTest(resource->blockBinding(0, "csa_state"))[2], csa_layer, "csa_state")
                  .kv_addr,
              nullptr);
    EXPECT_NE(manager
                  ->convertIndexToAddrByTag(
                      encodedPoolBlocksForTest(resource->blockBinding(0, "hca_state")).back(), hca_layer, "hca_state")
                  .kv_addr,
              nullptr);
    EXPECT_NE(manager
                  ->convertIndexToAddrByTag(
                      encodedPoolBlocksForTest(resource->blockBinding(0, "hca_kv"))[0], hca_layer, "hca_kv")
                  .kv_addr,
              nullptr);
    EXPECT_NE(manager
                  ->convertIndexToAddrByTag(
                      encodedPoolBlocksForTest(resource->blockBinding(0, "swa_kv"))[2], csa_layer, "swa_kv")
                  .kv_addr,
              nullptr);
    EXPECT_ANY_THROW((void)manager->convertIndexToAddrByTag(
        encodedPoolBlocksForTest(resource->blockBinding(0, "hca_kv"))[0], csa_layer, "hca_kv"));
...
TEST_F(KVCacheManagerTest, DSV4LayerRegionBlockTablesMatchInferenceAccessPattern) {
    ...
    auto expectTagGroup = [&](int layer_id, const std::string& tag) {
        ...
        EXPECT_EQ(encodedPoolBlocksForTest(resource->cacheResource(/*batch_id=*/0).blockBindingForLayer(layer_id, tag)),
                  encodedPoolBlocksForTest(resource->blockBinding(0, tag)))
            << "layer=" << layer_id << " tag=" << tag;
        const auto&                      binding = resource->cacheResource(0).blockBindingForLayer(layer_id, tag);
        PoolBlockToKernelBlockProjection projection(manager_config.group(tag).seq_size_per_block,
                                                    manager_config.group(tag).kernel_seq_size_per_block);
        EXPECT_EQ(projection.projectedSize(binding.size()),
                  encodedPoolBlocksForTest(resource->blockBinding(0, tag)).size()
                      * manager_config.group(tag).kernelBlocksPerPoolBlock())
            << "layer=" << layer_id << " tag=" << tag;
    };
...
TEST_F(KVCacheManagerTest, DSV4BlockCopyPreservesTypedRegionBytes) {
    ...
    for (const auto& group : manager_config.groups()) {
        const auto& blocks = encodedPoolBlocksForTest(resource->blockBinding(0, group.tag));
        EXPECT_NE(std::find(blocks.begin(), blocks.end(), src_block), blocks.end()) << "group " << group.tag;
        if (!isHcaStateGroup(group.tag)) {
            EXPECT_NE(std::find(blocks.begin(), blocks.end(), dst_block), blocks.end()) << "group " << group.tag;
        }
    }
...
TEST_F(KVCacheManagerTest, DSV4InsertIntoDeviceBlockCacheThenReuseSamePrefix) {
    ...
    for (const auto& group : manager_config.groups()) {
        first_blocks[group.tag] = encodedPoolBlocksForTest(first_resource->blockBinding(0, group.tag));
    }
    ...
    for (const auto& tag : dsv4GroupTagsByType(manager_config, CacheGroupType::FULL)) {
        ...
        EXPECT_EQ(encodedPoolBlocksForTest(second_resource->blockBinding(0, tag))[0], first_blocks.at(tag)[0]);
        EXPECT_EQ(encodedPoolBlocksForTest(second_resource->blockBinding(0, tag))[1], first_blocks.at(tag)[1]);
    }
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        EXPECT_EQ(encodedPoolBlocksForTest(second_resource->blockBinding(0, tag))[2], first_blocks.at(tag)[2]);
    }
...
TEST_F(KVCacheManagerTest, DSV4InitReuseKeepsSWAPrefixTailBlock) {
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        expectDsv4SwaAllocatedBlocks(
            manager_config, encodedPoolBlocksForTest(first_resource->blockBinding(0, tag)), tag, "first SWA");
        first_swa_tail_blocks[tag] = encodedPoolBlocksForTest(first_resource->blockBinding(0, tag))[3];
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        const auto& blocks = encodedPoolBlocksForTest(second_resource->blockBinding(0, tag));
...
TEST_F(KVCacheManagerTest, DSV4PopCachedBlocksPreservesGroupShape) {
    ...
    EXPECT_EQ(evicted->cacheResource(0).blocksByTag().size(), manager_config.groups().size());
    ...
    for (const auto& group : manager_config.groups()) {
        ASSERT_EQ(evicted->blocksNum(0, group.tag), static_cast<int>(evicted->cacheKeys(0).size()))
            << "group " << group.tag;
        for (auto block : encodedPoolBlocksForTest(evicted->blockBinding(0, group.tag))) {
...
TEST_F(KVCacheManagerTest, AsyncLoadCache_ReturnFromCoordinator_Success) {
    ...
    KVCacheConfig kv_cache_config;
    RuntimeConfig runtime_config;
    auto          allocator = std::make_shared<MockCoordinatorCacheManager>(cache_config);
...
TEST_F(KVCacheManagerTest, GetKVCacheInfo_MergesDeviceAndMemoryKeys_Dedup) {
    ...
    auto mem_connector = std::make_shared<KVCacheMemoryConnector>(
        cache_config, kv_cache_config, kv_cache_manager->coordinator_cache_manager_, std::vector<std::string>{});
...
TEST_F(KVCacheManagerTest, GetKVCacheInfo_UsesSmallestHybridPoolTokenCapacity) {
    ...
    auto hybrid_allocator = kv_cache_manager->coordinator_cache_manager_;
    ...
    for (const auto& group : cache_config.groups()) {
        ...
        ASSERT_NE(pool, nullptr);
        const size_t seq_size     = cache_config.cacheKeyBlockTokens();
    ...
    EXPECT_LT(info.total_kv_cache, kv_cache_manager->totalBlocksNum() * cache_config.cacheKeyBlockTokens());
}
...
TEST_F(KVCacheManagerTest, MaxAvailableTokensNumUsesCPVirtualBlockSizeForHybridPoolFullGroups) {
    ...
    auto         cp_slot_mapper    = std::make_shared<CPSlotMapper>(
        /*cp_rank=*/0, /*cp_size=*/2, static_cast<int>(cache_config.cacheKeyBlockTokens()));
    ...
    for (const auto& group : cache_config.groups()) {
        ...
        expected_logical_capacity = std::min(expected_logical_capacity,
                                             hybrid_allocator->blockPool(group.tag)->totalBlocksNum()
                                                 * static_cast<size_t>(cache_config.cacheKeyBlockTokens() * 2));
    }
...
TEST_F(KVCacheManagerTest, GetKVCacheInfo_IncludesMemoryBlocksInTotalAndAvailable) {
    ...
    const size_t device_only_total = kv_cache_manager->coordinator_cache_manager_->totalBlocksNum()
                                     * kv_cache_manager->cacheConfig().cacheKeyBlockTokens();
...
TEST_F(KVCacheManagerTest, DSV4EvictionTriggeredWhenPoolExhaustedByCache) {
    ...
    for (const auto& group : manager_config.groups()) {
        ...
        const auto& blocks = encodedPoolBlocksForTest(res_d->blockBinding(0, group.tag));
        if (isFullGroup(manager_config, group.tag)) {
            for (int i = 0; i < 3; ++i) {
...
TEST_F(KVCacheManagerTest, DSV4MaxConcurrencyOneReuseOneBlockAndAllocTwoTailBlocks) {
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        expectDsv4SwaAllocatedBlocks(
            manager_config, encodedPoolBlocksForTest(seed_res->blockBinding(0, tag)), tag, "seed group");
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        ...
        const auto& blocks = encodedPoolBlocksForTest(reuse_res->blockBinding(0, tag));
...
TEST_F(KVCacheManagerTest, DSV4EvictionOnSWAGroupsDuringInferenceWithDecodeContinuation) {
    ...
    for (const auto& group : manager_config.groups()) {
        ...
        const auto& blocks = encodedPoolBlocksForTest(res_c->blockBinding(0, group.tag));
        if (isFullGroup(manager_config, group.tag)) {
            for (int i = 0; i < 3; ++i) {
                EXPECT_FALSE(isNullBlockIdx(blocks[i])) << "FULL group " << group.tag << " pos " << i;
            }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        expectDsv4SwaAllocatedBlocks(
            manager_config, encodedPoolBlocksForTest(res_c->blockBinding(0, tag)), tag, "SWA group");
    }
...
TEST_F(KVCacheManagerTest, DSV4InitThenIncrWithRemoveSkippedBlocksFullLifecycle) {
    ...
    for (const auto& group : manager_config.groups()) {
        init_blocks[group.tag] = encodedPoolBlocksForTest(resource->blockBinding(0, group.tag));
    }
    ...
    for (const auto& tag : dsv4FixedTailGroupTags(manager_config)) {
        swa_new_C[tag] = encodedPoolBlocksForTest(resource->blockBinding(0, tag))[4];
    }
