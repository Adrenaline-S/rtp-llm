...
TEST_F(KVCacheManagerTest, GetKVCacheInfo_MergesDeviceAndMemoryKeys_Dedup) {
    ...
    {
        using GroupBlock = SharedBlockCache::UnifiedCacheItem::GroupBlock;
        shared_cache->put(10, {GroupBlock{"default", 1}}, false);
        shared_cache->put(11, {GroupBlock{"default", 2}}, false);
        shared_cache->put(12, {GroupBlock{"default", 3}}, false);
    }
...
TEST_F(KVCacheManagerTest, MaxAvailableTokensNumUsesCPVirtualBlockSizeForHybridPoolFullGroups) {
    ...
    auto cp_slot_mapper = std::make_shared<CPSlotMapper>(0, 2, static_cast<int>(cache_config.seq_size_per_block));
    ...
    for (const auto& group : cache_config.topology().groups()) {
        ...
        expected_logical_capacity =
            std::min(expected_logical_capacity,
                     pool->totalBlocksNum() * cp_slot_mapper->logicalSeqSizePerBlock(cache_config, group.tag));
    }
