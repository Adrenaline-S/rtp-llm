...
TaggedBlock taggedBlock(std::string_view tag, BlockIdxType block_id, bool matchable = true) {
    return {std::string(tag), block_id, matchable};
}

BlockIdxType evictedBlockForTag(const SharedBlockCache::EvictResult& result, CacheKeyType key, std::string_view tag) {
    const auto& blocks = result.evicted_blocks_by_tag.at(key);
    const auto  it = std::find_if(blocks.begin(), blocks.end(), [tag](const auto& block) { return block.tag == tag; });
    return it == blocks.end() ? NULL_BLOCK_IDX : it->block_id;
}
...
void putOne(SharedBlockCache&             cache,
            ...
            bool                          resident     = false) {
    cache.put(key, {taggedBlock(kFullTag, block)}, resident, namespace_id, dep);
}
...
TEST(SharedBlockCacheTest, EvictionPreservesPhysicalOrdinal) {
    TestSharedBlockCache cache;
    TaggedBlock          block{std::string(kFullTag),
                      /*block_id=*/101,
                      /*matchable=*/true,
                      /*created_time_us=*/0,
                      /*physical_ordinal=*/7};
    ...
    ASSERT_TRUE(evicted.evicted_blocks_by_tag.count(123));
    ASSERT_EQ(evicted.evicted_blocks_by_tag.at(123).size(), 1u);
    EXPECT_EQ(evicted.evicted_blocks_by_tag.at(123).front().physical_ordinal, 7u);
...
TEST(SharedBlockCacheTest, NonMatchableSlotStillEvictsButDoesNotMatchGroup) {
    TestSharedBlockCache cache;
    cache.put(1,
              {taggedBlock(kFullTag, 101), taggedBlock(kLinearTag, 201, /*matchable=*/false)},
              /*is_resident=*/false,
...
TEST(SharedBlockCacheTest, StateIndependentEvictionDropsDeepestNonLeafStateFirst) {
    ...
    cache.put(1,
              {taggedBlock(kFullTag, 101), taggedBlock(kIndependentTag, 301)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
              rootDep(0));
    cache.put(2,
              {taggedBlock(kFullTag, 102), taggedBlock(kIndependentTag, 302)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
              childDep(1, 1));
    cache.put(3,
              {taggedBlock(kFullTag, 103), taggedBlock(kIndependentTag, 303)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
              childDep(2, 2));
...
TEST(SharedBlockCacheTest, StateIndependentEvictionScansMultipleLeavesSafely) {
    ...
    cache.put(3,
              ...
              childDep(2, 2));
    cache.put(10,
              {taggedBlock(kFullTag, 110), taggedBlock(kIndependentTag, 310)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
              rootDep(0));
    cache.put(11,
              {taggedBlock(kFullTag, 111), taggedBlock(kIndependentTag, 311)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
              childDep(10, 1));
    cache.put(12,
              {taggedBlock(kFullTag, 112), taggedBlock(kIndependentTag, 312)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
              childDep(11, 2));
...
TEST(SharedBlockCacheTest, StateIndependentEvictionFallsBackToWholeChainWhenOnlyLeafStateRemains) {
    ...
    cache.put(1, {taggedBlock(kFullTag, 101)}, false, SharedBlockCache::kGpuLogicalNamespace, rootDep(0));
    cache.put(2,
              {taggedBlock(kFullTag, 102), taggedBlock(kIndependentTag, 302)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
...
TEST(SharedBlockCacheTest, SelectAndEvictForGroupSkipsChainsWithoutTargetSlot) {
    ...
    cache.put(1, {taggedBlock(kFullTag, 101)}, false, SharedBlockCache::kGpuLogicalNamespace, rootDep(0));
    cache.put(2, {taggedBlock(kFullTag, 102)}, false, SharedBlockCache::kGpuLogicalNamespace, childDep(1, 1));
    cache.put(10, {taggedBlock(kFullTag, 110)}, false, SharedBlockCache::kGpuLogicalNamespace, rootDep(0));
    cache.put(11,
              {taggedBlock(kFullTag, 111), taggedBlock(kIndependentTag, 311)},
              false,
              SharedBlockCache::kGpuLogicalNamespace,
...
TEST(SharedBlockCacheTest, SelectAndEvictForGroupPrunesBranchUntilTargetAncestorIsEvictable) {
    TestSharedBlockCache cache;
    cache.put(1,
              {taggedBlock(kFullTag, 101), taggedBlock(kLinearTag, 201)},
              /*is_resident=*/false,
              SharedBlockCache::kGpuLogicalNamespace,
              rootDep(0));
    cache.put(2,
              {taggedBlock(kFullTag, 102)},
              /*is_resident=*/false,
              SharedBlockCache::kGpuLogicalNamespace,
              childDep(1, 1));
    cache.put(3,
              {taggedBlock(kFullTag, 103)},
              /*is_resident=*/false,
              SharedBlockCache::kGpuLogicalNamespace,
              childDep(1, 2));
