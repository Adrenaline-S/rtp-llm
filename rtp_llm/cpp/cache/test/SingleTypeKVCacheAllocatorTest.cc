...
// Test init
TEST_F(KVCacheAllocatorTest, ConstructorAndInit) {
...
TEST_F(KVCacheAllocatorTest, InitSupportsSingleLinearGroup) {
...
TEST_F(KVCacheAllocatorTest, InitWithDifferentLayerNum) {
...
TEST_F(KVCacheAllocatorTest, GetNeedBlocksComputesCommonAndExtra) {
...
// Test malloc
TEST_F(KVCacheAllocatorTest, MallocSingleBatch) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, ReserveBlocksOnlyAppliedToInitMalloc) {
...
TEST_F(KVCacheAllocatorTest, ReserveBlocksCheckHappensAfterReuseReferenceInInitMallocForCommonLen) {
    auto config = createSingleTypeTestConfig(/*layer_num=*/4, /*block_num=*/10, /*seq_size_per_block=*/4);
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    ...
    {
        auto batch_resource = createBatchKVCacheResource(/*batch_size=*/1, config);
        batch_resource->setBatchCacheKeys(0, "default", CacheKeysType{100, 101, 102, 103, 200});
...
TEST_F(KVCacheAllocatorTest, MallocMultipleBatches) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
// TEST_F(KVCacheAllocatorTest, MallocWithInsufficientBlocks) {
...
// Test free
TEST_F(KVCacheAllocatorTest, FreeSingleBatch) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, FreeMultipleBatches) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, MallocFreeCycle) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, InsertIntoCache) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, InsertIntoCacheAsResident) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, PrefixReuseDisabledSkipsMatchAndInsert) {
    ...
    for (size_t i = 0; i < cached_blocks.size(); ++i) {
        shared_cache->put(static_cast<CacheKeyType>(100 + i),
                          {SharedBlockCache::UnifiedCacheItem::GroupBlock{"default", cached_blocks[i]}},
                          true);
    }
...
TEST_F(KVCacheAllocatorTest, ConvertIndexToAddr) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, ConvertToGlobalLayerIdSingleNoMtp) {
...
TEST_F(KVCacheAllocatorTest, ConvertToGlobalLayerIdSingleWithMtp) {
...
TEST_F(KVCacheAllocatorTest, MtpGlobalLayerIdRejectsInvalidModuleAndLocalIds) {
...
TEST_F(KVCacheAllocatorTest, SingleLayerMtpConfigSlicesDescriptorAndAttentionType) {
...
TEST_F(KVCacheAllocatorTest, SingleLayerMtpConfigSupportsDescriptorDrivenGroupPools) {
    auto config                                            = makeTestModelConfig(/*num_layers=*/2);
    config.hybrid_attention_config.enable_hybrid_attention = true;
...
TEST_F(KVCacheAllocatorTest, SingleLayerMtpConfigSupportsDescriptorsWithoutAttentionTypes) {
    auto config                                            = makeTestModelConfig(/*num_layers=*/2);
    config.hybrid_attention_config.enable_hybrid_attention = true;
    config.hybrid_attention_config.hybrid_attention_types  = {};
...
TEST_F(KVCacheAllocatorTest, ActiveMtpCacheLayoutValidationOnlyChecksModule0) {
    auto config                                            = makeTestModelConfig(/*num_layers=*/2);
    config.hybrid_attention_config.enable_hybrid_attention = true;
...
TEST_F(KVCacheAllocatorTest, MtpModuleConfigPlanKeepsWeightsAndCopiesActiveCacheLayout) {
    auto config                                 = makeTestModelConfig(/*num_layers=*/2);
...
TEST_F(KVCacheAllocatorTest, ConvertIndexToBuffer) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, LayerCacheBase) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, ManagerLayoutsPreserveSingleTypeGroupTensorsForMainAndMtp) {
    auto config = makeMtpCacheConfigByCreateSpConfig(
        /*main_layers=*/2, /*mtp_module_num=*/2, /*block_num=*/8, /*mtp_module_layers=*/3);
...
// Test block copy
TEST_F(KVCacheAllocatorTest, BlockCopySingle) {
    auto config = createSingleTypeTestConfig();
...
TEST_F(KVCacheAllocatorTest, BlockBatchCopyVector) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, BlockBatchCopyEmpty) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, BlockBatchCopyCopiesCompleteSparseIndexerStride) {
...
TEST_F(KVCacheAllocatorTest, BlockBatchCopyPointers) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, BlockBatchCopyBuffer) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    allocator_->init();
...
// Test getter methods
TEST_F(KVCacheAllocatorTest, FreeBlocksNums) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, AvailableBlocksNums) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, IncrKVCacheRefReferencesMatchedBlocksOnly) {
    auto config = createSingleTypeTestConfig(/*layer_num=*/4, /*block_num=*/10, /*seq_size_per_block=*/8);
    allocator_  = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator_->init());
...
TEST_F(KVCacheAllocatorTest, IncrKVCacheRefPreservesConnectorDummyTail) {
    auto config = createSingleTypeTestConfig(/*layer_num=*/4, /*block_num=*/10, /*seq_size_per_block=*/8);
    allocator_  = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator_->init());
...
TEST_F(KVCacheAllocatorTest, IncrKVCacheRefEmptyInputNoEffect) {
    auto config = createSingleTypeTestConfig(/*layer_num=*/4, /*block_num=*/10, /*seq_size_per_block=*/8);
    allocator_  = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator_->init());
...
TEST_F(KVCacheAllocatorTest, TotalBlocksNums) {
...
TEST_F(KVCacheAllocatorTest, MaxSeqLen) {
...
TEST_F(KVCacheAllocatorTest, CapacityAndNeedBlocksUseCPVirtualBlockSize) {
    auto config = createSingleTypeTestConfig(/*layer_num=*/4, /*block_num=*/10, /*seq_size_per_block=*/8);
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    ASSERT_TRUE(allocator_->init());
...
TEST_F(KVCacheAllocatorTest, MallocWithZeroSeqLength) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, FreeEmptyBatchResource) {
    auto config = createSingleTypeTestConfig();
    allocator_  = std::make_shared<KVCacheAllocator>(config);
    allocator_->init();
...
TEST_F(KVCacheAllocatorTest, InitMallocRollbackWhenInitMallocForCommonLenFails) {
    ...
    batch_resource->setBatchCacheKeys(0, "default", CacheKeysType{100, 101});
...
TEST_F(KVCacheAllocatorTest, IncrMallocRollback) {
...
TEST_F(KVCacheAllocatorTest, InitMallocRollbackWhenIncrMallocFails) {
...
TEST_F(KVCacheAllocatorTest, MixedOperations) {
...
TEST_F(KVCacheAllocatorTest, EstimateBatchPeakCoversPartialTailCopiesAtExactCapacity) {
