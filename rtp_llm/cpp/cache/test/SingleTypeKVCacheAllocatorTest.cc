...
TEST_F(SingleTypeKVCacheAllocatorTest, BlockBatchCopyCopiesCompleteSparseIndexerStride) {
    ...
    ASSERT_GT(config.kvScaleStrideBytes(), 0u);
    ASSERT_EQ(config.kvScaleStrideBytes(), config.kvScaleStrideBytesForGroup("default"));

    allocator_ = std::make_shared<SingleTypeKVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator_->init());

    const auto stride   = config.kvScaleStrideBytes();
    auto       snapshot = [&]() {
