...
TEST(HybridPoolConfigCreatorTest, MtpSwaOnlyLayerIsNotStripped) {
    ...
    ASSERT_EQ(config.layer_tag_to_group_id.size(), 1u);
    EXPECT_EQ(config.layer_tag_to_group_id[0][DSV4_TAG_SWA_KV], 6);
...
TEST(HybridPoolConfigCreatorTest, Dsv4SpecOrderDoesNotAffectLegacyGroupOrder) {
    ...
    const std::vector<std::string> expected_regions = {DSV4_TAG_CSA_KV,
                                                             DSV4_TAG_HCA_KV,
                                                             DSV4_TAG_INDEXER_KV,
    ...
    EXPECT_EQ(config.group_tags, expected_regions);
    ...
    EXPECT_EQ(config.layer_tag_to_group_id[2][DSV4_TAG_CSA_KV], 0);
    EXPECT_EQ(config.layer_tag_to_group_id[3][DSV4_TAG_HCA_KV], 1);
    EXPECT_EQ(config.layer_tag_to_group_id[2][DSV4_TAG_INDEXER_KV], 2);
    EXPECT_EQ(config.layer_tag_to_group_id[0][DSV4_TAG_SWA_KV], 6);
...
TEST(CacheConfigTest, FromLayerSpecsBuildsTagTopology) {
    CacheConfig config;
    config.layer_num     = 3;
    config.layer_all_num = 3;

    auto swa_spec = std::make_shared<DSV4StateSpec>();
    swa_spec->tag = DSV4_TAG_SWA_KV;
    ...
    layer_specs[0] = {swa_spec};
    layer_specs[1] = {swa_spec, csa_spec};
    layer_specs[2] = {swa_spec};
    config.fromLayerSpecs(layer_specs);

    EXPECT_EQ(config.group_tags,
              std::vector<std::string>({DSV4_TAG_SWA_KV, DSV4_TAG_CSA_KV}));
    EXPECT_EQ(config.layer_tag_to_group_id[1][DSV4_TAG_SWA_KV], 0);
    EXPECT_EQ(config.layer_tag_to_group_id[1][DSV4_TAG_CSA_KV], 1);
    EXPECT_EQ(config.groupIdForLayerTag(1, DSV4_TAG_SWA_KV), 0);
    EXPECT_EQ(config.groupIdForLayerTag(1, DSV4_TAG_CSA_KV), 1);
...
TEST(CacheConfigTest, FromLayerSpecsRejectsMissingLayer) {
    CacheConfig config;
    config.layer_num     = 2;
    config.layer_all_num = 2;
...
TEST(CacheConfigTest, FromLayerSpecsRejectsEmptyTag) {
    CacheConfig config;
    config.layer_num     = 1;
    config.layer_all_num = 1;
...
TEST(CacheConfigTest, FromLayerSpecsAllowsDuplicateLayerRoutesWhenTagsDiffer) {
    CacheConfig config;
    config.layer_num     = 1;
    config.layer_all_num = 1;
    ...
    LayerKVCacheSpecs layer_specs;
    layer_specs[0] = {spec0, spec1};
    ASSERT_NO_THROW(config.fromLayerSpecs(layer_specs));
    EXPECT_EQ(config.groupIdForLayerTag(0, "full"), 0);
    EXPECT_EQ(config.groupIdForLayerTag(0, "linear"), 1);
    ASSERT_EQ(config.layer_to_group_ids[0], std::vector<int>({0, 1}));
}
...
TEST(HybridPoolConfigCreatorTest, Dsv4ModelProvidedAlignmentPropagatesToCacheSpecs) {
    ...
    for (auto& layer_specs : mc.kv_cache_specs) {
        for (auto& spec : layer_specs.second) {
            if (spec->tag == "csa_kv") {
                auto* kv_spec = dynamic_cast<DSV4KVSpec*>(spec.get());
                ASSERT_NE(kv_spec, nullptr);
                kv_spec->block_size_bytes_alignment = 1024;
            } else if (spec->tag == "swa_kv") {
                auto* state_spec = dynamic_cast<DSV4StateSpec*>(spec.get());
...
TEST(HybridPoolConfigCreatorTest, Dsv4TagRoutesAreConsistent) {
    ...
    auto expect_route = [&](int layer_id, const std::string& tag, std::string region, int expected_gid) {
        ASSERT_LT(static_cast<size_t>(layer_id), config.layer_tag_to_group_id.size());
        const int region_gid =
            config.layer_tag_to_group_id[static_cast<size_t>(layer_id)][region];
    ...
    expect_route(2, "csa_kv", DSV4_TAG_CSA_KV, 0);
    expect_route(2, "indexer_kv", DSV4_TAG_INDEXER_KV, 2);
    expect_route(2, "indexer_state", DSV4_TAG_INDEXER_STATE, 3);
    expect_route(2, "csa_state", DSV4_TAG_CSA_STATE, 4);
    expect_route(2, "swa_kv", DSV4_TAG_SWA_KV, 6);

    expect_route(3, "hca_kv", DSV4_TAG_HCA_KV, 1);
    expect_route(3, "hca_state", DSV4_TAG_HCA_STATE, 5);
    expect_route(3, "swa_kv", DSV4_TAG_SWA_KV, 6);

    expect_route(0, "swa_kv", DSV4_TAG_SWA_KV, 6);
    ...
    ASSERT_EQ(mtp_config.layer_tag_to_group_id[0][DSV4_TAG_SWA_KV], 6);
}
...
TEST(HybridPoolConfigCreatorTest, HybridAttentionIndependentPoolSplitsFullAndSwaSpecs) {
    ...
    EXPECT_EQ(config.group_tags, std::vector<std::string>({"full", "swa", "linear"}));
    ...
    EXPECT_EQ(config.global_layer_ids[0].size(), 1u);
    EXPECT_EQ(config.global_layer_ids[1].size(), 2u);
    EXPECT_EQ(config.global_layer_ids[2].size(), 1u);
...
TEST(DSV4KVCacheSpecTest, KVSpecFromPoolSpec) {
    DSV4KVSpec spec(DSV4_TAG_CSA_KV,
...
TEST(DSV4KVCacheSpecTest, SWAFp8StateSpecUsesPaddedPhysicalBlockSize) {
    DSV4StateSpec spec(DSV4_TAG_SWA_KV,
                       kDsv4Fp8KvEntryBytes,
                       kDsv4TokensPerBlock,
                       DataType::TYPE_UINT8,
                       kDsv4TokensPerBlock,
    ...
    EXPECT_EQ(spec.tag, DSV4_TAG_SWA_KV);
}

TEST(DSV4KVCacheSpecTest, StateSpecFloat32) {
    DSV4StateSpec spec(DSV4_TAG_CSA_STATE, 2048, 8, DataType::TYPE_FP32, kDsv4TokensPerBlock);

    EXPECT_EQ(spec.block_size(), 8u * 2048u);
    EXPECT_EQ(spec.block_size_bytes(), 8u * 2048u * 4u);  // float32 = 4 bytes
    EXPECT_EQ(spec.tag, DSV4_TAG_CSA_STATE);
    EXPECT_EQ(spec.state_dim, 2048u);
}

TEST(DSV4KVCacheSpecTest, IndexerKVSpec) {
    DSV4KVSpec spec(DSV4_TAG_INDEXER_KV, 132, 64, DataType::TYPE_UINT8, kDsv4TokensPerBlock);

    EXPECT_EQ(spec.block_size(), 64u * 132u);
    EXPECT_EQ(spec.block_size_bytes(), 64u * 132u);
    EXPECT_EQ(spec.tag, DSV4_TAG_INDEXER_KV);
}

TEST(DSV4KVCacheSpecTest, HCAStateSpec) {
    DSV4StateSpec spec(DSV4_TAG_HCA_STATE, 1024, 128, DataType::TYPE_FP32, kDsv4TokensPerBlock);

    EXPECT_EQ(spec.block_size_bytes(), 128u * 1024u * 4u);
    EXPECT_EQ(spec.tag, DSV4_TAG_HCA_STATE);
}
...
TEST(CacheConfigTest, ModelSpecCloneKeepsExistingConfigStable) {
    ...
    EXPECT_EQ(config_tp1.cache_specs[0]->local_head_num_kv, 4u);
    EXPECT_NE(config_tp1.cache_specs[0].get(), model_config.kv_cache_specs[0][0].get());
    EXPECT_NE(config_tp2.cache_specs[0].get(), model_config.kv_cache_specs[0][0].get());
...
TEST(CacheConfigTest, DSV4MtpKeepsProposeLayerInSwaPool) {
    ...
    EXPECT_EQ(config.layer_tag_to_group_id[43][DSV4_TAG_SWA_KV], 6);
    EXPECT_EQ(config.layer_tag_to_group_id[44][DSV4_TAG_SWA_KV], 6);
...
TEST(HybridPoolConfigCreatorTest, MtpGenNum2RingEntriesMatch) {
    ...
    auto* swa_kv = dynamic_cast<DSV4StateSpec*>(config.cache_specs[6].get());
    ASSERT_NE(swa_kv, nullptr);
    EXPECT_EQ(swa_kv->tag, DSV4_TAG_SWA_KV);
...
TEST_F(DSV4AllocatorTest, HCAStateIsExcludedFromReuseCachePolicy) {
    ...
    for (size_t gid = 0; gid < config.group_tags.size(); ++gid) {
        if (gid == 5) {
            EXPECT_TRUE(config.group_skip_prefix_reuse[gid]) << "HCA_STATE should skip reuse cache";
        } else {
            EXPECT_FALSE(config.group_skip_prefix_reuse[gid]) << "group " << gid;
        }
    }
}
