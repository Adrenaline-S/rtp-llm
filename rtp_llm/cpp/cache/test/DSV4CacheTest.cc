...
static CacheConfig makeSingleStateCpConfig(const KVCacheSpec& spec, int cp_size) {
    ...
    group.policy.cp_slice = spec.block_size_bytes() == spec.block_payload_bytes() ?
...
TEST(HybridPoolConfigCreatorTest, MtpSwaOnlyLayerIsNotStripped) {
    ParallelismConfig pc;
    auto              config =
        CacheConfigCreator::createBasicConfig(makeFlashMtpModelConfig(), pc, true, 0);
...
TEST(HybridPoolConfigCreatorTest, Dsv4SpecsMissingFailsFastWithoutRatioFallback) {
    ...
    ParallelismConfig pc;
    EXPECT_THROW((void)CacheConfigCreator::createBasicConfig(mc, pc, false, 0),
                 std::exception);
}
...
TEST(HybridPoolConfigCreatorTest, HybridAttentionIndependentPoolUsesHybridPoolConfig) {
    ParallelismConfig pc;
    auto              config =
        CacheConfigCreator::createBasicConfig(makeHybridAttentionModelConfig(true), pc, false, 0);
...
TEST(HybridPoolConfigCreatorTest, HybridAttentionWithoutIndependentPoolKeepsSharedHybridConfig) {
    ParallelismConfig pc;
    auto              config =
        CacheConfigCreator::createBasicConfig(makeHybridAttentionModelConfig(false), pc, false, 0);
...
TEST(HybridPoolConfigCreatorTest, PagedPoolsShareTokensPerBlock) {
    // Pro config
    {
        ParallelismConfig pc;
        auto              config =
            CacheConfigCreator::createBasicConfig(makeProModelConfig(), pc, false, 0);
        EXPECT_EQ(config.seq_size_per_block, kDsv4TokensPerBlock);
    }
    // Flash config
    {
        ParallelismConfig pc;
        auto              config =
            CacheConfigCreator::createBasicConfig(makeFlashModelConfig(), pc, false, 0);
        EXPECT_EQ(config.seq_size_per_block, kDsv4TokensPerBlock);
    }
}
...
TEST(HybridPoolConfigCreatorTest, MtpGenNum2RingEntriesMatch) {
    ...
    ParallelismConfig pc;
    auto              config =
        CacheConfigCreator::createBasicConfig(mc, pc, false, /*gen_num_per_cycle=*/2);

    ASSERT_EQ(static_cast<size_t>(config.groupNums()), 7u);
    // Pool 3: INDEXER_STATE (ratio=4, overlap=1) → R=10
    auto* indexer_state =
        dynamic_cast<FixedStateCacheSpec*>(config.specForGroup(gidForTag(config, "indexer_state")).get());
...
TEST(HybridPoolConfigCreatorTest, DecodePrefillCp8MtpGenNum2ExpandsFixedAndSwaSlices) {
    ...
    auto* prefill_swa =
        dynamic_cast<FixedStateCacheSpec*>(prefill_config.specForGroup(gidForTag(prefill_config, "swa_kv")).get());
    auto* decode_swa =
        dynamic_cast<FixedStateCacheSpec*>(decode_config.specForGroup(gidForTag(decode_config, "swa_kv")).get());
    ...
    auto* indexer_state =
        dynamic_cast<FixedStateCacheSpec*>(decode_config.specForGroup(gidForTag(decode_config, "indexer_state")).get());
    auto* csa_state =
        dynamic_cast<FixedStateCacheSpec*>(decode_config.specForGroup(gidForTag(decode_config, "csa_state")).get());
    auto* hca_state =
        dynamic_cast<FixedStateCacheSpec*>(decode_config.specForGroup(gidForTag(decode_config, "hca_state")).get());
    auto* swa_kv =
        dynamic_cast<FixedStateCacheSpec*>(decode_config.specForGroup(gidForTag(decode_config, "swa_kv")).get());
    ASSERT_NE(indexer_state, nullptr);
    ASSERT_NE(csa_state, nullptr);
...
TEST_F(DSV4AllocatorTest, HCAStateIsExcludedFromReuseCachePolicy) {
    ...
    for (size_t gid = 0; gid < static_cast<size_t>(config.groupNums()); ++gid) {
        if (config.tagForGroup(gid) == "hca_state") {
            EXPECT_EQ(config.policyForGroup(gid).enable_prefix_reuse, false)
