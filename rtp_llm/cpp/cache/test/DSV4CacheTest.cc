...
static BlockPoolPtr dsv4PoolFor(const KVCacheAllocatorPtr& allocator, std::string_view tag) {
    return allocator->blockPool(tag);
}
...
TEST(HybridPoolConfigCreatorTest, DSV4StateSwaPoolsFollowGlobalBlocks) {
    ...
    EXPECT_EQ(config.policyForGroup("hca_state").explicit_block_num, 0u);
}
...
TEST(CacheConfigTest, FinalizeBlockNumsAppliesToIndependentPools) {
    ...
    EXPECT_EQ(config.policyForGroup("hca_state").explicit_block_num, 256u);
}
