...
TEST_F(HybridPoolKVCacheAllocatorTest, HeterogeneousReuseLengthIsIndependentOfGroupOrder) {
...
TEST_F(HybridPoolKVCacheAllocatorTest, FullPromptHitKeepsOneTokenForPrefill) {
...
TEST_F(HybridPoolKVCacheAllocatorTest, IncrDecrKVCacheRefReferencesOnlyMatchedValidBlocksAcrossGroups) {
    ...
    const CacheKeysByTag requested_keys{{std::string(kLinearTag), CacheKeysType{101, 999, 102}},
...
TEST_F(HybridPoolKVCacheAllocatorTest, ReserveCapacitySkipsLogicalGroupsMarkedNonReservable) {
    auto config = makeTinyHybridConfig();
    ...
    auto allocator =
        std::make_shared<HybridPoolKVCacheAllocator>(config, AllocationType::HOST, nullptr, /*reserve_ratio=*/50);
    ASSERT_TRUE(allocator->init());
