...
std::shared_ptr<MHAKVCacheSpec> makeSwaSpec(int seq_size_per_block) {
    return std::dynamic_pointer_cast<MHAKVCacheSpec>(makeMhaSpec("swa",
                                                                 seq_size_per_block,
                                                                 DataType::TYPE_FP16,
                                                                 /*local_head_num_kv=*/1,
                                                                 /*size_per_head=*/1));
...
GroupBase makeSwaGroupBase(const KVCacheSpecPtr& spec,
                           CacheGroupPolicy      policy = defaultCacheGroupPolicy(CacheGroupType::SWA)) {
    return makeTestGroupBase(spec, policy);
}
...
protected:
    void SetUp() override {
        ...
        shared_cache_->init({{"swa", block_pool_}, {"hca_state", block_pool_}, {"csa_state", block_pool_}});
    }
    ...
    SWAKVCacheGroup makeGroup(int seq_size_per_block) {
        auto spec = makeSwaSpec(seq_size_per_block);
        return SWAKVCacheGroup(makeSwaGroupBase(spec), block_pool_, /*linear_step=*/0, shared_cache_.get());
    }

    SWAKVCacheGroup makeGroupWithStep(int seq_size_per_block, int linear_step) {
        auto spec = makeSwaSpec(seq_size_per_block);
        return SWAKVCacheGroup(makeSwaGroupBase(spec, makePolicy(/*skip_prefix_reuse=*/false)),
                               block_pool_,
                               linear_step,
                               shared_cache_.get());
    }
...
TEST_F(SWAKVCacheGroupTest, DefaultPolicyDrivesBehaviorInterfaces) {
    auto            spec = makeSwaSpec(4);
    SWAKVCacheGroup group(makeSwaGroupBase(spec), block_pool_, /*linear_step=*/0, shared_cache_.get());
...
TEST_F(SWAKVCacheGroupTest, GetNeedBlocks_HCAStateReuseEnabledCountsTailOnly) {
    ...
    auto group =
        SWAKVCacheGroup(makeSwaGroupBase(spec, makePolicy(true)), block_pool_, /*linear_step=*/3, shared_cache_.get());
...
TEST_F(SWAKVCacheGroupTest, GetNeedBlocks_CSAStateReuseEnabledStillUsesSparse) {
    ...
    auto group =
        SWAKVCacheGroup(makeSwaGroupBase(spec, makePolicy()), block_pool_, /*linear_step=*/3, shared_cache_.get());
...
TEST_F(SWAKVCacheGroupTest, Malloc_SkipsNullTailCheckWhenPolicyDisablesValidation) {
    auto spec = makeSwaSpec(4);
    auto group =
        SWAKVCacheGroup(makeSwaGroupBase(spec, makePolicy(true)), block_pool_, /*linear_step=*/0, shared_cache_.get());
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheGroupTest, Malloc_RejectsNullTailWhenValidationEnabled) {
    auto spec = makeSwaSpec(4);
    auto group =
        SWAKVCacheGroup(makeSwaGroupBase(spec, makePolicy()), block_pool_, /*linear_step=*/0, shared_cache_.get());
    BlockIds block_ids(1);
    block_ids.assign(BlockIndicesType{NULL_BLOCK_IDX, NULL_BLOCK_IDX, NULL_BLOCK_IDX});
...
TEST_F(SWAKVCacheGroupTest, RemoveSkippedBlocks_WithStep_FreesNonStepBlocks) {
    ...
    auto            spec = makeSwaSpec(4);
    SWAKVCacheGroup group(
        makeSwaGroupBase(spec, makePolicy(/*skip_prefix_reuse=*/false)), block_pool, /*linear_step=*/2);
...
TEST_F(SWAKVCacheGroupTest, RemoveSkippedBlocks_HCAStateReuseEnabledKeepsTailOnly) {
    ...
    auto spec  = makeDsv4StateSpec("hca_state", 4);
    auto group = SWAKVCacheGroup(makeSwaGroupBase(spec, makePolicy(true)), block_pool, /*linear_step=*/2);
