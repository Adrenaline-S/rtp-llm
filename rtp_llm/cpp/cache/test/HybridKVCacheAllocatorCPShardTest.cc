...
// piggybacks on the helpers in KVCacheAllocatorTest.cc but
...
CacheConfig makeCPHybridConfig() {
    ...
    setTestTopology(config,
                    {makeTestGroupForConfig(linear_spec, {0, 1}, CacheGroupType::LINEAR, "linear"),
                     makeTestGroupForConfig(full_spec, {2, 3}, CacheGroupType::FULL, "full")});
...
void setCPMapper(const KVCacheAllocatorPtr& allocator, int cp_rank, int cp_size) {
    allocator->setCPSlotMapper(std::make_shared<CPSlotMapper>(cp_rank, cp_size, /*block_size=*/4));
}
...
seedCache(BlockPoolPtr block_pool, SharedBlockCachePtr shared_cache, std::string_view tag, const CacheKeysType& keys) {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        shared_cache->put(keys[i], {SharedBlockCache::UnifiedCacheItem::GroupBlock{std::string(tag), blocks[i]}}, true);
    }
...
class KVCacheAllocatorCPShardTest: public ::testing::Test {
protected:
    void SetUp() override {
        rtp_llm::initLogger();
        createDevice();
...
TEST_F(KVCacheAllocatorCPShardTest, NullMapperIsPassthrough) {
...
TEST_F(KVCacheAllocatorCPShardTest, ShardedAllocHalvesFullGroup) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
...
TEST_F(KVCacheAllocatorCPShardTest, ReuseHitOnLastRankCanonicalKey) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
    ...
    seedCache(allocator->blockPool("linear"), shared_cache, "linear", CacheKeysType{100});
    ...
    allocator->setCPSlotMapper(std::make_shared<CPSlotMapper>(/*cp_rank=*/1, /*cp_size=*/2, /*block_size=*/4));
    auto result = allocator->malloc(info);
    ASSERT_TRUE(result.success);

    // Stage-4 reuse accounting remains block-based and uses the configured logical block size.
...
TEST_F(KVCacheAllocatorCPShardTest, ShardedAllocSkipsReuseWhenDisabled) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
...
TEST_F(KVCacheAllocatorCPShardTest, InsertIntoCacheUsesCanonicalKeysAndVirtualBlockSize) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
...
TEST_F(KVCacheAllocatorCPShardTest, ShardedAllocCpSize4) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
