...
CacheConfig makeCPHybridConfig() {
    ...
    applyUniformTestBlockCount(config, 32);
...
BatchKVCacheResourcePtr makeBatchRes(int batch_size, const CacheConfig& config, CacheKeysType keys) {
    ...
    for (int b = 0; b < batch_size; ++b) {
        res->setBatchCacheKeys(b, "linear", keys);
        res->setBatchCacheKeys(b, "full", keys);
    }
    return res;
}
...
seedCache(BlockPoolPtr block_pool, SharedBlockCachePtr shared_cache, std::string_view tag, const CacheKeysType& keys) {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        shared_cache->put(
            keys[i], {SharedBlockCache::UnifiedCacheItem::TaggedBlock{std::string(tag), blocks[i]}}, true);
    }
...
TEST_F(HybridKVCacheAllocatorCPShardTest, ShardedAllocHalvesFullGroup) {
    ...
    info.enable_device_cache = false;
    info.reuse_cache         = false;
    setCPMapper(allocator, /*cp_rank=*/0, /*cp_size=*/2);
    auto result = allocator->malloc(info);
    ASSERT_TRUE(result.success);
...
TEST_F(HybridKVCacheAllocatorCPShardTest, FullPromptHitRetainsOneCPLogicalBlockForPrefill) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<HybridPoolKVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
    ...
    auto shared_cache = allocator->sharedBlockCache();
    ASSERT_NE(shared_cache, nullptr);
    const CacheKeysType canonical_keys{101, 103};
    auto                full_blocks = seedCache(allocator->blockPool("full"), shared_cache, "full", canonical_keys);
    seedCache(allocator->blockPool("linear"), shared_cache, "linear", canonical_keys);
    ...
    // A CP page-RR block represents 8 global tokens. A full 16-token hit must
    // back off to that logical boundary so prefill never overwrites a shared block.
    EXPECT_EQ(result.reuse_len, 8);
    ASSERT_EQ(batch_res->blocksNum(0, "full"), 2);
    EXPECT_EQ(batch_res->blocks(0, "full")[0], full_blocks[0]);
    EXPECT_NE(batch_res->blocks(0, "full")[1], full_blocks[1]);
    EXPECT_EQ(shared_cache->matchGroup(103, "full"), full_blocks[1]);
}

TEST_F(HybridKVCacheAllocatorCPShardTest, ConnectorRefPairsCanonicalKeysWithCompactLocalBlocks) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<HybridPoolKVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator->init());
    setCPMapper(allocator, /*cp_rank=*/0, /*cp_size=*/2);

    auto full_blocks = allocator->blockPool("full")->malloc(2);
    ASSERT_EQ(full_blocks.size(), 2u);

    KVCacheResource resource;
    resource.initGroups(config.topologyPtr());
    resource.setCacheKeys("full", CacheKeysType{100, 101, 102, 103});
    // CP page-RR allocation is rank-local compact even though the key namespace
    // is still the full logical sequence.
    resource.mutableBlockIds("full").assign(full_blocks);

    auto selected =
        allocator->incrKVCacheRef(resource, CacheKeysByTag{{"full", CacheKeysType{101, 103}}}, /*is_connector=*/true);
    ASSERT_NE(selected, nullptr);
    EXPECT_EQ(selected->cacheKeys("full"), (CacheKeysType{101, 103}));
    EXPECT_EQ(selected->blocks("full"), full_blocks);
    EXPECT_TRUE(selected->cacheKeysAreCpCanonical("full"));

    allocator->blockPool("full")->requestFree(full_blocks);
    selected.reset();
}

TEST_F(HybridKVCacheAllocatorCPShardTest, ReportedTokenCapacityUsesCPLogicalSpan) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<HybridPoolKVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator->init());
    setCPMapper(allocator, /*cp_rank=*/0, /*cp_size=*/2);

    const auto capacity = allocator->tokenCapacity();
    EXPECT_EQ(capacity.total_tokens, allocator->totalTokensNum());
    EXPECT_EQ(capacity.available_tokens, allocator->availableTokensNum());
}
...
TEST_F(HybridKVCacheAllocatorCPShardTest, ShardedAllocCpSize4) {
    ...
    info.enable_device_cache = false;
    info.reuse_cache         = false;
    setCPMapper(allocator, /*cp_rank=*/2, /*cp_size=*/4);
    auto result = allocator->malloc(info);
    ASSERT_TRUE(result.success);
