...
KVCacheSpecPtr makeFakeSpec(const std::string& tag, CacheGroupType group_type = CacheGroupType::FULL) {
    ...
    desc.tag = tag;
    desc.cache_type =
        group_type == CacheGroupType::FULL ? KVCacheSpecType::MultiHeadAttention : KVCacheSpecType::LinearAttention;
        ...
        if (group_count > 0) {
            ...
            const auto             append_group  = [&](const std::string& tag, CacheGroupType group_type) {
                ...
                groups.push_back(rtp_llm::test::makeTestGroupForConfig(
                    fake_config, makeFakeSpec(tag, group_type), std::move(layer_ids), group_type, tag));
            };
            for (const auto& tag : full_group_tags) {
                append_group(tag, CacheGroupType::FULL);
            }
            for (const auto& tag : other_group_tags) {
                append_group(tag, CacheGroupType::LINEAR);
            }
