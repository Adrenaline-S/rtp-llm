...
bool validateResourceGroupBlocks(const KVCacheResource&       resource,
    ...
    for (const auto& [tag, group] : expected_groups) {
        const auto resource_group = std::find_if(
            resource_groups.begin(), resource_groups.end(), [&tag](const auto& entry) { return entry.tag == tag; });
        if (resource_group == resource_groups.end() || resource_group->block_ids == nullptr) {
            RTP_LLM_LOG_WARNING("resource missing cache group tag [%s]", tag.c_str());
            return false;
        }
        const size_t actual_block_count = resource_group->block_ids->blocksNum();
...
bool DefaultLayerGroupPolicy::genBlockBuffers(const std::vector<std::string>& tags,
    ...
    kv_cache_manager::BlockBuffers generated;
    generated.reserve(block_ids.size());
    for (size_t i = 0; i < block_ids.size(); ++i) {
        const auto& tag      = tags[i];
        const auto  block_id = block_ids[i];
        generated.push_back({});
        const auto& layer_ids          = group_to_layer_ids_.at(tag);
        auto&       iovs               = generated.back().iovs;
    ...
    block_buffers = std::move(generated);
    return true;
}
