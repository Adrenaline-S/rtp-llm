...
// Two-group CP cache config used by the CP-sharded connector tests:
...
//   * SWA, cp_slice=NONE
//                      -> keeps the non-sharded logical coordinate system
//                         (block picked at the original logical key position)
//   * SWA, cp_slice!=NONE
//                      -> CP-compact fixed/state group: its block list is already
//                         in the canonical last-rank namespace, so it is copied
//                         prefix-wise. This is the policy-driven replacement for the
//                         old probe on the deleted per-group row-size vector
//                         (row tokens == seq_size_per_block * cp_size):
//                         CPSlotMapper::layoutForGroup only honours cp_slice for
//                         non-FULL groups while sharded, matching the old
//                         isCpCompactFixedGroup() guard exactly.
// The SWA group's row size is still widened to seq_size_per_block * cp_size so the
// topology keeps documenting the CP-compact row layout, but nothing in the
...
protected:
    void SetUp() override {
        ...
        createDevice();
        coordinator_cache_manager_ = std::make_shared<MockCoordinatorCacheManager>(cache_config_);
        // KVCacheConnectorCoordinator::asyncRead/asyncWrite logs free/available blocks via CoordinatorCacheManager.
        // Remote-connector initialization requires a sole physical pool. Install a minimal one on the mock.
        {
            const size_t block_stride_bytes =
                cache_config_.pagedBlockBudgetBytes() / std::max<size_t>(1, cache_config_.layerCount());
            auto pool_config = BlockPoolConfigHelper::createConfig(
                cache_config_.layerCount(), cache_config_.blockCountBasis(), block_stride_bytes, cache_config_.dtype());
            auto pool = std::make_shared<BlockPool>(pool_config, AllocationType::HOST);
            RTP_LLM_CHECK(pool->init());
            coordinator_cache_manager_->installSoleGroupBlockPoolForTest(pool);
        }
        // KVCacheMemoryConnector::init() validates cache-plan wiring by reading
        // coordinator_cache_manager_->allLayerCacheBase() routing. The gmock allocator materializes no cache
        ...
        ON_CALL(*coordinator_cache_manager_, allLayerCacheBase())
            .WillByDefault(testing::Return(makeTopologyOnlyLayerLayout(cache_config_)));
    ...
    // In production, CoordinatorCacheManager::incrKVCacheRef() typically returns a shared_ptr with a custom deleter
    // that decrements the ref-count via CoordinatorCacheManager::decrKVCacheRef(). Our gmock allocator does not provide
    // that, so tests that validate ref-counting must simulate it explicitly.
    std::shared_ptr<KVCacheResource> makeResourceWithAutoDecr() {
        // IMPORTANT: Use weak_ptr to avoid a reference cycle:
        // coordinator_cache_manager_ (mock) -> EXPECT_CALL action -> returned resource -> deleter ->
        // coordinator_cache_manager_.
        std::weak_ptr<MockCoordinatorCacheManager> allocator_weak = coordinator_cache_manager_;
        ...
        return std::shared_ptr<KVCacheResource>(owned.get(), [owned, allocator_weak](KVCacheResource*) mutable {
            if (auto allocator = allocator_weak.lock()) {
                allocator->decrKVCacheRef(*owned, false);
            }
            owned.reset();
        });
    }
...
private:
    ...
    std::shared_ptr<MockCoordinatorCacheManager> coordinator_cache_manager_;
...
TEST_F(KVCacheConnectorCoordinatorTest, Init_ReturnFalse_WhenMemoryConfigInvalid) {
    ...
    cache_config = TestCacheConfigBuilder::rebuildForTest(std::move(cache_config)).setPagedBlockBudgetBytes(1).build();
...
TEST_F(KVCacheConnectorCoordinatorTest, Init_ReturnTrue_WhenMemoryEnabled_HappyPath_AndStopsUpdateThread) {
    ...
    cache_config =
        TestCacheConfigBuilder::rebuildForTest(std::move(cache_config)).setPagedBlockBudgetBytes(1024).build();
    ...
    // KVCacheConnectorCoordinator::init requires a sole physical pool for remote registration.
    {
        const size_t block_stride_bytes =
            cache_config.pagedBlockBudgetBytes() / std::max<size_t>(1, cache_config.layerCount());
        auto pool_config = BlockPoolConfigHelper::createConfig(
            cache_config.layerCount(), cache_config.blockCountBasis(), block_stride_bytes, cache_config.dtype());
        auto pool = std::make_shared<BlockPool>(pool_config, AllocationType::HOST);
        ASSERT_TRUE(pool->init());
        allocator->installSoleGroupBlockPoolForTest(pool);
    }
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnNull_WhenStop) {
    ...
    auto allocator   = std::make_shared<testing::NiceMock<MockCoordinatorCacheManager>>(cache_config);
    auto coordinator = std::make_shared<KVCacheConnectorCoordinator>(cache_config,
                                                                     KVCacheConfig{},
                                                                     RuntimeConfig{},
                                                                     ParallelismConfig{},
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnNull_WhenConnectorContextNull) {
    coordinator_->coordinator_cache_manager_ = coordinator_cache_manager_;
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnNull_WhenCacheKeysEmpty) {
    ...
    EXPECT_CALL(*coordinator_cache_manager_, incrKVCacheRef(testing::_, testing::_, testing::_)).Times(0);
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnNull_WhenIncrKVCacheRefReturnsNull) {
    ...
    // MockCoordinatorCacheManager doesn't initialize its internal BlockPool unless we set it up explicitly.
    // Without this, coordinator_cache_manager_->freeBlocksNum() / availableBlocksNum() will dereference a null
    // BlockPool and the test process can crash/hang.
    {
        auto pool_config =
            BlockPoolConfigHelper::createConfig(cache_config_.layerCount(),
                                                /*block_num=*/1,
                                                /*block_stride_bytes=*/cache_config_.pagedBlockBudgetBytes(),
    ...
    EXPECT_CALL(*coordinator_cache_manager_, incrKVCacheRef(testing::_, testing::_, testing::_))
        .WillOnce(testing::Return(nullptr));
    EXPECT_CALL(*coordinator_cache_manager_, decrKVCacheRef(testing::_, testing::_)).Times(0);
    EXPECT_CALL(*mock_connector, asyncMatch(testing::_, testing::_)).Times(0);
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnNull_WhenNoMatchContexts) {
    // Use fixture coordinator_cache_manager_/exec_ctx_ so allocator has a valid BlockPool; otherwise coordinator's
    // remote-registration path can otherwise reject a mock without a sole physical pool.
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnContextAndEnqueue_WhenHasMatchContext) {
    // Use fixture coordinator_cache_manager_/exec_ctx_ so allocator has a valid BlockPool.
    auto coordinator = std::make_shared<KVCacheConnectorCoordinator>(cache_config_,
                                                                     KVCacheConfig{},
                                                                     RuntimeConfig{},
                                                                     ParallelismConfig{},
    ...
    EXPECT_CALL(*coordinator_cache_manager_, decrKVCacheRef(testing::_, testing::_)).Times(1);
    coordinator->fused_async_read_context_list_.clear();
    async_ctx.reset();
    coordinator->connectors_.clear();
    mock_connector.reset();
    resource.reset();     // trigger auto-decr while coordinator_cache_manager_ is alive
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncWrite_CPShardedKeepsNonFullGroupsInLogicalCoordinates) {
    CacheConfig cp_cache_config = makeCpFullPlusSwaCacheConfig(/*cp_compact_swa_group=*/false);
    ...
    EXPECT_CALL(*coordinator_cache_manager_, incrKVCacheRef(testing::_, testing::_, testing::Eq(true)))
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncWrite_CPShardedKeepsCompactFixedGroupsInCanonicalCoordinates) {
    CacheConfig cp_cache_config = makeCpFullPlusSwaCacheConfig(/*cp_compact_swa_group=*/true);
...
// Covers the connector-only "dummy tail" trick in CPSlotMapper::projectConnectorResource:
// when the source partial block belongs to a non-last CP rank the selected last-rank key
// is complete, so the original partial key is appended (push_back) and lastBlockAligned is
// forced back to false, which keeps the memory connector's drop-last contract discarding
// the dummy instead of a usable key.
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncWrite_ReturnNull_WhenIncrKVCacheRefReturnsNull) {
    ...
    ctx_resource->initGroups(cache_config_);
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncWrite_ReturnFusedContext_WhenMemoryCacheDisabledInMeta) {
    ...
    EXPECT_CALL(*coordinator_cache_manager_, incrKVCacheRef(testing::_, testing::_, testing::_))
        .WillOnce(testing::Invoke(
            [selected_resource_holder](const KVCacheResource&, const CacheKeysType&, bool is_connector) {
    ...
    coordinator_->connectors_.clear();
    mock_connector.reset();
    selected_resource.reset();  // trigger auto-decr while coordinator_cache_manager_ is alive
}
