...
public:
    explicit LayerBlockConverterImpl(const CoordinatorCacheManagerPtr& coordinator_cache_manager):
    ...
    std::vector<BlockInfo> convertIndexToBufferByTag(
        ...
        return filterValid(coordinator_cache_manager_->convertIndexToBufferByTag(
            layer_id, tag, block_id, partition_count, partition_id));
    }
