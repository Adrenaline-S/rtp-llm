...
static std::string requireSingleCacheTag(const CacheConfig& config) {
    RTP_LLM_CHECK_WITH_INFO(config.topology().groups().size() == 1,
                            "legacy untagged memory copy requires one global cache group, got %zu",
                            config.topology().groups().size());
    std::string tag;
...
static const GroupBase& requireSingleReusableGroup(const CacheConfig& config) {
    const GroupBase* selected = nullptr;
    for (const auto& group : config.topology().groups()) {
        if (!group.policy.enable_prefix_reuse) {
            continue;
        }
        RTP_LLM_CHECK_WITH_INFO(selected == nullptr,
                                "native memory wire requires exactly one reusable group, got more than one");
        selected = &group;
    }
    RTP_LLM_CHECK_WITH_INFO(selected != nullptr, "native memory wire requires exactly one reusable group, got zero");
    return *selected;
}
...
static bool copyItemIsComplete(const MemoryOperationRequestPB::CopyItem& item) {
    switch (item.cache_block_kind()) {
        case MemoryOperationRequestPB::INCOMPLETE_KV:
            return false;
        case MemoryOperationRequestPB::COMPLETE_KV:
            return true;
        case MemoryOperationRequestPB::COMPRESSED_KV:
        case MemoryOperationRequestPB::STATE_SWA_KV:
...
static CacheBlockKind copyItemBlockKind(const MemoryOperationRequestPB::CopyItem& item) {
    switch (item.cache_block_kind()) {
        case MemoryOperationRequestPB::COMPRESSED_KV:
...
KVCacheMemoryConnector::KVCacheMemoryConnector(const CacheConfig&                       cache_config,
    ...
    tp_addrs_(tp_addrs),
    layer_tag_slots_(buildLayerTagSlots(cache_config)),
...
void KVCacheMemoryConnector::checkLayerBlockStrideBytes() const {
    ...
    for (const auto& slot : slots) {
        RTP_LLM_CHECK_WITH_INFO(slot.stride_bytes > 0,
                                "invalid block stride bytes at layer=%d tag=%s: %zu",
                                slot.layer_id,
                                slot.tag.c_str(),
                                slot.stride_bytes);
    }
...
bool KVCacheMemoryConnector::isFullOnlySlot(const LayerTagSlot& slot) const {
    return slot.group_type == CacheGroupType::FULL;
}
...
void KVCacheMemoryConnector::initBlockPool() {
    ...
    for (const auto& slot : slots) {
        ...
        if (slot.block_kind == CacheBlockKind::COMPRESSED_KV) {
            compressed_size += slot.stride_bytes;
        } else if (slot.block_kind == CacheBlockKind::STATE_SWA_KV) {
            state_swa_size += slot.stride_bytes;
        }
    }
    RTP_LLM_CHECK_WITH_INFO(total_block_size > 0, "block size is invalid: %zu", total_block_size);

    const bool use_typed_memory_layout = prefix_tree_supported;
    if (!use_prefix_tree_memory_cache_ && !use_typed_memory_layout) {
        ...
        RTP_LLM_LOG_INFO("legacy memory pool init: layer_block_size=%zu", layer_block_size);
        return;
    }
...
bool KVCacheMemoryConnector::copyItemUsesLayerBlocks(const NormalizedCopyItem& item) const {
    return item.layer_kind && cache_config_.topology().hasSingleGlobalGroup()
           && item.gpu_blocks.size() == cache_config_.layer_all_num;
}
...
void KVCacheMemoryConnector::initDiskBlockPools() {
    ...
    if (usePrefixTreeMemoryCache()) {
        ...
        if (kv_cache_config_.prefix_tree_memory_state_swa_pool_ratio > 0) {
            const size_t state_bytes = total_disk_bytes
                                       * static_cast<size_t>(kv_cache_config_.prefix_tree_memory_state_swa_pool_ratio)
                                       / 100ULL;
        ...
        RTP_LLM_CHECK_WITH_INFO(compressed_slots > 0 && state_swa_slots > 0,
                                ...
                                kv_cache_config_.prefix_tree_memory_state_swa_pool_ratio);
        complete_disk_pool_ =
            make_disk_pool(CacheBlockKind::COMPRESSED_KV, compressed_slots * compressed_stride, compressed_block_size_);
...
std::vector<KVCacheMemoryConnector::LayerTagSlot>
KVCacheMemoryConnector::buildLayerTagSlots(const CacheConfig& cache_config) {
    ...
    for (size_t layer = 0; layer < layer_num; ++layer) {
        for (const auto& tag : cache_config.topology().layer(static_cast<int>(layer)).group_tags) {
            ...
            CacheBlockKind block_kind = policy.group_type == CacheGroupType::FULL ? CacheBlockKind::COMPRESSED_KV :
                                                                                    CacheBlockKind::STATE_SWA_KV;
            if (group.spec && group.spec->type == KVCacheSpecType::OpaqueKV) {
                block_kind = CacheBlockKind::COMPRESSED_KV;
...
const std::vector<KVCacheMemoryConnector::LayerTagSlot>& KVCacheMemoryConnector::layerTagSlots() const {
    return layer_tag_slots_;
}
...
bool KVCacheMemoryConnector::supportsTypedPrefixCacheLayout(const std::vector<LayerTagSlot>& slots) const {
    ...
    for (const auto& group : cache_config_.topology().groups()) {
        const auto& spec = cache_config_.specForGroup(group.tag);
        if (spec == nullptr) {
            return false;
        }
        if (cache_config_.kvBlockStrideBytesForGroup(group.tag) + cache_config_.kvScaleStrideBytesForGroup(group.tag)
            == 0) {
            return false;
        }
    ...
    for (size_t layer = 0; layer < cache_config_.layer_all_num; ++layer) {
        if (cache_config_.topology().layer(static_cast<int>(layer)).group_tags.empty()) {
            return false;
        }
    }

    for (const auto& slot : slots) {
        if (slot.layer_id < 0 || !cache_config_.topology().groupForLayer(slot.layer_id, slot.tag).spec) {
            return false;
        }
        const auto group_stride =
            cache_config_.kvBlockStrideBytesForGroup(slot.tag) + cache_config_.kvScaleStrideBytesForGroup(slot.tag);
...
std::shared_ptr<AsyncMatchContext> KVCacheMemoryConnector::asyncMatch(const RequestPrefixMatchView& view,
    ...
    if (!usePrefixTreeMemoryCache()) {
        const auto span = view.matchSpanTokens();
        if (view.reuseTokens() % span != 0) {
            return nullptr;
        }
    ...
    size_t matched_tokens = view.reuseTokens();
    {
        std::lock_guard<std::mutex> lock(endpoint_commit_mutex_);
        const size_t                span = view.matchSpanTokens();
        for (size_t endpoint = matched_tokens + span; endpoint <= view.matchLimitTokens(); endpoint += span) {
            const size_t key_index = endpoint / span - 1;
            if (key_index >= view.keys().size()
                || endpoint_commits_.find({view.keys()[key_index], endpoint}) == endpoint_commits_.end()) {
                break;
            }
            matched_tokens = endpoint;
        }
...
bool KVCacheMemoryConnector::gpuBlocksAllValid(const LayerAttnBlockIds&         layer_attn_block_ids,
    ...
    for (const auto& slot : slots) {
        ...
        if (layer >= layer_attn_block_ids.size()
            || layer_attn_block_ids[layer].find(slot.tag) == layer_attn_block_ids[layer].end()
            || layer_attn_block_ids[layer].at(slot.tag) == nullptr) {
            return false;
        }
        const auto& blocks = layer_attn_block_ids[layer].at(slot.tag)->blocks();
...
std::vector<uint8_t> KVCacheMemoryConnector::prefixSlotValidMask(const LayerAttnBlockIds&         layer_attn_block_ids,
                                                                 const std::vector<LayerTagSlot>& slots,
                                                                 size_t                           key_index,
                                                                 CacheBlockKind                   kind) const {
...
size_t KVCacheMemoryConnector::prefixKindBlockSize(CacheBlockKind kind, const std::vector<LayerTagSlot>& slots) const {
    size_t bytes = 0;
    for (const auto& slot : slots) {
        if (slot.block_kind == kind) {
...
std::shared_ptr<AsyncContext> KVCacheMemoryConnector::asyncRead(const std::shared_ptr<KVCacheResource>&   resource,
    ...
    if (!memory_match_context || memory_match_context->plannedStartToken() != start_token
        ...
        RTP_LLM_LOG_WARNING("memory read requires the exact matched endpoint token range");
        return nullptr;
    }
    const auto&               slots = layerTagSlots();
    ...
    if (!usePrefixTreeMemoryCache()) {
        ...
        const auto span        = resource->requestPrefix().matchSpanTokens();
        if (start_token % span != 0 || token_count % span != 0) {
            RTP_LLM_LOG_WARNING("memory connector requires aligned request keys");
            return nullptr;
        }
        const auto  begin      = start_token / span;
        const auto  count      = token_count / span;
        const auto& cache_keys = multi_group ? resource->requestPrefix().keys() :
                                               resource->cacheKeys(requireSingleReusableGroup(cache_config_).tag);
        ...
        copy_plan = buildEndpointCopyPlanForRead(*resource, layer_attn_block_ids, slots, start_token, token_count);
    }
    if (!copy_plan || copy_plan->copy_infos.empty()) {
        if (usePrefixTreeMemoryCache()) {
            eraseEndpointRange(resource->requestPrefix(), start_token, start_token + token_count);
        }
        return nullptr;
    }

    autil::ScopedTime2 timer;
    const bool         request_endpoint_control = usePrefixTreeMemoryCache();
    auto read_done = [resource, copy_plan, request_endpoint_control, start_token, token_count, timer, this](
                         bool success) mutable {
        if (success) {
            resource->setMemoryReuseTokenNum(token_count);
        ...
        if (success && request_endpoint_control) {
            eraseEndpointRange(resource->requestPrefix(), start_token, start_token + token_count);
        }
...
KVCacheMemoryConnector::buildCopyPlanForRead(const CacheKeysType&             cache_keys,
    ...
    for (int i = start_index; i < start_index + read_num; ++i) {
        ...
        for (const auto& slot : slots) {
            const auto layer = static_cast<size_t>(slot.layer_id);
            copy_info.gpu_blocks.push_back(layer_attn_block_ids.at(layer).at(slot.tag)->blocks().at(i));
        }
...
std::shared_ptr<AsyncContext> KVCacheMemoryConnector::asyncWrite(const std::shared_ptr<KVCacheResource>& resource,
    ...
    if (usePrefixTreeMemoryCache()) {
        ...
        auto copy_plan = buildEndpointCopyPlanForWrite(*resource, layer_attn_block_ids, slots, no_need_write);
        if (!copy_plan || copy_plan->copy_infos.empty()) {
            if (no_need_write) {
                publishReadyEndpoints(*resource);
            }
            return nullptr;
        }
        autil::ScopedTime2 timer;
        auto               write_done = [copy_plan, resource_copy = resource, timer, this](bool success) mutable {
            if (success) {
                for (auto& copy_info : copy_plan->copy_infos) {
                    putPrefixToCache(copy_info, copy_info.dependency, layer_tag_slots_);
                }
                publishReadyEndpoints(*resource_copy);
            }
            const auto item_count = copy_plan->copy_infos.size();
            resource_copy.reset();
            copy_plan.reset();
    ...
    const auto* native_group       = legacy_multi_group ? nullptr : &requireSingleReusableGroup(cache_config_);
    const auto& cache_keys =
        legacy_multi_group ? resource->requestPrefix().keys() : resource->cacheKeys(native_group->tag);
    const auto cache_keys_size =
        legacy_multi_group ?
            cache_keys.size() :
            (cache_keys.empty() ?
                 0 :
                 (resource->lastBlockAligned(native_group->tag) ? cache_keys.size() : cache_keys.size() - 1));
    ...
    if (!use_layer_blocks) {
        ...
        if (!checkLayerBlocks(layer_block_ids, cache_keys_size)) {
            RTP_LLM_LOG_WARNING("async write failed, invalid layer_block_ids, cache_keys_size=%zu resource_keys=%zu",
                                cache_keys_size,
                                cache_keys.size());
            reportWriteMetrics(false, timer.done_us(), cache_keys_size, 0);
            return nullptr;
    ...
    auto copy_plan =
        use_layer_blocks ?
            ...
            buildCopyPlanForWrite(cache_keys,
                                  layer_attn_block_ids,
                                  slots,
                                  mem_matched_num,
...
KVCacheMemoryConnector::buildCopyPlanForWrite(const CacheKeysType&             cache_keys,
    ...
    for (int i = start_index; i < start_index + write_num; ++i) {
        ...
        for (const auto& slot : slots) {
            const auto layer         = static_cast<size_t>(slot.layer_id);
            const int  gpu_block_idx = layer_attn_block_ids.at(layer).at(slot.tag)->blocks().at(i);
        ...
        CopyInfoPerKey copy_info;
        copy_info.cache_key   = cache_key;
        copy_info.kind        = blockKindFromComplete(is_complete);
...
KVCacheMemoryConnector::buildCopyPlanForWrite(const CacheKeysType& cache_keys,
    ...
    for (int i = start_index; i < start_index + write_num; ++i) {
        const auto                cache_key = cache_keys.at(i);
        std::vector<BlockIdxType> gpu_blocks;
        gpu_blocks.reserve(cache_config_.layer_all_num);
        size_t null_block_num = 0;
        for (size_t layer = 0; layer < cache_config_.layer_all_num; ++layer) {
...
std::optional<KVCacheMemoryConnector::NativeItemProjection> KVCacheMemoryConnector::nativeItemForOrdinal(
    const KVCacheResource& resource, std::string_view tag, size_t physical_ordinal) const {
    ...
    const auto  dependency_it =
        std::find_if(dependencies.begin(), dependencies.end(), [&](const BlockDependency& dependency) {
            return dependency.ordinal == physical_ordinal;
        });
    if (dependency_it == dependencies.end()) {
        return std::nullopt;
    }
    const size_t local_index = static_cast<size_t>(std::distance(dependencies.begin(), dependency_it));
    const auto&  native_keys = resource.cacheKeys(tag);
    if (local_index >= native_keys.size()) {
        return std::nullopt;
    }
    return NativeItemProjection{local_index, native_keys[local_index], *dependency_it};
}
...
std::vector<uint8_t> KVCacheMemoryConnector::nativeItemSlotValidMask(const KVCacheResource&   resource,
    ...
    if ((kind == CacheBlockKind::COMPRESSED_KV) != full_item) {
        return mask;
    }
    for (size_t slot_idx = 0; slot_idx < slots.size(); ++slot_idx) {
        const auto& slot = slots[slot_idx];
        if (slot.tag != tag || slot.block_kind != kind) {
            continue;
        }
        ...
        if (tag_it == layer_attn_block_ids[layer].end() || tag_it->second == nullptr
            || local_index >= tag_it->second->blocks().size()) {
            continue;
        }
        const auto block = tag_it->second->blocks()[local_index];
        if (gpu_blocks != nullptr) {
...
std::shared_ptr<KVCacheMemoryConnector::CopyPlan>
KVCacheMemoryConnector::buildEndpointCopyPlanForRead(const KVCacheResource&           resource,
                                                     const LayerAttnBlockIds&         layer_attn_block_ids,
                                                     const std::vector<LayerTagSlot>& slots,
                                                     size_t                           start_token,
                                                     size_t                           token_count) {
    ...
    for (const auto& group : cache_config_.topology().groups()) {
        ...
        const bool   full_item = group.policy.group_type == CacheGroupType::FULL;
        const size_t begin     = full_item ? start_token : 0;
        const int    cp_size   = cpSizeForMetrics();
        const int    cp_rank =
            cp_size > 1 && parallelism_config_.tp_size > 1 ? static_cast<int>(parallelism_config_.tp_rank) : 0;
        const auto selection = projectTokenRangeForGroup(group, begin, end_token, true, cp_rank, cp_size);
        for (const auto ordinal : selection.owned_global_positions) {
            const auto native_item = nativeItemForOrdinal(resource, group.tag, ordinal);
            if (!native_item.has_value()) {
                success = false;
                break;
            }
            const auto                kind = full_item ? CacheBlockKind::COMPRESSED_KV : CacheBlockKind::STATE_SWA_KV;
            std::vector<BlockIdxType> gpu_blocks;
            const auto                required_mask = nativeItemSlotValidMask(resource,
                                                               group.tag,
                                                               native_item->local_index,
                                                               full_item,
                                                               layer_attn_block_ids,
                                                               slots,
                                                               kind,
            ...
            const auto match = prefix_block_cache_->matchAndMarkInFlight(native_item->cache_key, kind, required_mask);
            if (!match.found) {
                success = false;
                break;
            }
            const auto release_match = [&]() {
                const auto retired = prefix_block_cache_->releaseInFlight(native_item->cache_key,
                                                                          kind,
                                                                          match.backing_type,
                                                                          match.block_index,
                                                                          match.disk_slot,
            ...
            CopyInfoPerKey info;
            info.cache_key       = native_item->cache_key;
            info.kind            = kind;
...
std::shared_ptr<KVCacheMemoryConnector::CopyPlan>
KVCacheMemoryConnector::buildEndpointCopyPlanForWrite(const KVCacheResource&           resource,
                                                      const LayerAttnBlockIds&         layer_attn_block_ids,
                                                      const std::vector<LayerTagSlot>& slots,
                                                      bool&                            no_need_write) {
    std::vector<CopyInfoPerKey>                               copy_infos;
    std::map<std::pair<CacheKeyType, CacheBlockKind>, size_t> planned;
    const auto&                                               prefix            = resource.requestPrefix();
    size_t                                                    previous_endpoint = 0;
    for (size_t endpoint = prefix.matchSpanTokens(); endpoint <= prefix.writeLimitTokens();
         endpoint += prefix.matchSpanTokens()) {
        for (const auto& group : cache_config_.topology().groups()) {
            if (!group.policy.enable_prefix_reuse) {
                continue;
            }
            const bool   full_item = group.policy.group_type == CacheGroupType::FULL;
            const size_t begin     = full_item ? previous_endpoint : 0;
            const int    cp_size   = cpSizeForMetrics();
            const int    cp_rank =
                cp_size > 1 && parallelism_config_.tp_size > 1 ? static_cast<int>(parallelism_config_.tp_rank) : 0;
            const auto selection = projectTokenRangeForGroup(group, begin, endpoint, true, cp_rank, cp_size);
            for (const auto ordinal : selection.owned_global_positions) {
                const auto native_item = nativeItemForOrdinal(resource, group.tag, ordinal);
                if (!native_item.has_value()) {
                    return nullptr;
                ...
                const auto kind = full_item ? CacheBlockKind::COMPRESSED_KV : CacheBlockKind::STATE_SWA_KV;
                std::vector<BlockIdxType> gpu_blocks;
                const auto                slot_mask = nativeItemSlotValidMask(resource,
                                                               group.tag,
                                                               native_item->local_index,
                                                               full_item,
                                                               layer_attn_block_ids,
                ...
                const auto plan_key   = std::make_pair(native_item->cache_key, kind);
                const auto planned_it = planned.find(plan_key);
                if (planned_it != planned.end()) {
                    auto& existing = copy_infos[planned_it->second];
                    if (existing.dependency.has_parent != native_item->dependency.has_parent
                        || existing.dependency.parent_key != native_item->dependency.parent_key
                        || existing.dependency.ordinal != native_item->dependency.ordinal) {
                        return nullptr;
                    }
                    for (size_t slot_idx = 0; slot_idx < slot_mask.size(); ++slot_idx) {
                        if (slot_mask[slot_idx] == 0) {
                            continue;
                        }
                        if (existing.slot_valid_mask[slot_idx] != 0
                            && existing.gpu_blocks[slot_idx] != gpu_blocks[slot_idx]) {
                            return nullptr;
                        }
                        existing.slot_valid_mask[slot_idx] = 1;
                        existing.gpu_blocks[slot_idx]      = gpu_blocks[slot_idx];
                    }
                    continue;
                }
                ...
                info.gpu_blocks      = std::move(gpu_blocks);
                info.dependency      = native_item->dependency;
                planned.emplace(plan_key, copy_infos.size());
                copy_infos.push_back(std::move(info));
            }
        }
        previous_endpoint = endpoint;
    }
    copy_infos.erase(std::remove_if(copy_infos.begin(),
                                    copy_infos.end(),
                                    [&](const CopyInfoPerKey& info) {
                                        return prefix_block_cache_->contains(
                                            info.cache_key, info.kind, info.slot_valid_mask);
                                    }),
                     copy_infos.end());
    no_need_write = copy_infos.empty();
    if (no_need_write) {
...
bool KVCacheMemoryConnector::endpointBackingsComplete(const KVCacheResource&           resource,
    ...
    for (const auto& group : cache_config_.topology().groups()) {
        ...
        for (const auto ordinal : selection.owned_global_positions) {
            ...
            const auto kind = full_item ? CacheBlockKind::COMPRESSED_KV : CacheBlockKind::STATE_SWA_KV;
            const auto mask = nativeItemSlotValidMask(
                resource, group.tag, native_item->local_index, full_item, layer_attn_block_ids, slots, kind, nullptr);
            if (std::none_of(mask.begin(), mask.end(), [](uint8_t value) { return value != 0; })
                || !prefix_block_cache_->contains(native_item->cache_key, kind, mask)) {
                return false;
            }
        }
    }
...
void KVCacheMemoryConnector::publishReadyEndpoints(const KVCacheResource& resource) {
    const auto&                                      prefix = resource.requestPrefix();
    const auto                                       span   = prefix.matchSpanTokens();
    const auto                                       blocks = resourceLayerRegionBlocks(resource, layerTagSlots());
    std::vector<std::pair<RequestPrefixKey, size_t>> ready;
    size_t                                           previous_endpoint = 0;
    for (size_t endpoint = span; endpoint <= prefix.writeLimitTokens(); endpoint += span) {
        const size_t key_index = endpoint / span - 1;
        if (key_index >= prefix.keys().size()
            || !endpointBackingsComplete(resource, previous_endpoint, endpoint, blocks, layerTagSlots())) {
            break;
        }
        ready.emplace_back(prefix.keys()[key_index], endpoint);
        previous_endpoint = endpoint;
    }
    std::lock_guard<std::mutex> lock(endpoint_commit_mutex_);
    endpoint_commits_.insert(ready.begin(), ready.end());
}

void KVCacheMemoryConnector::eraseEndpointRange(const RequestPrefixResource& prefix,
                                                size_t                       start_token,
                                                size_t                       end_token) {
    const size_t                span = prefix.matchSpanTokens();
    std::lock_guard<std::mutex> lock(endpoint_commit_mutex_);
    for (size_t endpoint = start_token + span; endpoint <= end_token; endpoint += span) {
        const size_t key_index = endpoint / span - 1;
        if (key_index < prefix.keys().size()) {
            endpoint_commits_.erase({prefix.keys()[key_index], endpoint});
        }
    }
}

void KVCacheMemoryConnector::clearEndpointCommits() {
    std::lock_guard<std::mutex> lock(endpoint_commit_mutex_);
    endpoint_commits_.clear();
}
...
KVCacheMemoryConnector::sendCopyPlan(const std::shared_ptr<CopyPlan>& copy_plan) const {
    ...
    for (const auto& copy_info : copy_plan->copy_infos) {
        ...
        } else if (copy_info.kind == CacheBlockKind::COMPLETE) {
            item->set_cache_block_kind(MemoryOperationRequestPB::COMPLETE_KV);
        } else if (copy_info.kind == CacheBlockKind::INCOMPLETE) {
            item->set_cache_block_kind(MemoryOperationRequestPB::INCOMPLETE_KV);
        }
        if (copy_info.backing_type == CacheBackingType::DISK) {
        ...
        if (copy_info.gpu_blocks.size() == slots.size()) {
            for (size_t i = 0; i < slots.size(); ++i) {
                ...
                tagged_block->set_block_id(copy_info.gpu_blocks[i]);
            }
        } else {
            RTP_LLM_CHECK_WITH_INFO(copy_info.gpu_blocks.size() == cache_config_.layer_all_num
                                        && cache_config_.topology().hasSingleGlobalGroup(),
                                    "cannot encode memory copy blocks by tag: blocks=%zu slots=%zu layers=%u",
                                    copy_info.gpu_blocks.size(),
                                    slots.size(),
                                    cache_config_.layer_all_num);
            for (size_t layer_id = 0; layer_id < copy_info.gpu_blocks.size(); ++layer_id) {
                auto* tagged_block = item->add_tagged_gpu_blocks();
                tagged_block->set_layer_id(static_cast<int32_t>(layer_id));
                tagged_block->set_tag(requireSingleCacheTag(cache_config_));
                tagged_block->set_block_id(copy_info.gpu_blocks[layer_id]);
            }
        }
...
bool KVCacheMemoryConnector::copyCache(const MemoryOperationRequestPB& wire_request,
                                       MemoryOperationResponsePB&      response) {
    RTP_LLM_PROFILE_FUNCTION();
    autil::ScopedTime2 timer;
    CopyDirection      copy_direction = CopyDirection::D2H;
    if (wire_request.copy_direction() == MemoryOperationRequestPB::H2D) {
    ...
    bool        has_layer_block_items = false;

    if (wire_request.copy_items_size() == 0) {
    ...
    NormalizedCopyItems items;
    items.reserve(wire_request.copy_items_size());
    for (const auto& wire_item : wire_request.copy_items()) {
        if (!validateCopyItemBacking(wire_item)) {
            response.set_success(false);
            reportCopyMetrics(false, timer.done_us(), copy_direction);
            return false;
        }
        items.push_back(normalizeCopyItem(wire_item, slots));
        const auto& item      = items.back();
        has_prefix_items      = has_prefix_items || !item.layer_kind;
        has_layer_block_items = has_layer_block_items || copyItemUsesLayerBlocks(item);
    ...
    if (has_prefix_items) {
        const bool success = copyPrefixMemoryItems(items, copy_direction, slots);
    ...
    if (has_disk_items) {
        bool success = true;
        if (has_memory_items) {
            success = copyMemoryItemsGeneric(items, copy_direction, slots);
        }
        if (success) {
            success = copyDiskItems(items, copy_direction, slots);
        }
        response.set_success(success);
        reportCopyMetrics(success, timer.done_us(), copy_direction);
        reportDiskCopyMetrics(success, timer.done_us(), copy_direction);
...
KVCacheMemoryConnector::normalizeCopyItemGpuBlocks(const MemoryOperationRequestPB::CopyItem& item,
    ...
    gpu_blocks.reserve(slots.size());
    for (const auto& slot : slots) {
        const auto it = tagged_blocks.find({slot.layer_id, slot.tag});
        RTP_LLM_CHECK_WITH_INFO(it != tagged_blocks.end(),
                                "memory copy request missing local layer=%d tag=%s",
                                slot.layer_id,
                                slot.tag.c_str());
        gpu_blocks.push_back(it->second);
    }
    RTP_LLM_CHECK_WITH_INFO(tagged_blocks.size() == slots.size(),
                            "memory copy tag topology mismatch: peer=%zu local=%zu",
                            tagged_blocks.size(),
                            slots.size());
    return gpu_blocks;
}

KVCacheMemoryConnector::NormalizedCopyItem
KVCacheMemoryConnector::normalizeCopyItem(const MemoryOperationRequestPB::CopyItem& item,
    ...
    normalized.gpu_blocks  = normalizeCopyItemGpuBlocks(item, slots);
    ...
    normalized.layer_kind  = item.cache_block_kind() == MemoryOperationRequestPB::COMPLETE_KV
                            || item.cache_block_kind() == MemoryOperationRequestPB::INCOMPLETE_KV;
    normalized.backing_type =
        item.backing_type() == MemoryOperationRequestPB::DISK ? CacheBackingType::DISK : CacheBackingType::MEMORY;
    normalized.disk_slot =
        item.disk_slot_presence_case() == MemoryOperationRequestPB::CopyItem::kDiskSlot ? item.disk_slot() : -1;
    normalized.kind = copyItemBlockKind(item);
    normalized.has_src_mem_block =
        item.src_mem_block_presence_case() == MemoryOperationRequestPB::CopyItem::kSrcMemBlock;
    normalized.src_mem_block =
        normalized.has_src_mem_block ? static_cast<BlockIdxType>(item.src_mem_block()) : NULL_BLOCK_IDX;
    normalized.slot_valid_mask.reserve(item.slot_valid_mask_size());
    for (const auto valid : item.slot_valid_mask()) {
        normalized.slot_valid_mask.push_back(valid != 0);
    }
    normalized.src_backing_type =
        item.src_backing_type() == MemoryOperationRequestPB::DISK ? CacheBackingType::DISK : CacheBackingType::MEMORY;
    normalized.has_src_disk_slot =
        item.src_disk_slot_presence_case() == MemoryOperationRequestPB::CopyItem::kSrcDiskSlot;
    normalized.src_disk_slot = normalized.has_src_disk_slot ? item.src_disk_slot() : -1;
    return normalized;
}
...
bool KVCacheMemoryConnector::validateCopyItemBacking(const MemoryOperationRequestPB::CopyItem& item) const {
    ...
    CacheBlockKind kind = copyItemBlockKind(item);
    if (item.backing_type() == MemoryOperationRequestPB::DISK) {
        ...
        if (!disk_pool || !disk_pool->validSlot(item.disk_slot())) {
            RTP_LLM_LOG_WARNING("disk copy item slot is out of range, slot=%d, is_complete=%d",
                                item.disk_slot(),
                                copyItemIsComplete(item));
            return false;
        }
    }

    if (has_src_disk_slot || item.src_backing_type() == MemoryOperationRequestPB::DISK) {
        if (item.src_backing_type() != MemoryOperationRequestPB::DISK || !has_src_disk_slot || item.src_disk_slot() < 0
            || has_src_mem_block) {
...
bool KVCacheMemoryConnector::copyMemoryItemsGeneric(const NormalizedCopyItems&       items,
    ...
    for (const auto& item : items) {
        ...
        const bool prepared =
            copyItemUsesLayerBlocks(item) ?
                prepareLayerCopyBuffers(item.mem_block, item.gpu_blocks, direction, dst_buffers, src_buffers) :
                prepareCopyBuffers(
                    item.mem_block, item.gpu_blocks, direction, item.is_complete, dst_buffers, src_buffers);
        if (!prepared) {
...
bool KVCacheMemoryConnector::copyPrefixMemoryItems(const NormalizedCopyItems&       items,
    ...
    for (const auto& item : items) {
        if (item.backing_type == CacheBackingType::DISK || item.src_backing_type == CacheBackingType::DISK
            || item.has_src_disk_slot) {
            has_disk_item = true;
            break;
        }
    }
    ...
    for (const auto& item : items) {
        const bool is_prefix_kind =
            item.kind == CacheBlockKind::COMPRESSED_KV || item.kind == CacheBlockKind::STATE_SWA_KV;
        if (!is_prefix_kind) {
            return false;
        }
        ...
        auto       pool      = memoryPoolFor(kind);
        auto       disk_pool = diskPoolFor(kind);
        if (item.gpu_blocks.size() != slots.size()) {
            return false;
        }
        ...
        if (item.backing_type == CacheBackingType::MEMORY) {
            ...
            auto mem_buffers = pool->convertIndexToBuffer(/*layer_id=*/0, item.mem_block);
        ...
        if (direction == CopyDirection::D2H && (item.has_src_mem_block || item.has_src_disk_slot)) {
            const bool src_is_disk = item.src_backing_type == CacheBackingType::DISK || item.has_src_disk_slot;
            if (src_is_disk) {
                if (!disk_pool || raw_buffer == nullptr || !item.has_src_disk_slot
                ...
                if (isNullBlockIdx(item.src_mem_block)) {
                    return false;
                }
                const auto src_mem_buffers = pool->convertIndexToBuffer(/*layer_id=*/0, item.src_mem_block);
        ...
        for (size_t slot_idx = 0; slot_idx < slots.size(); ++slot_idx) {
            ...
            const auto gpu_block = item.gpu_blocks[slot_idx];
            if (!isUsableBlockIdx(gpu_block)) {
                byte_off += slot.stride_bytes;
                continue;
            }
            const auto gpu_buffers      = allocator_->convertIndexToBuffer(slot.layer_id, slot.tag, gpu_block);
...
bool KVCacheMemoryConnector::copyDiskItems(const NormalizedCopyItems&       items,
                                           CopyDirection                    direction,
                                           const std::vector<LayerTagSlot>& slots) {
    void*        raw_buffer   = nullptr;
    const size_t stride_bytes = maxDiskSlotStrideBytes();
...
bool KVCacheMemoryConnector::copyDiskItem(const NormalizedCopyItem&        item,
                                          CopyDirection                    direction,
                                          const std::vector<LayerTagSlot>& slots,
                                          void*                            raw_buffer) {
    auto       disk_pool         = diskPoolFor(item.kind);
    const bool uses_layer_blocks = copyItemUsesLayerBlocks(item);
    if (!disk_pool || (!uses_layer_blocks && item.gpu_blocks.size() != slots.size())) {
        return false;
    }
    ...
    if (copyItemUsesLayerBlocks(item)) {
        const auto tag      = requireSingleCacheTag(cache_config_);
        size_t     byte_off = 0;
        for (size_t layer = 0; layer < cache_config_.layer_all_num; ++layer) {
            ...
            const auto gpu_buffers      = allocator_->convertIndexToBuffer(layer, tag, gpu_block);
            size_t     within_layer_off = 0;
            for (const auto& gpu_buffer : gpu_buffers) {
...
bool KVCacheMemoryConnector::prepareLayerCopyBuffers(BlockIdxType                     mem_block,
                                                     const std::vector<BlockIdxType>& gpu_blocks,
                                                     CopyDirection                    direction,
                                                     std::vector<torch::Tensor>&      dst,
                                                     std::vector<torch::Tensor>&      src) {
    RTP_LLM_CHECK_WITH_INFO(cache_config_.topology().hasSingleGlobalGroup(),
                            "layer copy requires exactly one cache group per layer");
    ...
    if (mem_buffers.empty()) {
        RTP_LLM_LOG_WARNING("prepare layer copy buffers failed, mem buffers are empty, block=%d, direction=%s",
                            mem_block,
                            direction == CopyDirection::H2D ? "H2D" : "D2H");
        return false;
    }
    ...
    for (size_t layer = 0; layer < layer_num; ++layer) {
        ...
        for (const auto& gpu_buffer : gpu_buffers) {
            if (within_layer_off + gpu_buffer.size_bytes > layer_stride) {
                RTP_LLM_LOG_WARNING("prepare layer copy buffers failed, gpu buffer overflow: "
        ...
        for (const auto& gpu_buffer : gpu_buffers) {
            if (within_layer_off + gpu_buffer.size_bytes > layer_stride) {
                RTP_LLM_LOG_WARNING("prepare copy buffers failed, gpu buffer overflow: "
                                    "layer=%d tag=%s byte_off=%zu within_layer_off=%zu gpu_buffer_size=%zu",
                                    slot.layer_id,
                                    slot.tag.c_str(),
                                    byte_off,
                                    within_layer_off,
...
KVCacheMemoryConnector::LayerBlockIds
KVCacheMemoryConnector::resourceLayerBlocks(const KVCacheResource& resource) const {
    const auto    tag = requireSingleCacheTag(cache_config_);
    LayerBlockIds blocks;
    blocks.reserve(cache_config_.layer_all_num);
    for (size_t layer = 0; layer < cache_config_.layer_all_num; ++layer) {
        blocks.push_back(&resource.blockIdsForLayer(static_cast<int>(layer), tag));
    }
    return blocks;
}

KVCacheMemoryConnector::LayerAttnBlockIds
...
bool KVCacheMemoryConnector::checkLayerRegionBlocks(const LayerAttnBlockIds&         layer_attn_block_ids,
    ...
    for (const auto& slot : slots) {
        ...
        if (layer >= layer_attn_block_ids.size()
            ...
            || layer_attn_block_ids[layer].at(slot.tag) == nullptr) {
            RTP_LLM_LOG_WARNING(
                "check layer-group blocks failed, missing slot layer=%d tag=%s", slot.layer_id, slot.tag.c_str());
            return false;
        }
        if (layer_attn_block_ids[layer].at(slot.tag)->blocksNum() < required_len) {
            RTP_LLM_LOG_WARNING(
                "check layer-group blocks failed, blocksNum is less than required_len, layer=%d tag=%s blocksNum=%zu required_len=%zu",
                slot.layer_id,
                slot.tag.c_str(),
                layer_attn_block_ids[layer].at(slot.tag)->blocksNum(),
                required_len);
            return false;
        }
    }
...
std::string KVCacheMemoryConnector::blockPoolDebugString() const {
    ...
    if (usePrefixTreeMemoryCache()) {
        oss << "compressed pool: total=" << compressed_pool_->totalBlocksNum()
            << " free=" << compressed_pool_->freeBlocksNum() << " available=" << compressed_pool_->availableBlocksNum()
            << " | state_swa pool: total=" << state_swa_pool_->totalBlocksNum()
            << " free=" << state_swa_pool_->freeBlocksNum() << " available=" << state_swa_pool_->availableBlocksNum();
        return oss.str();
    }
    if (isDualPool()) {
...
void KVCacheMemoryConnector::reportMetricsLoop() {
    ...
    while (!stop_.load()) {
        if (metrics_reporter_) {
            const auto item_num = usePrefixTreeMemoryCache() ? (prefix_block_cache_ ? prefix_block_cache_->size() : 0) :
            ...
            if (usePrefixTreeMemoryCache()) {
                ...
                collector.used_ratio          = total_blocks == 0 ? 0.0f :
            ...
            if (complete_disk_pool_) {
                ...
                disk_collector.used_ratio          = total_disk_slots == 0 ?
                                                         0.0f :
