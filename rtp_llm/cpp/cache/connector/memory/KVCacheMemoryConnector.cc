...
static void applySplitKvMultiCopyFieldsIfEligible(bool enable_sm_copy, const CacheConfig& cfg, MultiCopyParams& out) {
    ...
    const auto tag                  = requireSingleCacheTag(cfg);
    out.split_kv_layer_num          = static_cast<int>(cfg.totalLayerNum());
    out.split_kv_cache_stride_bytes = cfg.kvBlockStrideBytesForGroup(tag);
    out.split_kv_scale_stride_bytes = cfg.kvScaleStrideBytesForGroup(tag);
}
...
// Staged-copy chunk sizing: keeping every execStagedMemoryCopy call within the scratch that
// was pre-warmed at init means the copy path never calls cudaHostAlloc/cudaMalloc mid-flight.
// Those allocations implicitly synchronize the whole device and can deadlock against a
// spinning collective kernel when a worker rank services a copy while its engine loop is
// already parked inside the next step's TP broadcast.
static constexpr size_t kStagedCopyChunkBytes = 64ull * 1024 * 1024;
static constexpr size_t kStagedCopyMaxTiles   = 8192;
...
static std::string requireSingleCacheTag(const CacheConfig& config) {
    RTP_LLM_CHECK_WITH_INFO(config.topology().groups().size() == 1,
                            "split-KV memory copy requires one global cache group, got %zu",
...
KVCacheMemoryConnector::KVCacheMemoryConnector(
    ...
    const std::shared_ptr<CoordinatorKVCacheManager>& coordinator_cache_manager,
    ...
    tp_addrs_(tp_addrs),
    layer_group_slots_(buildLayerGroupSlots(cache_config)),
...
KVCacheMemoryConnector::~KVCacheMemoryConnector() {
    ...
    state_swa_pool_.reset();
    {
        ...
        for (auto& [_, scratches] : staged_copy_scratch_by_device_) {
...
bool KVCacheMemoryConnector::isFullOnlySlot(const LayerGroupSlot& slot) const {
...
void KVCacheMemoryConnector::initBlockPool() {
    ...
    const bool  typed_opaque_layout   = cache_config_.usesTypedCacheRegions() && cache_config_.usesOpaqueKVCacheStore();
    ...
    const bool use_dual =
        hasTypedLayerGroupSlots(slots) && full_only_block_size > 0 && full_only_block_size < total_block_size;
...
std::vector<KVCacheMemoryConnector::LayerGroupSlot>
KVCacheMemoryConnector::buildLayerGroupSlots(const CacheConfig& cache_config) {
    std::vector<LayerGroupSlot> slots;
    const size_t                layer_num = cache_config.totalLayerNum();
    ...
    for (size_t layer = 0; layer < layer_num; ++layer) {
        for (const auto& tag : cache_config.topology().layer(static_cast<int>(layer)).group_tags) {
            ...
            slots.push_back(LayerGroupSlot{static_cast<int>(layer),
                                           tag,
                                           cache_config.kvBlockStrideBytesForGroup(tag)
                                               + cache_config.kvScaleStrideBytesForGroup(tag),
                                           policy.group_type,
                                           block_kind});
        }
    }
...
const std::vector<KVCacheMemoryConnector::LayerGroupSlot>& KVCacheMemoryConnector::layerGroupSlots() const {
    return layer_group_slots_;
}

bool KVCacheMemoryConnector::hasTypedLayerGroupSlots(const std::vector<LayerGroupSlot>& slots) const {
    if (slots.size() != cache_config_.totalLayerNum()) {
        return true;
    }
...
bool KVCacheMemoryConnector::supportsTypedPrefixCacheLayout(const std::vector<LayerGroupSlot>& slots) const {
    ...
    if (cache_config_.groupNums() == 0 || cache_config_.topology().layers().size() < cache_config_.totalLayerNum()) {
        return false;
    }
    ...
    for (size_t layer = 0; layer < cache_config_.totalLayerNum(); ++layer) {
...
KVCacheMemoryConnector::sendCopyPlan(const std::shared_ptr<CopyPlan>& copy_plan) const {
    ...
    for (const auto& copy_info : copy_plan->copy_infos) {
        ...
        RTP_LLM_CHECK_WITH_INFO(copy_info.gpu_blocks.size() == slots.size(),
                                "cannot encode memory copy blocks by tag: blocks=%zu slots=%zu",
...
bool KVCacheMemoryConnector::copyCache(const MemoryOperationRequestPB& wire_request,
    ...
    const auto& slots            = layerGroupSlots();
    const bool  has_typed_slots  = hasTypedLayerGroupSlots(slots);
    ...
    if (has_typed_slots && tryCopyCacheWithBatchedMemoryCopy(items, copy_direction, slots)) {
        response.set_success(true);
        reportCopyMetrics(true, timer.done_us(), copy_direction);
        return true;
    }
...
KVCacheMemoryConnector::normalizeCopyItemGpuBlocks(const MemoryOperationRequestPB::CopyItem& item,
    ...
    std::map<std::pair<int, std::string>, BlockIdxType> group_blocks;
    for (const auto& tagged_block : item.tagged_gpu_blocks()) {
        ...
        RTP_LLM_CHECK_WITH_INFO(group_blocks.emplace(key, tagged_block.block_id()).second,
    ...
    gpu_blocks.reserve(slots.size());
    for (const auto& slot : slots) {
        const auto it = group_blocks.find({slot.layer_id, slot.tag});
        RTP_LLM_CHECK_WITH_INFO(it != group_blocks.end(),
                                "memory copy request missing local layer=%d tag=%s",
                                slot.layer_id,
                                slot.tag.c_str());
        gpu_blocks.push_back(it->second);
    }
    RTP_LLM_CHECK_WITH_INFO(group_blocks.size() == slots.size(),
...
bool KVCacheMemoryConnector::copyMemoryItemsGeneric(const NormalizedCopyItems&         items,
    ...
    for (const auto& item : items) {
        ...
        const bool prepared =
            prepareCopyBuffers(item.mem_block, item.gpu_blocks, direction, item.is_complete, dst_buffers, src_buffers);
        if (!prepared) {
    ...
    if (!dst_buffers.empty()) {
        ...
        const bool      can_use_split_kv_copy = cache_config_.topology().hasSingleGlobalGroup();
...
bool KVCacheMemoryConnector::copyDiskItem(const NormalizedCopyItem&          item,
    ...
    if (!disk_pool || item.gpu_blocks.size() != slots.size()) {
        return false;
    }
...
bool KVCacheMemoryConnector::tryCopyCacheWithStagedMemoryCopy(const NormalizedCopyItems&         items,
                                                              CopyDirection                      direction,
                                                              const std::vector<LayerGroupSlot>& slots) {
    ...
    const size_t chunk_bytes_limit =
        staged_copy_chunk_bytes_ > 0 ? staged_copy_chunk_bytes_ : kStagedCopyChunkBytes;
    ...
    for (const auto& item : items) {
        ...
        for (size_t slot_idx = 0; slot_idx < slots.size(); ++slot_idx) {
            ...
            for (const auto& gpu_buffer : gpu_buffers) {
                ...
                if (!params.tiles.empty()
                    && (params.tiles.size() >= kStagedCopyMaxTiles
                        || alignUp(params.host_bytes, kStagedTileAlignment) + gpu_buffer.size_bytes
                               > chunk_bytes_limit)) {
                    if (!flushChunk()) {
                        return false;
                    }
    ...
    if (params.tiles.empty() && chunk_num == 0) {
        return true;
    }

    RTP_LLM_LOG_DEBUG("cuda staged memory copy, direction=%s, rows=%zu, bytes=%zu, chunks=%zu, device=%d",
                      direction == CopyDirection::H2D ? "H2D" : "D2H",
                      logical_rows,
                      payload_bytes,
                      chunk_num + (params.tiles.empty() ? 0 : 1),
                      params.device_index);
    return flushChunk();
}

std::unique_ptr<StagedMemoryCopyScratch> KVCacheMemoryConnector::acquireStagedCopyScratchForDevice(int device_index) {
    std::unique_lock<std::mutex> lock(staged_copy_scratch_mutex_);
    auto&                        scratches = staged_copy_scratch_by_device_[device_index];
    if (scratches.empty() && staged_copy_prewarmed_devices_.count(device_index) > 0) {
        // Wait for the pre-warmed scratch instead of allocating a fresh one: growing scratch
        // mid-copy calls cudaHostAlloc/cudaMalloc, which device-synchronize and can deadlock
        // against an in-flight collective kernel.
        staged_copy_scratch_cv_.wait_for(
            lock, std::chrono::minutes(10), [&scratches]() { return !scratches.empty(); });
        if (scratches.empty()) {
            RTP_LLM_LOG_WARNING("staged copy scratch wait timed out on device %d, allocating fresh scratch",
                                device_index);
        }
    }
    if (scratches.empty()) {
        return std::make_unique<StagedMemoryCopyScratch>();
    }
    auto scratch = std::move(scratches.back());
    scratches.pop_back();
    return scratch;
}

void KVCacheMemoryConnector::recycleStagedCopyScratchForDevice(int                                      device_index,
    ...
    staged_copy_scratch_cv_.notify_one();
}

void KVCacheMemoryConnector::prewarmStagedCopyScratch() {
    const auto& slots      = layerGroupSlots();
    size_t      max_stride = 0;
    for (const auto& slot : slots) {
        max_stride = std::max(max_stride, slot.stride_bytes);
    }
    staged_copy_chunk_bytes_ = std::max(kStagedCopyChunkBytes, alignUp(max_stride, 16));

    auto scratch = std::make_unique<StagedMemoryCopyScratch>();
    if (!prewarmStagedMemoryCopyScratch(
            *scratch, /*device_index=*/-1, staged_copy_chunk_bytes_, kStagedCopyMaxTiles)) {
        RTP_LLM_LOG_INFO("staged copy scratch prewarm unavailable, staged copies fall back to lazy allocation");
        return;
    }
    const int device_index = scratch->device_index;
    {
        std::lock_guard<std::mutex> lock(staged_copy_scratch_mutex_);
        staged_copy_scratch_by_device_[device_index].push_back(std::move(scratch));
        staged_copy_prewarmed_devices_.insert(device_index);
    }
    RTP_LLM_LOG_INFO("staged copy scratch prewarmed: device=%d chunk_bytes=%zu max_tiles=%zu",
                     device_index,
                     staged_copy_chunk_bytes_,
                     kStagedCopyMaxTiles);
}
...
KVCacheMemoryConnector::resourceLayerRegionBlocks(const KVCacheResource&             resource,
                                                  const std::vector<LayerGroupSlot>& slots) const {
    LayerAttnBlockIds blocks(cache_config_.totalLayerNum());
    for (const auto& slot : slots) {
