...
KVCacheConnectorCoordinator::asyncRead(const std::shared_ptr<KVCacheConnectorReadWriteContext>& connector_context) {
    ...
    CacheKeysByTag ref_keys_by_tag;
    for (const auto& group : cache_config_.topology().groups()) {
        ...
        if (cp_size > 1 && !kvcache_resource.cacheKeysAreCpCanonical(group.tag)) {
            ...
            if (mapper.usesCpCanonicalKeys(cache_config_, group.tag)) {
                ...
                if (!canonical.empty()) {
                    ref_keys_by_tag.emplace(group.tag, std::move(canonical));
                }
            } else {
                ref_keys_by_tag.emplace(group.tag, keys);
            }
        } else {
            ref_keys_by_tag.emplace(group.tag, keys);
        }
    }
    if (ref_keys_by_tag.empty()) {
        return nullptr;
    }
    auto resource = allocator_->incrKVCacheRef(kvcache_resource, ref_keys_by_tag, true);
    if (!resource) {
