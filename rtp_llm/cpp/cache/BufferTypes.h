...
struct CacheLayerLayout {
    ...
    std::vector<std::map<std::string, torch::Tensor>> layers_to_kv_buffer_ptrs_by_tag;
    std::vector<std::map<std::string, torch::Tensor>> layers_to_scale_buffer_ptrs_by_tag;
};
