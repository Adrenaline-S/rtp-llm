...
public:
    ...
    struct UnifiedCacheItem {
        ...
        std::unordered_map<std::string, TaggedBlock> blocks_by_tag;
        int64_t                                      created_time_us = 0;
    ...
    struct EvictResult {
        ...
        std::unordered_map<CacheKeyType, std::vector<UnifiedCacheItem::TaggedBlock>> evicted_blocks_by_tag;
    ...
    struct MatchResult {
        bool                                       found = false;
        std::vector<UnifiedCacheItem::TaggedBlock> blocks_by_tag;
    };
...
public:
    ...
    void init(const std::vector<TaggedPool>& pools);

    void put(CacheKeyType cache_key, const std::vector<UnifiedCacheItem::TaggedBlock>& blocks, bool is_resident);
...
private:
    ...
    std::unordered_map<std::string, BlockPoolPtr>                                          pools_by_tag_;
