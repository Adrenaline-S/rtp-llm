...
public:
    ...
    // True when the group maintains only a bounded tail of blocks instead of the whole prefix, so its
    // block count reflects that window rather than the matched prefix length.
    // False when groups are not initialized or the tag is unknown.
    bool maintainsTailBlocksOnly(std::string_view tag) const;
