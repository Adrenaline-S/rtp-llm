...
ErrorInfo DecodeRpcServer::loadCacheAsyncForTp(DecodeGenerateContext& decode_context,
    ...
    for (int i = 0; i < worker_size; i++) {
        ...
        if (load_cache_config.usesMla() || load_cache_config.usesOpaqueKVCacheStore()) {
...
grpc::Status DecodeRpcServer::RemoteLoad(grpc::ServerContext*          server_context,
    ...
    GroupBlockIds             group_block_ids = decodeGroupBlockIds(*request, topology);
...
DecodeRpcServer::GroupBlockIds DecodeRpcServer::decodeGroupBlockIds(const BroadcastLoadRequestPB& request,
