...
public:
    using TaggedBlockIdsMap = std::map<std::string, std::shared_ptr<BlockIds>>;
    DecodeRpcServer() {}
    ~DecodeRpcServer();
...
private:
    ...
    BroadcastLoadRequestPB constructRemoteLoadRequestForMla(const LoadKVCacheContext&       load_context,
                                                            int                             index,
                                                            const std::vector<std::string>& peer_ips) const;
    static TaggedBlockIdsMap decodeGroupBlockIds(const BroadcastLoadRequestPB& request, const CacheTopology& topology);
