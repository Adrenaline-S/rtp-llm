...
struct SystemPromptParams {
    SystemPromptParams() {}
    SystemPromptParams(const std::vector<int>& prompt_tokens, std::map<std::string, std::vector<int>> blocks_by_tag):
        prompt_tokens(prompt_tokens), blocks_by_tag(std::move(blocks_by_tag)) {}
