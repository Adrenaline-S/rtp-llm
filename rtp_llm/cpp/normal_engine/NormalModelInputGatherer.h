...
public:
    ...
    // Build a packed host/device block-table snapshot for the MTP refresh path.
    // Read-only over streams: no step() and no unrelated gather sub-step.
    // Empty input returns four defined empty tensors and an empty plan.
    absl::StatusOr<PackedBlockTableSnapshot> gatherKvCacheKernelBlockId(const StreamGroups& stream_groups,
                                                                        TensorHolder&       host_holder,
                                                                        bool include_pool_tables = false) const;
    CacheBlockTablePackingPlan               blockTablePackingPlan(const StreamGroups& stream_groups) const;

private:
    CacheBlockTablePackingPlan makeBlockTablePackingPlan(const StreamGroups& stream_groups) const;
