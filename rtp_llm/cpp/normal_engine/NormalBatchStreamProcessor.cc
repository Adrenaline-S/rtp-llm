...
NormalBatchStreamProcessor::NormalBatchStreamProcessor(
    ...
    auto layer_group_ids                                    = cache_config.layerGroupIdsSnapshot();
    model_input_gatherer_config_.kv_cache_layer_to_group.reserve(layer_group_ids.size());
    for (const auto& gids : layer_group_ids) {
        RTP_LLM_CHECK_WITH_INFO(gids.size() <= 1,
                                "NormalBatchStreamProcessor expects at most one cache group per layer, got %zu",
                                gids.size());
        model_input_gatherer_config_.kv_cache_layer_to_group.push_back(gids.empty() ? 0 : gids.front());
    }
