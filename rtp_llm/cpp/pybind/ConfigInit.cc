    ...
    py::class_<KVCacheSpecDesc>(m, "KVCacheSpecDesc")
        ...
        .def(py::pickle(
            [](const KVCacheSpecDesc& self) {
                ...
                return py::make_tuple(kSchemaVersion,
                                      self.tag,
                                      self.cache_type,
                                      self.dtype,
                                      self.is_state_cache,
            ...
            [](py::tuple t) {
                ...
                if (t.size() == 20) {
                    throw std::runtime_error(
                        "KVCacheSpecDesc pickle schema v0 is unsupported; expected versioned schema v1");
                }
                if (t.size() != 21 || t[0].cast<int>() != 1) {
                    throw std::runtime_error("Invalid KVCacheSpecDesc pickle schema; expected version 1");
                }
