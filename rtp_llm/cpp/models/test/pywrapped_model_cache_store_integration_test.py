...
from rtp_llm.cpp.models.test.libth_pywrapped_model_cache_store_integration_test import (
    PyModelInputs,
    PyModelOutputs,
    run_invalid_group_input_diagnostics,
    run_scenario,
)
...
class PyWrappedModelCacheStoreIntegrationTest(unittest.TestCase):
    ...
    def test_group_inputs_reject_missing_tags_before_python_forward(self) -> None:
        model = CacheStoreForwardModel()
        with self.assertRaisesRegex(RuntimeError, "cache tags"):
            run_scenario(model, "missing_group_tags")
        self.assertEqual(model.forward_calls, 0)
    ...
    def test_group_inputs_reorder_tags_and_all_parallel_rows_together(self) -> None:
        canonical = run_scenario(CacheStoreForwardModel(), "multi_tag")
        reordered = run_scenario(CacheStoreForwardModel(), "reordered_group_tags")
    ...
    def test_group_inputs_reject_all_invalid_parallel_payloads_before_publication(
        self,
    ) -> None:
        for scenario in (
            "duplicate_group_tags",
            "unknown_group_tag",
            "unequal_group_types",
            ...
            with self.subTest(scenario=scenario):
                self._assert_group_input_failure_has_no_publication(scenario)
