...
GptModelInputs makeInputs(const CacheConfig&          cache_config,
    ...
    std::vector<size_t> sorted_group_ids(groups.size());
    std::iota(sorted_group_ids.begin(), sorted_group_ids.end(), size_t{0});
    std::sort(sorted_group_ids.begin(), sorted_group_ids.end(), [&](size_t lhs, size_t rhs) {
        return groups[lhs].tag < groups[rhs].tag;
    });
    for (const size_t group_id : sorted_group_ids) {
        ...
        inputs.kv_cache_block_ids_by_group.push_back(table);
        inputs.kv_cache_kernel_block_ids_by_group.push_back(table.clone().pin_memory());
    }
