...
void tpSyncModelInputs(GptModelInputs& inputs, const ParallelismConfig& parallelism_config) {
    ...
    for (size_t i = 0; i < kv_cache_group_num; ++i) {
        ...
        const bool        type_inserted =
            group_types.emplace(tag, static_cast<CacheGroupType>(shape_hints_ptr[group_hint_offset + 1])).second;
        const bool width_inserted =
            group_widths.emplace(tag, static_cast<size_t>(shape_hints_ptr[group_hint_offset + 2])).second;
        const bool kernel_width_inserted =
            group_kernel_widths.emplace(tag, static_cast<size_t>(shape_hints_ptr[group_hint_offset + 3])).second;
