        ...
        if (params.kv_cache_layer_layout.has_value()) {
            for (const auto& group : params.kv_cache_layer_layout->topology().groups()) {
                ...
                RTP_LLM_CHECK_WITH_INFO(physical_tokens_per_block > 0,
                                        "CUDA graph KV cache tag=%s physical tokens per block must be positive",
                                        group.tag.c_str());
                RTP_LLM_CHECK_WITH_INFO(kernel_tokens_per_block > 0,
                                        "CUDA graph KV cache tag=%s kernel tokens per block must be positive",
                                        group.tag.c_str());
                RTP_LLM_CHECK_WITH_INFO(physical_tokens_per_block % kernel_tokens_per_block == 0,
                                        "CUDA graph KV cache tag=%s physical tokens per block=%zu must be divisible by "
                                        "kernel tokens per block=%zu",
                                        group.tag.c_str(),
                                        physical_tokens_per_block,
                                        kernel_tokens_per_block);
                const int64_t physical_tokens_per_block_i64 = static_cast<int64_t>(physical_tokens_per_block);
                const int64_t kernel_tokens_per_block_i64   = static_cast<int64_t>(kernel_tokens_per_block);
                const int64_t physical_block_count =
                    (static_cast<int64_t>(params.max_seq_len) + physical_tokens_per_block_i64 - 1)
                        / physical_tokens_per_block_i64
                    + static_cast<int64_t>(graph_params.sp_steps);
                const int64_t kernel_block_count =
                    physical_block_count * physical_tokens_per_block_i64 / kernel_tokens_per_block_i64;
                graph_params.kv_cache_block_table_capacities.emplace(
                    group.tag, CacheBlockTableCapacity{physical_block_count, kernel_block_count});
            }
        }
