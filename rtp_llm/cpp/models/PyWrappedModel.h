    ...
    if (enable_cuda_graph_ && !params.kv_cache_layer_layout.has_value() && !is_prefill_cuda_graph_mode) {
        ...
        if (params.kv_cache_layer_layout.has_value()) {
            for (const auto& group : params.kv_cache_layer_layout->topology().groups()) {
                ...
                graph_params.physical_tokens_per_block_by_tag.emplace(group.tag, group.seq_size_per_block);
                graph_params.kernel_tokens_per_block_by_tag.emplace(group.tag, group.kernel_seq_size_per_block);
            }
        }
