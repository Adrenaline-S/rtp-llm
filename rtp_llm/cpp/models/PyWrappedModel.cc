...
std::optional<PyCacheStoreInputs> PyWrappedModel::prepareWriteCacheParams(const GptModelInputs& inputs) {
    ...
    if (!inputs.warmup && inputs.pd_separation) {
        ...
        for (const auto& group : topology.groups()) {
            ...
            cache_store_inputs.tokens_per_block_by_tag.emplace(group.tag,
...
static BlockTablesByGroup
sliceBlockTablesByBatch(const BlockTablesByGroup& block_tables, size_t batch_offset, size_t batch_size) {
    BlockTablesByGroup sliced;
