...
torch_ext::PyAttentionInputs PyWrappedModel::buildPyAttentionInputs(const GptModelInputs& inputs) {
    ...
    py_attn_inputs.prefix_lengths_device = tensorHoldHostAndToCuda(py_attn_inputs.prefix_lengths);
    py_attn_inputs.input_lengths_device  = tensorHoldHostAndToCuda(py_attn_inputs.input_lengths);
...
PyWrappedModel::setupKVCacheForAttentionInputs(torch_ext::PyAttentionInputs& py_attn_inputs,
    ...
    for (const auto& [tag, table] : inputs.block_tables_by_tag) {
        ...
        group_inputs.kv_cache_kernel_block_id        = table.kernel_block_ids;
        group_inputs.kv_cache_kernel_block_id_device = tensorHoldHostAndToCuda(group_inputs.kv_cache_kernel_block_id);
        if (table.block_ids.defined()) {
            RTP_LLM_CHECK_WITH_INFO(
                table.block_ids.dim() == 2, "physical block table must be two-dimensional for tag=%s", tag.c_str());
            group_inputs.kv_cache_block_id        = table.block_ids;
    ...
    if (by_tag.size() == 1) {
        for (const auto& [tag, group_inputs] : by_tag) {
            (void)tag;
            py_attn_inputs = group_inputs;
        }
        return {};
    }
    return by_tag;
...
std::optional<PyCacheStoreInputs> PyWrappedModel::prepareWriteCacheParams(const GptModelInputs& inputs) {
    ...
    if (!inputs.warmup && inputs.pd_separation) {
        ...
        const auto& topology = cache_manager_->cacheConfig().topology();
        RTP_LLM_CHECK_WITH_INFO(!inputs.block_tables_by_tag.empty(),
                                "PD cache-store requires tagged block-table metadata");
        ...
        for (const auto& [tag, table] : inputs.block_tables_by_tag) {
            RTP_LLM_CHECK_WITH_INFO(
                table.tag == tag, "cache-store block-table key/tag mismatch for tag=%s", tag.c_str());
            RTP_LLM_CHECK_WITH_INFO(
                topology.contains(tag), "cache-store block-table tag=%s is missing from cache topology", tag.c_str());
            const auto& group = topology.group(tag);
            cache_store_inputs.kv_cache_group_policies.emplace(tag, group.policy);
            cache_store_inputs.tokens_per_block_by_tag.emplace(tag, table.seq_size_per_block);
            cache_store_inputs.kv_block_stride_bytes_by_tag.emplace(tag, table.kv_block_stride_bytes);
            cache_store_inputs.kv_scale_stride_bytes_by_tag.emplace(tag, table.kv_scale_stride_bytes);
            ...
            cache_store_inputs.kv_block_transfer_bytes_by_tag.emplace(tag, table.kv_block_stride_bytes);
            cache_store_inputs.kv_scale_transfer_bytes_by_tag.emplace(tag, table.kv_scale_stride_bytes);
        }
        if (inputs.block_tables_by_tag.size() == 1) {
            const auto& table                        = inputs.block_tables_by_tag.begin()->second;
            cache_store_inputs.tokens_per_block      = table.seq_size_per_block;
            cache_store_inputs.kv_block_stride_bytes = table.kv_block_stride_bytes;
            cache_store_inputs.kv_scale_stride_bytes = table.kv_scale_stride_bytes;
        }
...
GptModelOutputs PyWrappedModel::forwardMicroBatched(const GptModelInputs& inputs) {
    ...
    for (size_t i = 0; i < split_inputs.size(); ++i) {
        const auto& micro_inputs     = !split_inputs[i].block_tables_by_tag.empty() ? split_inputs[i] : split_inputs[0];
...
static BlockTablesByTag
sliceBlockTablesByBatch(const BlockTablesByTag& block_tables, size_t batch_offset, size_t batch_size) {
    BlockTablesByTag sliced;
    for (const auto& [tag, table] : block_tables) {
        auto entry             = table;
        entry.block_ids        = table.block_ids.narrow(0, batch_offset, batch_size);
        entry.kernel_block_ids = table.kernel_block_ids.narrow(0, batch_offset, batch_size);
        sliced.emplace(tag, std::move(entry));
    }
    return sliced;
}
