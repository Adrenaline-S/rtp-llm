...
class TestComputeLocalLastHidden(unittest.TestCase):
    ...
    def _run_case(self, stream_lens, cp_size, hidden_dim=3, device="cpu"):
        ...
        gathered_layout = torch.zeros(
            (padded_total, hidden_dim), dtype=torch.float32
        )
        gathered_layout.index_copy_(
            0, restore_indice.to(torch.long), original_hidden
        )
        rank_chunks = gathered_layout.reshape(
            cp_size, total_token_size, hidden_dim
        ).to(device)
